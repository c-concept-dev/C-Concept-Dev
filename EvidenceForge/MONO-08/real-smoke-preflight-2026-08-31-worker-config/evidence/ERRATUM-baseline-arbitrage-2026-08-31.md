...
dans ce dossier pour MONO-07 et MONO-08 v0.5 (via
`evidence/frozen-zips-worker-config-tour.sha256` et
`evidence/frozen-integrity-cross-tour-check.txt`) :
...
- Le résultat mesuré du preflight officiel de ce tour reste `overallStatus: BLOCKED`
  (`MONO-08 — ENVIRONMENT_BLOCKED`), y compris avec la configuration providers Cloudflare
  Workers testée dans ce tour : cette divergence de baseline n'affecte ni la cause du
  blocage (politique d'egress réseau de l'environnement, mesurée indépendamment pour les
  deux architectures provider) ni la conclusion de ce tour.
- La comparaison croisée « même jeu de 9 hashes que le tour précédent »
  (`evidence/frozen-integrity-cross-tour-check.txt`) reste vraie et valable en tant que
  telle : elle prouve la stabilité des ZIP entre les deux tours, indépendamment de la
  question de savoir laquelle des deux lignées de packaging est la baseline canonique.
