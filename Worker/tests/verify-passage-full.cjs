...
const content=Array.from({length:180},(_,i)=>'Texte'+i+' exact'+i+' «é&<>».\n').join('');db.prepare('INSERT INTO chunks VALUES(?,?,?,?,?,?,?,?,?)').run('fr','b','Titre','Auteur','ifs','fr',3,5,content);db.prepare('INSERT INTO chunks VALUES(?,?,?,?,?,?,?,?,?)').run('en','b','Title','Author','ifs','en',6,8,content);
...
d=await(await run({id:'en'})).json();assert.equal(d.full_content,content);assert.equal(d.full_translated_content,calls.map(c=>'Traduit '+c.input.text).join('\n'));assert.equal(calls.map(c=>c.input.text).join(''),content);assert.ok(calls.every(c=>c.input.text.length<=500));assert.equal(calls[0].input.target_lang,'fr');assert.ok(d.is_machine_translated);
calls=[];failed=true;d=await(await run({id:'en'})).json();assert.equal(d.translation_failed,true);assert.equal(d.is_machine_translated,false);assert.equal(d.full_content,content);assert.equal(d.full_translated_content,undefined);
env.AI.run=async()=>({translated_text:'une phrase qui se répète beaucoup trop souvent '.repeat(6)});d=await(await run({id:'en'})).json();assert.equal(d.translation_failed,true);assert.equal(d.full_translated_content,undefined);
