...
const raw = await env.CLONE_KV.get(‘session:’ + patientId);
...
if (!env.CLONE_KV) return jsonErr(‘KV binding CLONE_KV not configured’, 500);
const list = await env.CLONE_KV.list({ prefix: ‘session:’ });
const keys = (list.keys || []).map(k => ({
patientId : k.name.replace(‘session:’, ‘’),
