...
(async () => {
  ...
  await T("BUDGET-UI-01 / BUDGET-UI-02", "saisie réelle max=3 alerte=2 → création → budget.json = 3 / 2 → interface « plafond 3.00 / alerte 2.00 » → reprise conserve les valeurs", async () => {
    ...
    const CDP = 9343; const chrome = spawn(CH, ["--headless=new", "--disable-gpu", "--no-sandbox", "--remote-debugging-port=" + CDP, "--window-size=1280,1600", "--user-data-dir=" + tmp(), "about:blank"], { stdio: "ignore" }); await sleep(1500);
    try {
