...
(async () => {
  ...
  await T("LAUNCH-06 / LAUNCH-10", "port libre : start lance la version active, /api/config renvoie EXACTEMENT " + VERSION + ", résumé complet (worker, modèle, pricing, lots, runs, git), journal écrit", async () => {
    ...
    const c = await (await fetch("http://127.0.0.1:" + PORT + "/api/config")).json(); assert(c.version === VERSION && c.runtime && c.runtime.pid === running.child.pid && c.runtime.runsRoot === RUNS, JSON.stringify(c.runtime)); assert(fs.existsSync(running.logPath) && new RegExp(VERSION.replace(/\./g, "\\.")).test(fs.readFileSync(running.logPath, "utf8"))); });
