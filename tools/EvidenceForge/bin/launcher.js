...
async function checkWorker(env, PD, timeoutMs) { const d = await PD.probeWorker({ env, timeoutMs: timeoutMs || 15000 }); const cause = { PROVIDER_DNS_ERROR: "DNS", PROVIDER_CONNECTION_REFUSED: "CONNEXION", PROVIDER_TLS_ERROR: "TLS", PROVIDER_TIMEOUT: "TIMEOUT", PROVIDER_AUTH_ERROR: "AUTH", PROVIDER_ROUTE_NOT_FOUND: "ROUTE", NETWORK_UNAVAILABLE: "RÉSEAU", PROVIDER_CAPACITY: "CAPACITÉ", PROVIDER_RATE_LIMITED: "DÉBIT" }[d.code] || d.code;
  const proxyInfo = d.proxyVersion ? " · proxy " + d.proxyVersion + (d.proxyTimeouts ? " (" + d.proxyTimeouts + ")" : "") : (d.ready ? " · proxy sans version annoncée (v0.6 / v0.7 : délai en-têtes 8 s)" : "");   /* v1.0.9 : quel proxy est déployé ? */
  return { ok: d.ready === true, code: d.code, reachable: d.reachable, authOk: d.authOk, httpStatus: d.httpStatus, ms: d.ms, proxyVersion: d.proxyVersion || null, proxyTimeouts: d.proxyTimeouts || null, message: d.ready ? "joignable, clé acceptée (sonde gratuite, " + d.ms + " ms)" + proxyInfo : "Worker inaccessible — Cause : " + cause + " — " + d.userMessage, action: d.ready ? null : (d.code === "PROVIDER_TIMEOUT" || d.code === "NETWORK_UNAVAILABLE" ? "vérifier la connexion réseau puis réessayer" : "./tools/EvidenceForge/setup.sh") }; }
...
async function doctor(opts) {
  ...
  row("worker DNS / HTTP", worker.ok || worker.reachable ? { ok: true, message: worker.reachable ? "joignable" + (worker.httpStatus ? " (HTTP " + worker.httpStatus + ")" : "") : "" } : worker); row("worker auth", worker.ok ? { ok: true, message: "clé acceptée (sonde gratuite, aucun appel amont)" + (worker.proxyVersion ? " · proxy " + worker.proxyVersion + " · " + (worker.proxyTimeouts || "") : " · proxy sans version annoncée (v0.6 / v0.7 : délai en-têtes 8 s — déployer v0.8)") } : (worker.reachable ? worker : { ok: false, code: "SKIPPED", message: "non testable (worker injoignable)", action: worker.action }));
...
async function main() {
  ...
  try {
    ...
    if (cmd === "start") {
      ...
      out(""); out("EvidenceForge"); out("Version : " + r.version + (r.reused ? " (déjà en cours, réutilisée)" : "")); if (!r.reused) { out("Git : " + (r.gitSha || "?") + (r.repo && r.repo.remote ? " · GitHub main : " + r.repo.remote : "")); out("Worker : OK (" + r.worker.ms + " ms, sonde gratuite" + (r.worker.proxyVersion ? ", proxy " + r.worker.proxyVersion + ", " + (r.worker.proxyTimeouts || "") : ", proxy sans version annoncée : v0.6 / v0.7") + ")"); out("Model : " + r.model.model); out("Pricing : OK (" + r.model.pricingVersion + ")"); out("Frozen lots : OK"); out("Runs : " + r.runs.root); }
