...
/* v1.0.5 — cost = { ledger, budget } (optionnel) : garde de budget avant l'appel du kit et capture de l'usage fournisseur (ligne KIT_CALL), par l'adaptateur additif */
const realMono04 = (runDir, cost, stage, purpose) => wrapMono04(K("build-real-mono04.js").buildRealMono04(P.BUNDLE, process.env), { recordsPath: path.join(runDir, "ef01-evidence", "fence-normalization-records.jsonl"), rawDir: path.join(runDir, "ef01-evidence", "raw-original"),
  cost: cost ? { ledger: cost.ledger || null, budget: cost.budget || null, stage: stage || null, purpose: purpose || null } : null });
...
async function runDisciplines(input) {
  ...
  const model = ensureRealModel(); const mono04 = realMono04(input.runDir, input.cost, "DISCIPLINES", "EF-01B resolver");
...
async function runPlanner(input) {
  ...
  const model = ensureRealModel(); const mono04 = realMono04(input.runDir, input.cost, "PLAN", "EF-01C1 planner");
