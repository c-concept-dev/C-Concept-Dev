...
"use strict";
/**
 * MONO-10 v0.14 — test/test-mono10-v0.14-documentary.js
 *
 ...
 *
 * POURQUOI CE FICHIER A ETE REECRIT (audit final de v0.13, constats F1 a F4,
 * detecteurs obligatoires D-A a D-D).
 *
 ...
 *
 * Usage : node test/test-mono10-v0.14-documentary.js [racineDuPaquet]
 */
...
const VERSION_TAG = (path.basename(__filename).match(/v0\.\d+/) || ["v0.14"])[0];   // "v0.14"
...
const CITED = /annon[cç]ai(?:t|ent)|d[eé]clarai(?:t|ent)|affirmai(?:t|ent)|[eé]crivai(?:t|ent)|comptai(?:t|ent)|portai(?:t|ent)|publiai(?:t|ent)|relevai(?:t|ent)|trouvai(?:t|ent)|pr[eé]sent(?:[eé]e?s?|ait|er) comme|au lieu de|corrig[eé]|faux|fausse|sous-d[eé]clar|r[eé]alit[eé] mesur[eé]e|valeur courante|\[v0\.1\d — |historique|«|mesure de v0\.1\d|jusqu'[aà] v0\.1\d|avant v0\.1\d|remplac[eé]|a [eé]t[eé] mesur[eé]/i;
...
detector("DOC-03", "aucun document ne sur-promet l'attestation des champs",
  function (docs) {
    ...
    const VERBE = "(sont|est|seraient|serait|se trouvent?)\\s+(attest|prouv|garanti|certifi)";
    const res = [
...
detector("DOC-04", "toute phrase affirmant l'inscription du transport la qualifie dans la phrase meme",
  function (docs) {
    const AFFIRME = /le fait qu'il ait essay[eé] (est|soit) \**inscrit|\best \**inscrit\**\s+(dans|[aà]) l'artefact|inscrit dans l'artefact de capacit[eé]/i;
    ...
    Object.keys(docs).forEach(function (f) {
      blocksOf(f, docs[f]).forEach(function (b) {
        ...
        FAUX.forEach(function (x) {
          if (x.re.test(b.text) && !x.vrai.test(b.text) && !isCited(b, b.text)) {
            hits.push(f + ":" + b.line + " :: " + b.text.trim().slice(0, 80));
          }
        });
      });
    });
    ...
    Object.keys(docs).forEach(function (f) {
      blocksOf(f, docs[f]).forEach(function (b) {
        ...
        sentencesOf(b).forEach(function (sent) {
          const re = /\b(une|deux|trois|quatre|cinq|six|\d+)(?:\*\*|\*|_)?\s+r[eé]serves/gi;
          let m;
          while ((m = re.exec(sent)) !== null) {
            if (!CONSTAT.test(sent) && !CONSTAT.test(b.text)) continue;
            if (isCited(b, sent)) continue;
            const n = numOf(m[1]);
            if (n !== reel) hits.push(f + ":" + b.line + " annonce « " + m[1] + " reserves » au lieu de " + reel);
          }
        });
      });
    });
...
detector("DOC-13", "chaque commande imprimee avec un « # attendu » reproduit exactement le resultat imprime",
  ...
  [
    mut("NON-REGRESSION.md : le chiffre attendu du cadre v0.11 est altere (24 -> 26)", "NON-REGRESSION.md", function (t) {
      return mustReplace(t, /(MONO-10\/v0\.11,MONO-10\/v0\.12,MONO-10\/v0\.13,MONO-10\/v0\.14\n# attendu : SEALED_HISTORICAL_LOTS_COUNT = )24/, "$126", "attendu cadre v0.11");
    }),
    mut("NON-REGRESSION.md : la commande imprimee du cadre v0.11 n'exclut plus que v0.11 (chiffre inchange)", "NON-REGRESSION.md", function (t) {
      return mustReplace(t, /node tools\/seal-inventory\.js <racine> MONO-10\/v0\.11,MONO-10\/v0\.12,MONO-10\/v0\.13,MONO-10\/v0\.14\n/, "node tools/seal-inventory.js <racine> MONO-10/v0.11\n", "commande cadre v0.11");
    }),
  ]);
...
detector("DOC-19", "aucun statut NOT_FIXED_IN_V0xx / resolvedByV0xx d'une version anterieure n'est presente comme courant",
  ...
  [mut("OPEN-FINDINGS.md : un NOT_FIXED_IN_V014 redevient NOT_FIXED_IN_V013", "OPEN-FINDINGS.md", function (t) {
    return mustReplace(t, /NOT_FIXED_IN_V014/, "NOT_FIXED_IN_V013", "NOT_FIXED_IN_V014");
  })]);

const EXTRA_CHECK_IDS = ["COH-01", "COH-02", "COH-03", "COH-04", "COH-05", "COH-06", "RT-01", "RT-02", "RT-03", "RT-04", "RT-05",
  "SEAL-01", "SEAL-02", "SEAL-03", "SEAL-04", "SEAL-05", "SEAL-06", "MAN-01", "MAN-02", "MAN-03"];
...
/* ============================ execution ============================ */
(function () {
  console.log("MONO-10 v0.14 — controles de verite documentaire\n");
  ...
  const man = manifest || {};
  const of = docs["OPEN-FINDINGS.md"] || "";
  check("COH-01. OPEN-FINDINGS.md porte R1 a R5 avec statut explicite NOT_FIXED_IN_V014",
    ["R1", "R2", "R3", "R4", "R5"].every(function (r) { return new RegExp("\\*\\*" + r + "\\*\\*").test(of); })
    && (of.match(/NOT_FIXED_IN_V014/g) || []).length >= 5 && /NON_BLOCKING/.test(of) && /`OPEN`/.test(of));
  check("COH-02. aucune reserve n'est presentee comme resolue (MANIFEST.knownOpenFindings)",
    (man.knownOpenFindings || []).length === 5
    && (man.knownOpenFindings || []).every(function (r) { return r.resolvedByV014 === false && r.status === "OPEN" && r.blocking === false; }),
    JSON.stringify((man.knownOpenFindings || []).map(function (r) { return r.id + ":" + r.status + ":" + r.resolvedByV014; })));
  ...
  check("COH-04. MIGRATION v0.13 -> v0.14 existe et affirme l'absence de changement runtime",
    !!docs["MIGRATION-v0.13-v0.14.md"] && /Aucune API ne change/.test(docs["MIGRATION-v0.13-v0.14.md"] || ""));
  check("COH-05. le MANIFEST declare la nature documentaire et les trois identites runtime",
    /DOCUMENTAIRE/i.test(man.natureOfLot || "")
    && ["MONO-10-v0.11", "MONO-10-v0.12", "MONO-10-v0.13"].every(function (v) { return (man.runtimeByteIdenticalTo || []).indexOf(v) !== -1; }),
  ...
  const staleKeys = Object.keys(man.measurements || {}).filter(function (k) { return /V01[0-3]\b/.test(k) && !/runtimeByteIdenticalToV01[0-3]/.test(k); });
  check("MAN-03. MANIFEST : reserves R1-R5 OPEN / NON_BLOCKING / NOT_FIXED_IN_V014, closesFindings vide, aucune mesure indexee sur v0.13 recopiee comme courante",
    (man.closesFindings || []).length === 0 && (man.knownOpenFindings || []).length === 5
    && (man.knownOpenFindings || []).every(function (r) { return r.status === "OPEN" && r.blocking === false && r.resolvedByV014 === false && r.fixedIn === "NOT_FIXED_IN_V014"; })
    && staleKeys.length === 0 && man.correctsFalseStatementsOf === "MONO-10-v0.13",
  ...
  [["v0.11", "RT-01"], ["v0.12", "RT-02"], ["v0.13", "RT-03"]].forEach(function (pair) {
  ...
  check("RT-05. les outils et tests documentaires ajoutes n'ecrivent rien (aucun appel fs d'ecriture)",
    ["tools/seal-inventory.js", "test/test-mono10-v0.14-documentary.js"].every(function (t) {
  ...
  console.log("\n  -- inventaire des sceaux : quatre cadres --");
  if (!fs.existsSync(path.join(BUNDLE, "MONO-10"))) {
    skip("SEAL-01..06. inventaire", "paquet non joignable depuis cette extraction");
  } else {
    const SI = require(path.join(LOT, "tools", "seal-inventory.js"));
    const frames = [
      ["v0.11", "SEAL-01", ["MONO-10/v0.11", "MONO-10/v0.12", "MONO-10/v0.13", "MONO-10/v0.14"], [24, 1577, 9, 0]],
      ["v0.12", "SEAL-02", ["MONO-10/v0.12", "MONO-10/v0.13", "MONO-10/v0.14"], [25, 1644, 9, 0]],
      ["v0.13", "SEAL-03", ["MONO-10/v0.13", "MONO-10/v0.14"], [26, 1715, 9, 0]],
    ];
    let f11 = null;
    ...
    const f14 = SI.inventory(BUNDLE, "MONO-10/v0.14");
    console.log("  INFO  cadre v0.14 mesure : " + f14.sealedHistoricalLots + " lots, " + f14.sealedReferences + " references, "
    ...
    check("SEAL-04. cadre v0.14 : memes 9 divergences, 0 lot non scelle, v0.13 devenu historique (27 lots)",
      f14.sealedDivergences === 9 && f14.unverifiableHistoricalLots.length === 0 && f14.sealedHistoricalLots === 27,
    ...
    const nr = docs["NON-REGRESSION.md"] || "";
    check("SEAL-06. les quatre cadres sont nommes dans NON-REGRESSION.md, et le MANIFEST porte le cadre v0.14 mesure",
      ["v0.11", "v0.12", "v0.13", "v0.14"].every(function (v) { return new RegExp("cadre v0\\." + v.slice(3)).test(nr); })
      && man.sealInventory && man.sealInventory.frameV014
      && man.sealInventory.frameV014.sealedHistoricalLots === f14.sealedHistoricalLots
      && man.sealInventory.frameV014.sealedReferences === f14.sealedReferences,
      JSON.stringify(man.sealInventory && man.sealInventory.frameV014));
  }
  ...
  console.log("KNOWN_FALSE_STATEMENTS_REMAINING (mesure, DOC-01..DOC-19 hors DOC-17 qui la compare) = " + DETECTORS.reduce(function (n, d) {
