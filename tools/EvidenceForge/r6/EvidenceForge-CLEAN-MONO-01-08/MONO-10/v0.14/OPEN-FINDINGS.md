# MONO-10 v0.14 — constats ouverts
...
**non résolues par v0.14**. v0.14, comme v0.12 et v0.13, est un successeur
...
| Réserve | Statut | Bloquant ? | Corrigée en v0.14 ? | Couche où elle est compensée |
|---|---|---|---|---|
| **R1** résultat de sonde déclaré non confronté | `OPEN` | `NON_BLOCKING` | `NOT_FIXED_IN_V014` | `assertCapabilityUsable`, puis `qualifyProcess`, puis autorisation aval |
| **R2** assertion de préparation plus permissive que son consommateur | `OPEN` | `NON_BLOCKING` | `NOT_FIXED_IN_V014` | `qualifyProcess` (recalcul depuis les sources), puis autorisation aval |
| **R3** champs informatifs non attestés ; schéma non fermé | `OPEN` | `NON_BLOCKING` | `NOT_FIXED_IN_V014` | aucune couche n'en a besoin : **aucun consommateur critique ne les lit** |
| **R4** 9 divergences de sceau historiques préexistantes | `OPEN` | `NON_BLOCKING` | `NOT_FIXED_IN_V014` | hors périmètre MONO-10 — décision du propriétaire sur `MONO-07` et `MONO-08/v0.6` |
| **R5** `LLM_SUBJECT_OUT_OF_ALLOWLIST` inatteignable | `OPEN` | `NON_BLOCKING` | `NOT_FIXED_IN_V014` | la garantie comportementale tient par le refus de sujet (`LLM_SUBJECT_MISMATCH`) et par `probe()` en amont |

`NOT_FIXED_IN_V014` n'est pas une négligence : fermer l'une de ces réserves
...
**Elles ne sont imputables ni à v0.11, ni à v0.12, ni à v0.13, ni à v0.14.** Leurs dates de
...
séparément. Ni v0.12, ni v0.13, ni v0.14 ne les corrigent, ni ne prétendent le faire.
...
et `RUNTIME_BYTE_IDENTICAL_TO_V013 = YES` sont les propriétés centrales de ce
lot, et elles sont vérifiables : voir `NON-REGRESSION.md` §1.
