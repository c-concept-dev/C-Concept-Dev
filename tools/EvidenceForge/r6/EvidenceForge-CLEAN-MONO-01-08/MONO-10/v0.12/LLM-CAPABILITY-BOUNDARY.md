# MONO-10 v0.12 — frontière de capacité LLM
...
`costUsd` · `configurationPresent` · `llmBoundaryId` · `callerVerifierIgnored` ·
`schemaVersion` · `probeTimestamp` · `capabilities[]` · `authMode` ·
`probeContract` · `failureReason` · `credentialPresenceProblem`

Et, parce que **le schéma n'est pas fermé à l'enregistrement**, tout champ
additionnel arbitraire est toléré.
...
**Ce qu'il ne faut donc pas écrire** : que la certification à elle seule
garantit la véracité de tous les champs de l'artefact. Elle garantit le sujet,
l'artefact et son empreinte. Rien de plus. Consigné en
`OPEN-FINDINGS-v0.12.md` (R1).
...
observé. `LLM_SUBJECT_OUT_OF_ALLOWLIST` existe comme défense en profondeur — la
...
retour. Consigné en `OPEN-FINDINGS-v0.12.md` (R5).
