# MONO-10 v0.12 — modèle de menace
...
| ré-étiqueter vers un triplet **hors liste blanche** | la capacité reste **inutilisable** (T10). Le code observé est `LLM_SUBJECT_MISMATCH` : la branche `LLM_SUBJECT_OUT_OF_ALLOWLIST` existe mais l'ordonnancement la rend inatteignable — garantie **comportementale**, pas garantie de code (R5) |
...
| champs de surface `namespace`, `provisionedFrom`, `transportOrigin` mensongers | sans effet **utile à l'attaquant**, mais la raison n'est pas celle qui était écrite : `assertCapabilityUsable` lit le champ **de l'artefact**, sans recoupement avec le descripteur. Ce qui bloque réellement est l'impossibilité d'obtenir une `probeRef` hors de la vraie frontière (R3) |
...
## Ce que ce modèle de menace ne couvre PAS (correction v0.12)

Quatre affirmations de v0.11 promettaient plus que le code. Elles sont
corrigées ci-dessus et consignées en `OPEN-FINDINGS-v0.12.md` :

| Affirmation de v0.11 | Ce qui est vrai |
|---|---|
| « l'appelant peut dégrader une préparation, il ne peut pas l'améliorer » | faux à la couche `assertReadinessPhase` ; vrai de bout en bout (R2, `READINESS.md`) |
| « champs de surface mensongers : le descripteur authentifié prime » | il n'y a **aucun recoupement** avec le descripteur ; ce qui bloque est ailleurs (R3) |
| « le fait qu'il ait essayé est inscrit dans l'artefact » (`callerTransportIgnored`) | contrôle **auto-défaisant** : l'effacer fait passer, l'honnêteté fait échouer. Traçabilité, pas garantie (R3) |
| `LLM_SUBJECT_OUT_OF_ALLOWLIST` présenté comme le code qui s'active | branche inatteignable ; garantie comportementale seulement (R5) |
...
elles relèvent d'une décision du propriétaire (R4, `NON-REGRESSION.md` §3).
