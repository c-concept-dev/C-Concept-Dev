...
function assessCandidates(input) {
  ...
  const assessments = (discovery.candidates || []).map(function (c) {
    ...
    /**
     * §10 (v0.10) — on ne se contente plus de CONSTATER la capacite : on
     * verifie que sa derivation a ete inscrite par l'emetteur de LA frontiere
     * de ce run. v0.9 acceptait `AC.hasCapability` seul ; l'audit A a concede
     * une capacite portant `derivationRef: "je-decrete-que-cest-authentifie"`
     * et ses trois references sont entrees dans le corpus presente a l'humain.
     *
     * Un registre absent ne vaut plus laisser-passer : toutes les references
     * sont alors ecartees et consignees, jamais admises par defaut.
     */
    const idVerifier = idOpts.verifier;
    const capExpect = { runId: runId, missionHash: input.missionHash,
      executionMode: (reg && reg.executionMode) || null };
    const missionEvidenceRefs = declaredEvidenceRefs.filter(function (r) {
      let e = null;
      if (reg && typeof reg.get === "function") {
        try { e = r && r.artifactId ? reg.get(r.artifactId) : null; } catch (err) { e = null; }
      }
      const chk = AC.capabilityVerifiedBy(e, AC.CAPABILITY.AUTHENTICATED_PROVENANCE, idVerifier, capExpect);
      if (!chk.verified) {
        unauthenticatedEvidenceRefs.push({ artifactId: (r && r.artifactId) || null,
          reason: "provenance non authentifiee par l'autorite de l'exploitant : "
            + (chk.problems || []).slice(0, 1).join("") });
      }
      return chk.verified;
    });
