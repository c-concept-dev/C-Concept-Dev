...
- **B2** — `MIGRATION-v0.13-v0.14.md` portait le token `NOT_FIXED_IN_V015` suivi du mot « depuis » :
...
valeur passée au présent, aucun mot de continuité.

> **Claim de v0.17, invalidée par l'audit final de v0.17.** v0.17 affirmait
> ici que `DOC-19` ne tolérait un ancien token que dans une phrase « à l'époque
> de v0.NN » nommant le même NN, et refusait « depuis » quel que soit le
> contexte. C'était **faux** : le marqueur générique « à l'époque » du
> mécanisme de citation était évalué avant ces deux contrôles, et la faute
> réelle de v0.16 réinjectée verbatim donnait 0 hit. Le comportement réel est
> décrit et rendu causal en v0.18 (`MIGRATION-v0.17-v0.18.md` §1). Les témoins
> de v0.17 mordaient sur des formes différentes de la faute réelle.
...
- il ne ferme **aucune** des cinq réserves R1 à R5 (à l'époque de v0.17 : `NOT_FIXED_IN_V017` ; le statut courant est dans `OPEN-FINDINGS.md`) ;
