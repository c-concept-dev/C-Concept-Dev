"use strict";
/**
 * MONO-10 v0.5 — core/scientific-qualification.js  (§0, §20)
 *
...
function qualifyProcess(input) {
  ...
  if (ctx.manifest) {
    try {
      RM.assertManifestAuthentic(ctx.manifest, ctx);
      trust.attestationVerified = true; trust.authorityId = ctx.manifest.authorityId; trust.runId = ctx.manifest.runId; trust.operatorTrustBoundaryId = ctx.manifest.operatorTrustBoundaryId;
  ...
  if (production) {
    try { OTV.assertProductionVerifier(ctx.verifier, "qualification"); }
    catch (e) { trust.attestationVerified = false; trust.problems.push(e.message); }
  }
  if (!AAR.isAuthenticatedRegistry(input.artifactRegistry)) {
    trust.problems.push("registre d'artefacts non rattache a un run authentifie.");
    trust.attestationVerified = false;
  }
  ...
  [[input.readinessPre, PHASE.PRE], [input.readinessFull, PHASE.FULL]].forEach(function (p) {
    ...
    try {
      /**
       * §11 (v0.11) — le consommateur critique passe DESORMAIS le verificateur.
       * v0.10 appelait cette validation sans lui : le consommateur empruntait
       * lui-meme le chemin faible que l'audit A a exploite.
       */
      assertReadinessPhase(p[0], p[1], { registry: input.artifactRegistry,
        verifier: ctx.verifier, manifest: ctx.manifest,
...
function build(status, criteria, reservations, unknowns, limitations, input, actionable, ctx, pre, full, trust, internalOk) {
  const production = RM.isProductionContext(ctx);
  const out = {
    schema: "EvidenceForge.ScientificQualification", schemaVersion: "MONO-10-v5",
