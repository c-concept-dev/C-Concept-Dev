"use strict";
/**
 * MONO-10 v0.11 — core/lineage.js   (§38, §39, §40, §23)
 *
...
/** Chargement tardif : `operator-trust-verifier` depend de ce module. */
function OTVmod() { return require("./operator-trust-verifier.js"); }
...
function resolveLineage(refs, registry, opts) {
  ...
  /**
   * §6 (v0.10) — l'autorite d'entree historique n'est plus recue.
   *
   * v0.9 acceptait `opts.historicalInputAuthority` ou allait la chercher par
   * `verifier.historicalInputAuthority()`. v0.10 ne connait que l'OPERATION
   * `verifier.authorizeHistoricalInput(request, expectedContext)`, et le
   * contexte attendu est derive du manifeste ou du registre de DESTINATION —
   * jamais du verificateur lui-meme (fermeture B2).
   */
  if (opts.historicalInputAuthority) {
    problems.push("historicalInputAuthority fournie en argument : IGNOREE. Un emetteur ne se passe pas en parametre ; "
      + "l'autorisation se DEMANDE au verificateur de la frontiere qui a ouvert le run de destination.");
  }
  ...
  if (hiRequests.length) {
    // §4 (v0.9) — identite COMPOSITE. Un identifiant declare ne suffit pas.
    // §7/§8 (v0.10) — elle est lue sur le CONTEXTE, pas sur le verificateur.
    const src = (opts.manifest ? { operatorBoundaryId: opts.manifest.operatorTrustBoundaryId,
        configBindingHash: opts.manifest.configBindingHash, executionMode: opts.manifest.executionMode } : null)
      || (registry && registry.boundaryIdentity
        ? { operatorBoundaryId: registry.operatorTrustBoundaryId,
            configBindingHash: registry.boundaryIdentity.configBindingHash,
            executionMode: registry.executionMode } : null);
    const expectedIdentity = (src && isNonEmptyStr(src.operatorBoundaryId) && isNonEmptyStr(src.configBindingHash)
      && isNonEmptyStr(src.executionMode)) ? src : null;
    if (!expectedIdentity) {
      problems.push("entree historique demandee sans identite de frontiere operateur COMPLETE (identifiant, "
        + "liaison de configuration et mode d'execution) — refus par defaut.");
    } else if (!OTVmod().isOperatorTrustVerifier(opts.verifier)) {
      problems.push("entree historique demandee sans verificateur issu d'une frontiere operateur — refus par defaut. "
        + "Un objet d'appelant portant une methode d'autorisation n'est pas une autorite.");
    } else {
      hiRequests.forEach(function (req, i) {
        const r = opts.verifier.authorizeHistoricalInput(req, expectedIdentity);
