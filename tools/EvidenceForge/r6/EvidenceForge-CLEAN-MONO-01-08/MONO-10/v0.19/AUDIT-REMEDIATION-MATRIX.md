# MONO-10 v0.19 — matrice de remédiation (v0.12 à v0.19 : lots documentaires)
...
| résultat de sonde déclaré non confronté | Audit A, mesuré à froid par Audit B | **documenté**, non corrigé — `OPEN-FINDINGS.md` (R1) |
| champs informatifs non attestés | Audit A en relevait 5, Audit B davantage ; l'énumération canonique de v0.14 en compte **douze** nommés, plus le schéma non fermé (`LLM-CAPABILITY-BOUNDARY.md`) | **énumérés**, non corrigés — R3 |
...
|---|---|---|
| **B10-01-A/B/C** ré-étiquetage fournisseur / modèle / worker | REPRODUIT — `certified = true`, `usable = true`, hors liste blanche | sujet reconstruit depuis l'artefact et confronté au sujet réellement sondé ; liste blanche ré-appliquée. Code observé : `LLM_SUBJECT_MISMATCH`. `LLM_SUBJECT_OUT_OF_ALLOWLIST` est **inatteignable** dans l'ordonnancement actuel (`OPEN-FINDINGS.md` R5) |
...
## 4. Mesures historiques v0.9 (telles que rapportées par le lot v0.9 ; valeurs courantes : `README.md`)
...
| rendre `LLM_SUBJECT_OUT_OF_ALLOWLIST` atteignable — il est aujourd'hui **inatteignable** — ou le retirer | modifierait `core/` |
...
## 10. v0.13 et v0.14 — ce que les audits finaux de v0.12 et de v0.13 ont établi

Ni l'un ni l'autre n'a trouvé de bloqueur runtime. Les deux ont rendu
`NON_GELABLE` pour des **affirmations documentaires connues fausses** — et le
second pour des **détecteurs qui ne les auraient pas empêchées de revenir**.

| Constat | Établi par | Statut atteint en v0.14 (inchangé depuis) |
|---|---|---|
| B1, B2 : deux affirmations fausses résiduelles de v0.12 (`governance/README.md`, `LLM-CAPABILITY-BOUNDARY.md` §3) | audit final de v0.12 | corrigées en v0.13, tenues par `DOC-05` et `DOC-04` |
| KF-1 à KF-8 : huit affirmations fausses de v0.13 (hypothèses, champs non attestés, totaux de sceaux, commande imprimée, structure du `THREAT-MODEL`, `runtimeIdentityProof.paths`, fichier absent référencé, `knownFalseStatementsRemaining` pré-rempli) | audit final de v0.13 | **corrigées**, chacune tenue par un contrôle nommé (`MIGRATION-v0.13-v0.14.md` §2) |
| F1 à F4 : `DOC-04` inopérant, `DOC-08` satisfait par un mot, aucun `.json` balayé, `DOC-03` contournable | audit final de v0.13 | **corrigés** ; portée à la phrase, balayage `.md` + `.json`, huit variantes (`MIGRATION-v0.13-v0.14.md` §3) |
| D-A à D-D : détecteurs manquants (hypothèses comptées, champs dérivés, commandes rejouées, structure markdown) | audit final de v0.13 | **ajoutés** (`MIGRATION-v0.13-v0.14.md` §4) |

### Ce que cette séquence enseigne, et qu'il faut inscrire

Un lot documentaire peut lui-même porter des affirmations fausses sur ses
propres corrections : v0.13 déclarait une correction de structure qu'il n'avait
faite qu'à moitié (KF-5), et un total de sceaux qu'il n'avait pas remesuré
(KF-3). La réponse de v0.14 n'est pas seulement de corriger : c'est de faire
**exécuter** ce qui est imprimé (commandes rejouées), **compter** ce qui est
annoncé (hypothèses, champs) et **dériver** ce qui était pré-rempli
(`knownFalseStatementsRemaining`). Une affirmation qu'aucun contrôle ne peut
faire échouer n'est pas une mesure.

## 11. v0.15 à v0.19 — ce que les audits finaux de v0.14 à v0.18 ont établi

Même constante : aucun bloqueur runtime, runtime byte-identique. Les cinq
audits ont rendu `NON_GELABLE` pour des résidus documentaires — le deuxième a
montré qu'un témoin causal peut passer **pour une mauvaise raison** (`DOC-03`),
le troisième qu'un titre de version **se mesure** contre le lot qu'il nomme, le
quatrième qu'un détecteur n'est causal que contre **la faute réelle réinjectée
verbatim** (`DOC-19`), le cinquième qu'un motif doit être testé sur **toutes**
les formes qu'il promet — `\b` et Unicode — et qu'un nombre saisi dans le
`MANIFEST` finit toujours par être faux.

| Constat | Établi par | Statut en v0.19 |
|---|---|---|
| B1 (v0.14) : `OPEN-FINDINGS.md` R4 citait « 24 lots / 1 577 références » au présent, sans cadre | audit final de v0.14 | **corrigé en v0.15** : cadre historique v0.11 et cadre courant nommés ; `DOC-05` refuse tout chiffre de cadre sans cadre (`MIGRATION-v0.14-v0.15.md` §1) |
| B2 (v0.14) : `MANIFEST.v014BuildWindowOpened` saisi, non reproductible | audit final de v0.14 | **corrigé en v0.15** : méthode canonique (mtime minimal, seconde paire), valeur dérivée, commande publiée, `DOC-20` (`MIGRATION-v0.14-v0.15.md` §2) |
| lacunes de détecteurs (`DOC-03` « se trouvent? », `DOC-04` flexions, `DOC-09` « au nombre de N », `DOC-14` déplacements séparés par des lignes vides) et section « Mesures » de v0.9 non versionnée | audit final de v0.14 | **fermées en v0.15** (`MIGRATION-v0.14-v0.15.md` §3) ; cette section §4 est désormais titrée « Mesures historiques v0.9 », contrôle `DOC-21` |
| B1 (v0.15) : `DOC-03` ne détectait pas le quantificateur **post-verbal** (« les champs sont tous attestés ») — l'alternative ajoutée en v0.15 n'était atteignable qu'après un quantificateur pré-verbal, où l'ancien patron mordait déjà ; le témoin passait pour une mauvaise raison | audit final de v0.15 | **corrigé en v0.16** : patron `(sont\|figurent\|se trouvent\|…) tou(s\|tes) (attest\|prouv\|garanti\|certifi)`, les cinq phrases manquées comme témoins exacts (`MIGRATION-v0.15-v0.16.md` §1) |
| B2 (v0.15) : cette matrice plaçait des lignes corrigées en v0.15 sous le heading « v0.13 et v0.14 » et la colonne « Statut en v0.14 » | audit final de v0.15 | **corrigé en v0.16** : ce §11 les porte ; `DOC-22` refuse toute ligne postérieure à son heading ou à sa colonne de statut |
| lacunes non bloquantes : chiffres en gras invisibles pour `DOC-11`/`DOC-12`, nom de fichier racine pointé invisible pour `DOC-15`, `COH-01` par simple comptage, `DOC-05` aveugle à un chiffre faux mais cadré, titres de version périmés, « six champs » / « sept dimensions » non expliqués | audit final de v0.15 | **fermées en v0.16** (`MIGRATION-v0.15-v0.16.md` §3 et §4 ; `DOC-23`, `DOC-24`, `DOC-25`) |
| B1/B3 (v0.16) : `HISTORICAL-INPUT-AUTHORITY.md` et `LINEAGE.md`, titrés « v0.7 » et déclarés d'origine v0.7, alors que leur contenu courant apparaît pour la première fois en **v0.10** (byte-identique de v0.10 à v0.17) ; `RT-08` ne comparait qu'à v0.11 | audit final de v0.16 | **corrigé en v0.17** : `MANIFEST.originTitledDocuments` distingue l'origine **conceptuelle** (v0.7, le H1) de l'origine **de contenu** (v0.10), avec chemins, empreintes et verdict ; `DOC-24` compare au lot **nommé** et re-mesure le premier lot identique ; `MANIFEST.documentHistory` mesuré pour chaque document (`DOC-27`) |
| B2 (v0.16) : `MIGRATION-v0.13-v0.14.md`, le token `NOT_FIXED_IN_V015` suivi du mot « depuis » — ancien token présenté comme courant | audit final de v0.16 | **texte corrigé en v0.17** (« à l'époque de v0.NN : `NOT_FIXED_IN_V0NN` ; statut courant dans `OPEN-FINDINGS.md` » dans les trois migrations) ; **détecteur rendu causal en v0.18** seulement — la claim de v0.17 sur `DOC-19` était fausse (ligne suivante) |
| lacunes non bloquantes : `DOC-22` heading masqué par la colonne, `DOC-25` et `DOC-06` satisfaits par un mot, parenthèses « corrigé en » des H1 écrites à la main et fausses pour deux documents | audit final de v0.16 | **fermées en v0.17** (`MIGRATION-v0.16-v0.17.md` §4) : bornes de heading et de colonne indépendantes ; énumérations comparées comme ensembles ; non-imputabilité lue comme une négation ; parenthèses retirées, historique mesuré |
| B1 (v0.17) : `DOC-19` **non causal** — la faute B2 de v0.16 réinjectée verbatim donnait 0 hit : le marqueur générique « à l'époque » était évalué avant le couplage de version et la détection de « depuis » ; quatre documents affirmaient pourtant « même NN, depuis refusé » | audit final de v0.17 | **corrigé en v0.18** : ordre impératif continuité adjacente → couplage de version → citation, « à l'époque » n'exempte plus ; témoin obligatoire verbatim, sept variantes, trois contrôles ; claims réécrites (`MIGRATION-v0.17-v0.18.md` §1). *Couverture avant-token incomplète en v0.18 (26/32), complétée en v0.19 (ligne suivante)* |
| lacunes non bloquantes : `DOC-25` laissait passer le retrait de « plus `executionMode` » dans R1 et une dimension fantôme ; gloss « conçu en v0.7 » pour `LINEAGE.md` (existe depuis v0.1) ; emplacement de B2 mêlant faute exacte et recadrages préventifs | audit final de v0.17 | **fermées en v0.18** (`MIGRATION-v0.17-v0.18.md` §2–§4) |
| B1 (v0.18) : `DOC-19`, continuité **avant** le token — 26/32 : `\b` ne précède jamais « à » (« à ce jour »), « depuis » absent du motif avant-token ; B2 : `README.md` et `MIGRATION-v0.17-v0.18.md` promettaient les huit mots des deux côtés | audit final de v0.18 | **corrigé en v0.19** : frontières Unicode `(?<![\p{L}])` / `(?![\p{L}])` (drapeau `u`), huit mots dans les deux motifs, 32 témoins (cadre couplé × citation × avant/après), 4 contrôles ; claims réécrites, claim de v0.18 annotée invalidée (`MIGRATION-v0.18-v0.19.md` §1) |
| B3 (v0.18) : `README.md` citait une clé `fixLocation` inexistante ; B4 : `MANIFEST.measurements.documentaryChecks` « 94 témoins » saisi (113 mesurés) ; B5 : `documentHistoryNote` « v0.1..v0.16 » périmée | audit final de v0.18 | **corrigé en v0.19** : `DOC-29` (toute clé citée existe), `documentaryMeasurements` dérivé du run + `DOC-28` + `MEAS-01`, `documentHistoryMeasuredRange` mesuré + `DOC-27` (`MIGRATION-v0.18-v0.19.md` §2–§4) |
| non bloquant (v0.18) : `DOC-06` localisation au mot-clé | audit final de v0.18 | **durci en v0.19** : tableau de R4 comparé lot par lot et fichier par fichier aux divergences mesurées ; limite restante (prose hors tableau) documentée `NON_BLOCKING` |
