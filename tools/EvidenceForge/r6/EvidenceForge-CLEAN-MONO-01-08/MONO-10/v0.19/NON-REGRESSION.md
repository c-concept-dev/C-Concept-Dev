# MONO-10 v0.19 — non-régression

## 1. La propriété centrale de ce lot : le runtime est inchangé

v0.19 est un successeur **strictement documentaire** de v0.18, qui l'était de
chaque lot depuis v0.11. La seule affirmation forte qu'il porte est celle-ci,
et elle est vérifiable en **quatre-vingt-seize comparaisons** (douze chemins ×
huit lots de référence) :
...
| `RUNTIME_BYTE_IDENTICAL_TO_V018` | **YES** |
| `MONO10_V19_CAUSED_REGRESSION` | **NO** |
| `MONO10_V19_CHANGED_ANY_API` | **NO** |
| `MONO10_V19_CHANGED_ANY_BEHAVIOUR` | **NO** |
...
| | `test/test-mono10-v0.19-documentary.js` (ajout, lecture seule) |
...
```
cd MONO-10
for prev in v0.11 v0.12 v0.13 v0.14 v0.15 v0.16 v0.17 v0.18; do
  for d in core adapters validators schemas contracts; do diff -r $prev/$d v0.19/$d; done
  for f in test/fixture-chain.js test/test-mono10-v0.11.js \
    ...
    diff $prev/$f v0.19/$f; done
done
# attendu : comparaisons = 96 ; VIDES = 96
```

Sortie attendue : **rien**, sur les quatre-vingt-seize comparaisons. La ligne
`# attendu` est **rejouée** par `test/test-mono10-v0.19-documentary.js`
...
- `test/test-mono10-v0.19-documentary.js` — contrôles documentaires, réécrits en
...
```
node tools/seal-inventory.js <racine> MONO-10/v0.11,MONO-10/v0.12,MONO-10/v0.13,MONO-10/v0.14,MONO-10/v0.15,MONO-10/v0.16,MONO-10/v0.17,MONO-10/v0.18,MONO-10/v0.19
...
l'historique. **Neuf cadres, neuf jeux de chiffres, tous vrais, tous
rejoués :**

| Cadre | Lots exclus | Lots scellés | Références | Divergences | Non scellés |
|---|---|---|---|---|---|
| **cadre v0.11** | v0.11 à v0.19 | 24 | 1 577 | 9 | 0 |
| **cadre v0.12** | v0.12 à v0.19 | 25 | 1 644 | 9 | 0 |
| **cadre v0.13** | v0.13 à v0.19 | 26 | 1 715 | 9 | 0 |
| **cadre v0.14** | v0.14 à v0.19 | 27 | 1 787 | 9 | 0 |
| **cadre v0.15** | v0.15 à v0.19 | 28 | 1 860 | 9 | 0 |
| **cadre v0.16** | v0.16 à v0.19 | 29 | 1 934 | 9 | 0 |
| **cadre v0.17** | v0.17, v0.18, v0.19 | 30 | 2 010 | 9 | 0 |
| **cadre v0.18** | v0.18, v0.19 | 31 | 2 087 | 9 | 0 |
| **cadre v0.19** | v0.19 | 32 | 2 165 | 9 | 0 |

```
node tools/seal-inventory.js <racine> MONO-10/v0.12,MONO-10/v0.13,MONO-10/v0.14,MONO-10/v0.15,MONO-10/v0.16,MONO-10/v0.17,MONO-10/v0.18,MONO-10/v0.19
# attendu : SEALED_HISTORICAL_LOTS_COUNT = 25 ; SEALED_REFERENCES_COUNT = 1644 ; SEALED_DIVERGENCES = 9 ; UNVERIFIABLE_HISTORICAL_LOTS = 0
```

```
node tools/seal-inventory.js <racine> MONO-10/v0.13,MONO-10/v0.14,MONO-10/v0.15,MONO-10/v0.16,MONO-10/v0.17,MONO-10/v0.18,MONO-10/v0.19
# attendu : SEALED_HISTORICAL_LOTS_COUNT = 26 ; SEALED_REFERENCES_COUNT = 1715 ; SEALED_DIVERGENCES = 9 ; UNVERIFIABLE_HISTORICAL_LOTS = 0
```

```
node tools/seal-inventory.js <racine> MONO-10/v0.14,MONO-10/v0.15,MONO-10/v0.16,MONO-10/v0.17,MONO-10/v0.18,MONO-10/v0.19
# attendu : SEALED_HISTORICAL_LOTS_COUNT = 27 ; SEALED_REFERENCES_COUNT = 1787 ; SEALED_DIVERGENCES = 9 ; UNVERIFIABLE_HISTORICAL_LOTS = 0
```

```
node tools/seal-inventory.js <racine> MONO-10/v0.15,MONO-10/v0.16,MONO-10/v0.17,MONO-10/v0.18,MONO-10/v0.19
# attendu : SEALED_HISTORICAL_LOTS_COUNT = 28 ; SEALED_REFERENCES_COUNT = 1860 ; SEALED_DIVERGENCES = 9 ; UNVERIFIABLE_HISTORICAL_LOTS = 0
```

```
node tools/seal-inventory.js <racine> MONO-10/v0.16,MONO-10/v0.17,MONO-10/v0.18,MONO-10/v0.19
# attendu : SEALED_HISTORICAL_LOTS_COUNT = 29 ; SEALED_REFERENCES_COUNT = 1934 ; SEALED_DIVERGENCES = 9 ; UNVERIFIABLE_HISTORICAL_LOTS = 0
```

```
node tools/seal-inventory.js <racine> MONO-10/v0.17,MONO-10/v0.18,MONO-10/v0.19
# attendu : SEALED_HISTORICAL_LOTS_COUNT = 30 ; SEALED_REFERENCES_COUNT = 2010 ; SEALED_DIVERGENCES = 9 ; UNVERIFIABLE_HISTORICAL_LOTS = 0
```

```
node tools/seal-inventory.js <racine> MONO-10/v0.18,MONO-10/v0.19
# attendu : SEALED_HISTORICAL_LOTS_COUNT = 31 ; SEALED_REFERENCES_COUNT = 2087 ; SEALED_DIVERGENCES = 9 ; UNVERIFIABLE_HISTORICAL_LOTS = 0
```

```
node tools/seal-inventory.js <racine> MONO-10/v0.19
# attendu : SEALED_HISTORICAL_LOTS_COUNT = 32 ; SEALED_REFERENCES_COUNT = 2165 ; SEALED_DIVERGENCES = 9 ; UNVERIFIABLE_HISTORICAL_LOTS = 0
```
...
valeurs mesurées portées par `MANIFEST.sealInventory`. Le cadre v0.19 est
**mesuré après scellement** de v0.19, jamais postulé : la valeur « 0 non
scellé » n'est vraie qu'une fois `SHA256SUMS.txt` écrit.

### Le total, tous sceaux confondus — cadre : l'arbre complet au scellement de v0.19
...
v0.16 ; 41 / 3 570 dans l'arbre de v0.17 ; 42 / 3 648 dans l'arbre de v0.18).
Le total ne se recopie pas : il se mesure, dans l'arbre où on le publie.

```
node tools/seal-inventory.js <racine>
# attendu : ALL_SEALS_FILE_COUNT = 43 ; ALL_SEALS_REFERENCE_COUNT = 3727 ; ALL_SEALS_DIVERGENCES = 9
```

| Population (arbre complet, v0.19 scellé) | Sceaux | Références | Divergences |
|---|---|---|---|
| lots historiques distincts (cadre v0.19) | 32 | 2 165 | 9 |
| copies imbriquées sous `dependencies/` | 10 | 1 483 | 0 |
| lot candidat v0.19 | 1 | 79 | 0 |
| **tous sceaux confondus** | **43** | **3 727** | **9** |
...
à v0.15, ni à v0.16, ni à v0.17, ni à v0.18, ni à v0.19.** Leurs dates de modification vont du 31/08/2026 au 03/09/2026, soit neuf à douze
...
Ce n'est **pas** une vérité générale des arbres v0.12 à v0.19 : chaque lot
...
dans aucune des neuf fenêtres de construction** (chaque fenêtre est définie en
§7).
...
`MONO10_V17_CAUSED_REGRESSION = NO`, `MONO10_V18_CAUSED_REGRESSION = NO` et
`MONO10_V19_CAUSED_REGRESSION = NO` le sont de la même façon, chacun mesuré
...
- `test/test-mono10-v0.19-documentary.js` — échoue si une fausseté documentaire
  corrigée par ce lot ou par ses prédécesseurs réapparaît. **Vingt-huit**
  détecteurs (DOC-01 à DOC-25, DOC-27 à DOC-29), chacun validé par trois états sur le document réel :
...
```
find MONO-10/v0.19 -type f ! -name '*.zip' -exec stat -f %m {} + | sort -n | head -1 | awk '{print $1 + ($1 % 2)}'
# attendu : BUILD_WINDOW_OPENED = 1789322680
```
...
`sealInventory.v019BuildWindowOpened` est le minimum de ces mtimes, arrondi.
...
Les fenêtres de v0.11 à v0.18 (`sealInventory.v01xBuildWindowOpened`) sont
...
dans un lot historique ; pour v0.15 (`1789295186`), v0.16 (`1789306460`),
v0.17 (`1789313214`) et v0.18 (`1789315996`) les valeurs sont dérivées par
cette méthode et ont été reproduites par les audits finaux de v0.15 à v0.18.
...
scellés joignables — la plage réellement utilisée est **écrite par le
constructeur** dans `MANIFEST.documentHistoryMeasuredRange` et nommée dans
`documentHistoryNote` (v0.18 y avait laissé « v0.1..v0.16 », périmé : `DOC-27`
compare désormais la note à la plage mesurée) — et re-mesurée par le test :
...
| `HISTORICAL-INPUT-AUTHORITY.md` | v0.7 — la version que le titre affiche | v0.7 | **v0.10** — le contenu courant est celui de v0.10, byte-identique de v0.10 à v0.19 ; v0.7 n'est pas l'origine du contenu |
| `LINEAGE.md` | v0.7 — la version que le titre affiche ; le fichier n'a **pas** été « conçu en v0.7 » | **v0.1** — le fichier existe depuis le premier lot | **v0.10** — contenu de v0.10, byte-identique de v0.10 à v0.19 |
...
v0.17, v0.18 ni v0.19 : les retitrer aurait fait du lot courant l'origine de leur
