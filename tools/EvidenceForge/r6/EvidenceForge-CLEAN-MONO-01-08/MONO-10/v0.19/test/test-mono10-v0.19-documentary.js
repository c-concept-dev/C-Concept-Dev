...
"use strict";
/**
 * MONO-10 v0.19 — test/test-mono10-v0.19-documentary.js
 *
 ...
 *
 * CE QUE v0.19 CORRIGE (audit final de v0.18) :
 *   B1  DOC-19, continuite AVANT le token : le motif utilisait `\b`, qui ne
 *       precede jamais un caractere non ASCII (« à ce jour »), et omettait
 *       « depuis ». Frontiere Unicode (?<![\p{L}]) avec le drapeau u, les huit
 *       mots dans les deux motifs. Temoins : 8 formes avant le token x {cadre
 *       couple, citation explicite} = 16, et 8 formes apres le token x 2 = 16 ;
 *       4 controles PASS.
 *   B4  DOC-28 : les mesures documentaires du MANIFEST (nombre de detecteurs,
 *       de temoins, de controles, de temoins needsLots) sont DERIVEES de la
 *       structure des detecteurs et comparees ; les resultats dynamiques
 *       (PASS_CLEAN, FAIL_SINGLE_TARGET_FAULT, PASS_AFTER_REVERT) sont
 *       compares au run (MEAS-01). Plus aucun nombre saisi.
 *   B5  DOC-27 : documentHistoryNote porte la plage de lots reellement mesuree.
 *   B3  DOC-29 : tout identifiant cite comme cle du MANIFEST existe.
 *   DOC-06 : le tableau des divergences de R4 est compare lot par lot et
 *       fichier par fichier a MANIFEST.sealInventory.divergences.
 *
 * CE QUE v0.18 CORRIGEAIT (audit final de v0.17) :
 ...
 *
 * Usage : node test/test-mono10-v0.19-documentary.js [racineDuPaquet]
 */
...
const VERSION_TAG = (path.basename(__filename).match(/v0\.\d+/) || ["v0.19"])[0];   // "v0.19"
...
detector("DOC-05", "aucune mesure de sceau fausse ou perimee n'est AFFIRMEE comme courante, ni cadree avec un chiffre d'un autre cadre",
  ...
  [
    ...
    mut("OPEN-FINDINGS.md R4 : « cadre historique v0.11 … : 24 lots » devient « 25 lots »", "OPEN-FINDINGS.md", function (t) {
      return mustReplace(t, /(cadre historique v0\.11\*\* \(v0\.11 à v0\.19 exclus\) : )24 lots/, "$125 lots", "cadre historique v0.11");
    }),
    mut("AUDIT-VERDICTS.v0.12-v0.19.md (fichier racine a nom pointe) : « 24 lots / 1 577 références » sans cadre", "AUDIT-VERDICTS.v0.12-v0.19.md", function (t) {
...
detector("DOC-06", "OPEN-FINDINGS R4 documente, localise et attribue les 9 divergences",
  function (docs) {
    ...
    /** v0.19 — LOCALISATION : le tableau de R4 (lot | nombre | fichiers) est compare lot par lot
     *  et fichier par fichier a MANIFEST.sealInventory.divergences (lui-meme mesure : SEAL-05). */
    let man = null;
    try { man = JSON.parse(docs["MANIFEST.json"] || "null"); } catch (e) { manque.push("MANIFEST.json illisible"); }
    const div = (man && man.sealInventory && man.sealInventory.divergences) || null;
    if (!div) manque.push("MANIFEST.sealInventory.divergences absent");
    else {
      const rows = {};
      sec.split("\n").forEach(function (l) {
        const m = l.match(/^\|\s*`([^`]+)`\s*\|\s*(\d+)\s*\|(.*)\|\s*$/);
        if (m) rows[m[1]] = { n: parseInt(m[2], 10), files: (m[3].match(/`([^`]+)`/g) || []).map(function (x) { return x.replace(/`/g, ""); }).sort() };
      });
      Object.keys(div).forEach(function (lot) {
        if (!rows[lot]) { manque.push("le tableau de R4 n'a pas de ligne pour " + lot); return; }
        const exp = div[lot].slice().sort();
        if (rows[lot].n !== exp.length) manque.push("R4 : " + lot + " annonce " + rows[lot].n + " divergence(s), MANIFEST en mesure " + exp.length);
        if (JSON.stringify(rows[lot].files) !== JSON.stringify(exp)) manque.push("R4 : fichiers de " + lot + " " + JSON.stringify(rows[lot].files) + " ≠ MANIFEST " + JSON.stringify(exp));
      });
      Object.keys(rows).forEach(function (lot) { if (!div[lot]) manque.push("R4 : lot " + lot + " absent des divergences mesurees"); });
    }
    return manque.map(function (m) { return "OPEN-FINDINGS.md R4 : " + m; });
  },
  [
    ...
    mut("OPEN-FINDINGS.md R4 : dans le tableau, la divergence de MONO-07 est attribuee a MONO-06 (le nom MONO-07 reste dans la prose)", "OPEN-FINDINGS.md", function (t) {
      const sec = t.match(/^## R4[\s\S]*?(?=^## R5)/m)[0];
      return t.replace(sec, mustReplace(sec, /^\| `MONO-07` \| 1 \|/m, "| `MONO-06` | 1 |", "ligne MONO-07 du tableau R4"));
    }),
    mut("OPEN-FINDINGS.md R4 : dans le tableau, `package.json` de MONO-07 devient `index.js`", "OPEN-FINDINGS.md", function (t) {
      const sec = t.match(/^## R4[\s\S]*?(?=^## R5)/m)[0];
      return t.replace(sec, mustReplace(sec, /^(\| `MONO-07` \| 1 \| )`package\.json`/m, "$1`index.js`", "fichier de MONO-07"));
    }),
  ]);
...
detector("DOC-13", "chaque commande imprimee avec un « # attendu » reproduit exactement le resultat imprime",
  ...
  [
    mut("NON-REGRESSION.md : le chiffre attendu du cadre v0.11 est altere (24 -> 26)", "NON-REGRESSION.md", function (t) {
      return mustReplace(t, /(MONO-10\/v0\.11,MONO-10\/v0\.12,MONO-10\/v0\.13,MONO-10\/v0\.14,MONO-10\/v0\.15,MONO-10\/v0\.16,MONO-10\/v0\.17,MONO-10\/v0\.18,MONO-10\/v0\.19\n# attendu : SEALED_HISTORICAL_LOTS_COUNT = )24/, "$126", "attendu cadre v0.11");
    }),
    mut("NON-REGRESSION.md : la commande imprimee du cadre v0.11 n'exclut plus que v0.11 (chiffre inchange)", "NON-REGRESSION.md", function (t) {
      return mustReplace(t, /node tools\/seal-inventory\.js <racine> MONO-10\/v0\.11,MONO-10\/v0\.12,MONO-10\/v0\.13,MONO-10\/v0\.14,MONO-10\/v0\.15,MONO-10\/v0\.16,MONO-10\/v0\.17,MONO-10\/v0\.18,MONO-10\/v0\.19\n/, "node tools/seal-inventory.js <racine> MONO-10/v0.11\n", "commande cadre v0.11");
    }),
...
detector("DOC-20", "MANIFEST.sealInventory.v019BuildWindowOpened est derive de la methode canonique et re-mesurable",
  function (docs) {
    let man;
    try { man = JSON.parse(docs["MANIFEST.json"] || ""); } catch (e) { return ["MANIFEST.json illisible"]; }
    const hits = [];
    const declared = (man.sealInventory || {}).v019BuildWindowOpened;
    const unix = typeof declared === "number" ? declared : parseInt(String(declared || "").replace(/.*unix\s*/, ""), 10);
    if (!(unix > 0)) return ["v019BuildWindowOpened absent ou illisible : " + declared];
    ...
    else if (ceilEven(Math.min.apply(null, mt)) !== unix) hits.push("v019BuildWindowOpened = " + unix + " ; derive de min(files[].mtime) = " + ceilEven(Math.min.apply(null, mt)));
  ...
  [
    mut("MANIFEST.json : v019BuildWindowOpened est decale d'une seconde", "MANIFEST.json", function (t) {
      const man = JSON.parse(t); man.sealInventory.v019BuildWindowOpened = man.sealInventory.v019BuildWindowOpened + 1; return JSON.stringify(man, null, 2);
    }),
...
detector("DOC-22", "dans un registre multi-versions, aucune ligne ne decrit une correction d'une version posterieure a son heading / sa colonne de statut",
  ...
  [
    mut("AUDIT-REMEDIATION-MATRIX.md §11 : la colonne « Statut en v0.19 » redevient « Statut en v0.15 » au-dessus de lignes corrigees en v0.16 / v0.17 / v0.18", "AUDIT-REMEDIATION-MATRIX.md", function (t) {
      return mustReplace(t, /\| Statut en v0\.19 \|/, "| Statut en v0.15 |", "colonne Statut en v0.19");
    }),
    mut("AUDIT-REMEDIATION-MATRIX.md §11 : le HEADING « v0.15 à v0.19 » redevient « v0.14 et v0.15 », la colonne « Statut en v0.18 » restant juste", "AUDIT-REMEDIATION-MATRIX.md", function (t) {
      return mustReplace(t, /^## 11\. v0\.15 à v0\.19 — /m, "## 11. v0.14 et v0.15 — ", "heading §11");
    }),
...
detector("DOC-23", "le tableau de statut de OPEN-FINDINGS porte, pour chacune de R1..R5, OPEN / NON_BLOCKING / NOT_FIXED_IN_V019",
  function (docs) {
    const of = docs["OPEN-FINDINGS.md"] || "";
  ...
  [mut("OPEN-FINDINGS.md : la ligne R5 passe a CLOSED / FIXED_IN_V016, les quatre autres inchangees", "OPEN-FINDINGS.md", function (t) {
    return mustReplace(t, /(^\|\s*\*\*R5\*\*[^\n]*?)`OPEN`([^\n]*?)`NOT_FIXED_IN_V019`/m, "$1`CLOSED`$2`FIXED_IN_V019`", "ligne R5");
  })]);
...
detector("DOC-24", "chaque document a titre d'origine declare une origine de CONTENU vraie (empreintes, lot nomme, premier lot identique) ; aucun autre H1 n'est perime",
  ...
  [
    ...
    mut("README.md : le H1 redevient « MONO-10 v0.18 »", "README.md", function (t) {
      return mustReplace(t, /^# MONO-10 v0\.19/m, "# MONO-10 v0.18", "H1 README");
    }),
  ]);
...
detector("DOC-27", "MANIFEST.documentHistory est present pour chaque .md du lot et egale l'historique re-mesure sur les lots ; aucun H1 ne porte de parenthese de version ecrite a la main",
  function (docs) {
    ...
    const hits = [];
    /** v0.19 (B5) — la note de portee doit nommer exactement la plage de lots mesuree */
    const range = man.documentHistoryMeasuredRange || null;
    if (!range || !range.from || !range.to) hits.push("MANIFEST.documentHistoryMeasuredRange absent");
    else {
      if ((man.documentHistoryNote || "").indexOf(range.from + ".." + range.to) === -1) hits.push("documentHistoryNote ne nomme pas la plage mesuree " + range.from + ".." + range.to);
      if (LOTS_REACHABLE) {
        const names = sortedLotNames();
        if (names[0] !== range.from || names[names.length - 1] !== range.to) hits.push("plage declaree " + range.from + ".." + range.to + " ≠ lots joignables " + names[0] + ".." + names[names.length - 1]);
      }
    }
    Object.keys(docs).forEach(function (f) {
      if (isJson(f) || f === CHARTE) return;
  ...
  [
    ...
    mut("TRUST-MODEL.md : le H1 reprend une parenthese « (contenu de v0.12, corrigé en v0.14) »", "TRUST-MODEL.md", function (t) {
      return mustReplace(t, /^# MONO-10 v0\.19 — modèle de confiance/m, "# MONO-10 v0.19 — modèle de confiance (contenu de v0.12, corrigé en v0.14)", "H1 TRUST-MODEL");
    }),
    mut("MANIFEST.json : documentHistoryNote annonce « v0.1..v0.16 » (note perimee de v0.18)", "MANIFEST.json", function (t) {
      const man = JSON.parse(t); man.documentHistoryNote = man.documentHistoryNote.replace(man.documentHistoryMeasuredRange.from + ".." + man.documentHistoryMeasuredRange.to, "v0.1..v0.16"); return JSON.stringify(man, null, 2);
    }),
  ]);
...
/* ----- B4 (v0.19) / DOC-28 — mesures documentaires DERIVEES, jamais saisies ----- */
/**
 * MANIFEST.measurements.documentaryMeasurements est ecrit par le constructeur a
 * partir du run. Les grandeurs STATIQUES (nombre de detecteurs, de temoins, de
 * controles, de temoins exigeant les lots) sont re-derivees ici de la
 * structure des detecteurs ; les grandeurs DYNAMIQUES (PASS_CLEAN,
 * FAIL_SINGLE_TARGET_FAULT, PASS_AFTER_REVERT) sont comparees au run par MEAS-01.
 */
function staticMeasurements() {
  let witnessCount = 0, passControlCount = 0, needsLotsWitnessCount = 0;
  DETECTORS.forEach(function (d) { d.mutations.forEach(function (m) { if (m.control) passControlCount++; else { witnessCount++; if (m.needsLots) needsLotsWitnessCount++; } }); });
  return { detectorCount: DETECTORS.length, witnessCount: witnessCount, passControlCount: passControlCount, needsLotsWitnessCount: needsLotsWitnessCount };
}
detector("DOC-28", "MANIFEST.measurements.documentaryMeasurements (detecteurs, temoins, controles, temoins needsLots) egale les grandeurs derivees de la structure des detecteurs",
  function (docs) {
    let man;
    try { man = JSON.parse(docs["MANIFEST.json"] || ""); } catch (e) { return ["MANIFEST.json illisible"]; }
    const dm = (man.measurements || {}).documentaryMeasurements;
    if (!dm) return ["MANIFEST.measurements.documentaryMeasurements absent"];
    const st = staticMeasurements();
    const hits = [];
    ["detectorCount", "witnessCount", "passControlCount", "needsLotsWitnessCount"].forEach(function (k) {
      if (dm[k] !== st[k]) hits.push("documentaryMeasurements." + k + " = " + dm[k] + " ; derive de la structure : " + st[k]);
    });
    if (typeof dm.cleanPassCount !== "number" || typeof dm.singleFaultFailCount !== "number" || typeof dm.revertPassCount !== "number") hits.push("documentaryMeasurements : grandeurs dynamiques absentes");
    if (dm.detectorCount !== undefined && (dm.cleanPassCount > dm.detectorCount || dm.singleFaultFailCount > dm.detectorCount || dm.revertPassCount > dm.detectorCount)) hits.push("documentaryMeasurements : grandeur dynamique > nombre de detecteurs");
    if (!/deriv/i.test((man.measurements || {}).documentaryChecks || "")) hits.push("measurements.documentaryChecks ne se declare pas derive du run");
    /** les nombres de detecteurs / temoins / controles annonces en prose (README, NON-REGRESSION, migration courante) */
    const NUMW = { "vingt-six": 26, "vingt-sept": 27, "vingt-huit": 28, "vingt-neuf": 29, "trente": 30 };
    const prose = [["README.md", /\| Contrôles documentaires \| \*\*(\d+) \/ (\d+)\*\*/], ["NON-REGRESSION.md", /\*\*(Vingt-[a-z]+|Trente)\*\*\s+détecteurs/i], ["MIGRATION-v0.18-v0.19.md", /(Vingt-[a-z]+|Trente)\s+détecteurs/i]];
    prose.forEach(function (pr) {
      const m = (docs[pr[0]] || "").match(pr[1]);
      if (!m) { hits.push(pr[0] + " : nombre de detecteurs non trouve"); return; }
      const n = /^\d+$/.test(m[1]) ? parseInt(m[1], 10) : NUMW[m[1].toLowerCase()];
      if (n !== st.detectorCount) hits.push(pr[0] + " annonce " + m[1] + " detecteurs, structure : " + st.detectorCount);
    });
    return hits;
  },
  [
    mut("MANIFEST.json : documentaryMeasurements.witnessCount - 1", "MANIFEST.json", function (t) {
      const man = JSON.parse(t); man.measurements.documentaryMeasurements.witnessCount -= 1; return JSON.stringify(man, null, 2);
    }),
    mut("MANIFEST.json : documentaryMeasurements.passControlCount + 1", "MANIFEST.json", function (t) {
      const man = JSON.parse(t); man.measurements.documentaryMeasurements.passControlCount += 1; return JSON.stringify(man, null, 2);
    }),
    mut("MANIFEST.json : documentaryMeasurements.detectorCount faux (26)", "MANIFEST.json", function (t) {
      const man = JSON.parse(t); man.measurements.documentaryMeasurements.detectorCount = 26; return JSON.stringify(man, null, 2);
    }),
    mut("README.md : « Contrôles documentaires | 26 / 26 » (nombre de v0.18)", "README.md", function (t) {
      return mustReplace(t, /\| Contrôles documentaires \| \*\*\d+ \/ \d+\*\*/, "| Contrôles documentaires | **26 / 26**", "ligne Controles documentaires");
    }),
  ]);

/* ----- B3 (v0.19) / DOC-29 — toute cle du MANIFEST citee dans un document existe ----- */
function manifestDeepKeys(node, acc) {
  if (Array.isArray(node)) node.forEach(function (x) { manifestDeepKeys(x, acc); });
  else if (node && typeof node === "object") Object.keys(node).forEach(function (k) { acc[k] = 1; manifestDeepKeys(node[k], acc); });
  return acc;
}
detector("DOC-29", "tout identifiant cite comme cle du MANIFEST (phrase mentionnant MANIFEST ou `where`) existe reellement dans MANIFEST.json",
  function (docs) {
    let man;
    try { man = JSON.parse(docs["MANIFEST.json"] || ""); } catch (e) { return ["MANIFEST.json illisible"]; }
    const keys = manifestDeepKeys(man, {});
    const hits = [];
    Object.keys(docs).forEach(function (f) {
      if (isJson(f) || f === CHARTE) return;
      blocksOf(f, docs[f]).forEach(function (b) {
        if (b.kind === "code") return;
        sentencesOf(b).forEach(function (sent) {
          /** phrase qui parle de CLES du MANIFEST : « clé », « `where` », ou un chemin « MANIFEST.xxx » */
          if (!/\bcl[eé]s?\b|`where`|MANIFEST\.[a-zA-Z]/.test(sent)) return;
          /** une cle citee comme INEXISTANTE est une description, pas une affirmation */
          const deniesExistence = /inexistant|n'existe pas|absente? d|absent d/i.test(sent);
          (sent.match(/`((?:MANIFEST\.)?[a-z][A-Za-z0-9]+(?:\.[a-zA-Z][A-Za-z0-9]*)*)`/g) || []).forEach(function (tok) {
            const id = tok.replace(/`/g, "");
            /** segment generique « frameV0NN », « v01xBuildWindowOpened » : un motif, pas une cle */
            const parts = id.replace(/^MANIFEST\./, "").split(".").filter(function (p) { return !/NN|xx|01x/i.test(p); });
            const missing = parts.filter(function (p) { return !keys[p]; });
            if (missing.length && deniesExistence && new RegExp("`" + id.replace(/\./g, "\\.") + "`[^.]{0,60}(inexistant|n'existe pas|absent)|(inexistant|n'existe pas|absent)[^.]{0,60}`" + id.replace(/\./g, "\\.") + "`", "i").test(sent)) return;
            /** identifiants de CODE ou de fichiers, jamais des cles : ignores */
            if (missing.length && !/^(core|test|tools|governance)\//.test(id) && !/\.(md|js|json)$/.test(id)) {
              hits.push(f + ":" + b.line + " :: cle « " + id + " » citee comme cle du MANIFEST, absente de MANIFEST.json");
            }
          });
        });
      });
  ...
  [
    mut("MIGRATION-v0.18-v0.19.md §3 : « textFixLocation » redevient « fixLocation », cle inexistante (residu B3 de v0.18)", "MIGRATION-v0.18-v0.19.md", function (t) {
      return mustReplace(t, /`README\.md` cite désormais `textFixLocation` et `detectorFixLocation`, les clés\nqui existent\./, "`README.md` cite désormais `fixLocation` et `detectorFixLocation`, les clés\nqui existent.", "phrase cles qui existent");
    }),
    mut("README.md : « MANIFEST.documentHistoryMeasuredRange » devient « MANIFEST.documentHistoryRange », chemin inexistant", "README.md", function (t) {
      return mustReplace(t, /`MANIFEST\.documentHistoryMeasuredRange`/, "`MANIFEST.documentHistoryRange`", "chemin documentHistoryMeasuredRange dans README");
    }),
  ]);
...
detector("DOC-15", "tous les chemins references dans MANIFEST.json existent",
  function (docs) {
    ...
    manifestPaths(man).forEach(function (x) {
      if (/\.(was|replaces|where|historicalFaultLocation|textFixLocation|detectorFixLocation)$|\.preventiveReframings\[/.test(x.trail)) return;  // affirmation ancienne citee, fichier remplace, ou lieu d'une faute / d'un correctif historique
...
detector("DOC-19", "aucun ancien token NOT_FIXED_IN_V0NN / resolvedByV0NN n'est presente comme courant : continuite refusee, cadre couple a la meme version, « a l'epoque » n'exempte pas",
      ...
      blocksOf(f, docs[f]).forEach(function (b) {
        ...
        sentencesOf(b).forEach(function (sent) {
          ...
          while ((m = re.exec(sent)) !== null) {
            ...
            /** v0.19 — frontieres UNICODE : `\b` ne precede jamais « à » ; (?<![\p{L}]) et (?![\p{L}]) avec le drapeau u.
             *  Les HUIT mots, avant comme apres le token. */
            const contAfter = /^[\s`»)]*[,;:—-]?\s*(?:(?:est|et|c'est|le statut est)\s+)?(depuis|encore|toujours|[aà] ce jour|aujourd'hui|reste|demeure|d[eé]sormais)(?![\p{L}])/iu.test(after);
            const contBefore = /(?<![\p{L}])(depuis|encore|toujours|[aà] ce jour|aujourd'hui|reste|demeure|d[eé]sormais)\s*[,:;—-]?\s*(?:le statut\s+|statut\s+)?`?$/iu.test(before);
  ...
  [
    ...
    mut("OPEN-FINDINGS.md : un NOT_FIXED_IN_V019 redevient NOT_FIXED_IN_V015", "OPEN-FINDINGS.md", function (t) {
      return mustReplace(t, /NOT_FIXED_IN_V019/, "NOT_FIXED_IN_V015", "NOT_FIXED_IN_V019");
    }),
    ...
    /** v0.19 — 8 mots x {A : cadre couple, B : citation explicite} x {avant, apres} = 32 temoins :
     *  le cadre ou la citation est legitime, la continuite ADJACENTE doit quand meme faire FAIL. */
    const WORDS = ["depuis", "encore", "toujours", "à ce jour", "aujourd'hui", "reste", "demeure", "désormais"];
    const out = [];
    WORDS.forEach(function (w) {
      out.push(mut("avant le token, cadre couplé : « À l'époque de v0.15, " + w + " `NOT_FIXED_IN_V015`. »", "MIGRATION-v0.14-v0.15.md", function (t) {
        return mustReplace(t, /(## 6\. Ce que v0\.15 ne fait pas\n\n)/, "$1À l'époque de v0.15, " + w + " `NOT_FIXED_IN_V015`.\n\n", "section 6");
      }));
      out.push(mut("avant le token, citation explicite : « Comme v0.16 l'annonçait, " + w + " `NOT_FIXED_IN_V015`. »", "MIGRATION-v0.14-v0.15.md", function (t) {
        return mustReplace(t, /(## 6\. Ce que v0\.15 ne fait pas\n\n)/, "$1Comme v0.16 l'annonçait, " + w + " `NOT_FIXED_IN_V015`.\n\n", "section 6");
      }));
      out.push(mut("après le token, cadre couplé : « À l'époque de v0.15 : `NOT_FIXED_IN_V015` " + w + ". »", "MIGRATION-v0.14-v0.15.md", function (t) {
        return mustReplace(t, /(## 6\. Ce que v0\.15 ne fait pas\n\n)/, "$1À l'époque de v0.15 : `NOT_FIXED_IN_V015` " + w + ".\n\n", "section 6");
      }));
      out.push(mut("après le token, citation explicite : « Comme v0.16 l'annonçait, `NOT_FIXED_IN_V015` " + w + ". »", "MIGRATION-v0.14-v0.15.md", function (t) {
        return mustReplace(t, /(## 6\. Ce que v0\.15 ne fait pas\n\n)/, "$1Comme v0.16 l'annonçait, `NOT_FIXED_IN_V015` " + w + ".\n\n", "section 6");
      }));
    });
    return out;
  })()).concat([
    ...
    ctl("CONTROLE : « Statut courant : `NOT_FIXED_IN_V019`. » ajoute -> ne doit PAS mordre", "README.md", function (t) {
      return mustReplace(t, /(Détail intégral, avec la couche qui compense chacune : `OPEN-FINDINGS\.md`\.\n)/, "$1\nStatut courant : `NOT_FIXED_IN_V019`.\n", "phrase detail integral");
    }),
    ctl("CONTROLE : « Dans le cadre v0.14 : `NOT_FIXED_IN_V014`. » ajoute -> ne doit PAS mordre", "MIGRATION-v0.13-v0.14.md", function (t) {
      return mustReplace(t, /(## 7\. Ce que v0\.14 ne fait pas\n\n)/, "$1Dans le cadre v0.14 : `NOT_FIXED_IN_V014`.\n\n", "section 7");
    }),
  ]));

const EXTRA_CHECK_IDS = ["COH-01", "COH-02", "COH-03", "COH-04", "COH-05", "COH-06", "RT-01", "RT-02", "RT-03", "RT-04", "RT-05", "RT-06", "RT-07", "RT-08", "RT-09", "RT-10", "RT-11",
  "SEAL-01", "SEAL-02", "SEAL-03", "SEAL-04", "SEAL-05", "SEAL-06", "SEAL-07", "SEAL-08", "SEAL-09", "SEAL-10", "SEAL-11", "MAN-01", "MAN-02", "MAN-03", "SCAN-01", "SCAN-02", "SCAN-03", "SCAN-04", "MEAS-01"];
...
/* ============================ execution ============================ */
(function () {
  console.log("MONO-10 v0.19 — controles de verite documentaire\n");
  ...
  check("SCAN-04. le balayage atteint un fichier documentaire racine a nom pointe (AUDIT-VERDICTS.v0.12-v0.19.md)",
    md.indexOf("AUDIT-VERDICTS.v0.12-v0.19.md") !== -1);
  ...
  const MEAS = { cleanPassCount: 0, singleFaultFailCount: 0, revertPassCount: 0, controlsPassCount: 0, needsLotsSkippedWitnessCount: 0 };
  DETECTORS.forEach(function (d) {
    total++;
    ...
    if (ok) caught++;
    if (passClean) MEAS.cleanPassCount++;
    if (allBite && nFault > 0) MEAS.singleFaultFailCount++;
    if (allRevert) MEAS.revertPassCount++;
    MEAS.controlsPassCount += controlsOk; MEAS.needsLotsSkippedWitnessCount += witnessSkips;
    check(d.id + ". " + d.label
  ...
  /** mesures documentaires : statiques (structure) + dynamiques (ce run) — lues par le constructeur, jamais saisies */
  const MEASUREMENTS = Object.assign(staticMeasurements(), MEAS, { lotsReachable: LOTS_REACHABLE });
  console.log("  DOCUMENTARY_MEASUREMENTS = " + JSON.stringify(MEASUREMENTS));
  ...
  const man = manifest || {};
  const of = docs["OPEN-FINDINGS.md"] || "";
  check("COH-01. OPEN-FINDINGS.md : chaque ligne R1..R5 du tableau de statut porte OPEN / NON_BLOCKING / NOT_FIXED_IN_V019 (verification individuelle, DOC-23)",
    ["R1", "R2", "R3", "R4", "R5"].every(function (r) {
      const row = (of.match(new RegExp("^\\|\\s*\\*\\*" + r + "\\*\\*[^\\n]*$", "m")) || [""])[0];
      return /`OPEN`/.test(row) && /`NON_BLOCKING`/.test(row) && /`NOT_FIXED_IN_V019`/.test(row);
    }));
  check("COH-02. aucune reserve n'est presentee comme resolue (MANIFEST.knownOpenFindings)",
    (man.knownOpenFindings || []).length === 5
    && (man.knownOpenFindings || []).every(function (r) { return r.resolvedByV019 === false && r.status === "OPEN" && r.blocking === false; }),
    JSON.stringify((man.knownOpenFindings || []).map(function (r) { return r.id + ":" + r.status + ":" + r.resolvedByV019; })));
  ...
  check("COH-04. MIGRATION v0.18 -> v0.19 existe et affirme l'absence de changement runtime",
    !!docs["MIGRATION-v0.18-v0.19.md"] && /Aucune API ne change/.test(docs["MIGRATION-v0.18-v0.19.md"] || ""));
  check("COH-05. le MANIFEST declare la nature documentaire et les huit identites runtime",
    /DOCUMENTAIRE/i.test(man.natureOfLot || "")
    && ["MONO-10-v0.11", "MONO-10-v0.12", "MONO-10-v0.13", "MONO-10-v0.14", "MONO-10-v0.15", "MONO-10-v0.16", "MONO-10-v0.17", "MONO-10-v0.18"].every(function (v) { return (man.runtimeByteIdenticalTo || []).indexOf(v) !== -1; }),
  ...
  /** MEAS-01 — les grandeurs dynamiques du MANIFEST egalent ce run (quand les lots sont joignables, comme a la construction) */
  (function () {
    const dm = (man.measurements || {}).documentaryMeasurements;
    if (!dm) { check("MEAS-01. MANIFEST.measurements.documentaryMeasurements present", false, "absent"); return; }
    if (!LOTS_REACHABLE) { skip("MEAS-01. grandeurs dynamiques du MANIFEST == ce run", "lots non joignables : les temoins needsLots sont SKIP, les grandeurs different par construction"); return; }
    const bad = ["cleanPassCount", "singleFaultFailCount", "revertPassCount", "controlsPassCount"].filter(function (k) { return dm[k] !== MEASUREMENTS[k]; });
    check("MEAS-01. grandeurs dynamiques du MANIFEST (PASS_CLEAN, FAIL_SINGLE_TARGET_FAULT, PASS_AFTER_REVERT, controles) == ce run", bad.length === 0, JSON.stringify(bad) + " manifest=" + JSON.stringify(dm));
  })();
  ...
  const staleKeys = Object.keys(man.measurements || {}).filter(function (k) { return /V01[0-8]\b/.test(k) && !/runtimeByteIdenticalToV01[0-8]/.test(k); });
  check("MAN-03. MANIFEST : reserves R1-R5 OPEN / NON_BLOCKING / NOT_FIXED_IN_V019, closesFindings vide, aucune mesure indexee sur une version anterieure recopiee comme courante",
    (man.closesFindings || []).length === 0 && (man.knownOpenFindings || []).length === 5
    && (man.knownOpenFindings || []).every(function (r) { return r.status === "OPEN" && r.blocking === false && r.resolvedByV019 === false && r.fixedIn === "NOT_FIXED_IN_V019"; })
    && staleKeys.length === 0 && man.correctsFalseStatementsOf === "MONO-10-v0.18",
  ...
  [["v0.11", "RT-01"], ["v0.12", "RT-02"], ["v0.13", "RT-03"], ["v0.14", "RT-06"], ["v0.15", "RT-07"], ["v0.16", "RT-09"], ["v0.17", "RT-10"], ["v0.18", "RT-11"]].forEach(function (pair) {
  ...
  check("RT-05. les outils et tests documentaires ajoutes n'ecrivent rien (aucun appel fs d'ecriture)",
    ["tools/seal-inventory.js", "test/test-mono10-v0.19-documentary.js"].every(function (t) {
  ...
  console.log("\n  -- inventaire des sceaux : neuf cadres --");
  if (!fs.existsSync(path.join(BUNDLE, "MONO-10"))) {
    skip("SEAL-01..11. inventaire", "paquet non joignable depuis cette extraction");
  } else {
    const SI = require(path.join(LOT, "tools", "seal-inventory.js"));
    const frames = [
      ["v0.11", "SEAL-01", ["MONO-10/v0.11", "MONO-10/v0.12", "MONO-10/v0.13", "MONO-10/v0.14", "MONO-10/v0.15", "MONO-10/v0.16", "MONO-10/v0.17", "MONO-10/v0.18", "MONO-10/v0.19"], [24, 1577, 9, 0]],
      ["v0.12", "SEAL-02", ["MONO-10/v0.12", "MONO-10/v0.13", "MONO-10/v0.14", "MONO-10/v0.15", "MONO-10/v0.16", "MONO-10/v0.17", "MONO-10/v0.18", "MONO-10/v0.19"], [25, 1644, 9, 0]],
      ["v0.13", "SEAL-03", ["MONO-10/v0.13", "MONO-10/v0.14", "MONO-10/v0.15", "MONO-10/v0.16", "MONO-10/v0.17", "MONO-10/v0.18", "MONO-10/v0.19"], [26, 1715, 9, 0]],
      ["v0.14", "SEAL-07", ["MONO-10/v0.14", "MONO-10/v0.15", "MONO-10/v0.16", "MONO-10/v0.17", "MONO-10/v0.18", "MONO-10/v0.19"], [27, 1787, 9, 0]],
      ["v0.15", "SEAL-08", ["MONO-10/v0.15", "MONO-10/v0.16", "MONO-10/v0.17", "MONO-10/v0.18", "MONO-10/v0.19"], [28, 1860, 9, 0]],
      ["v0.16", "SEAL-09", ["MONO-10/v0.16", "MONO-10/v0.17", "MONO-10/v0.18", "MONO-10/v0.19"], [29, 1934, 9, 0]],
      ["v0.17", "SEAL-10", ["MONO-10/v0.17", "MONO-10/v0.18", "MONO-10/v0.19"], [30, 2010, 9, 0]],
      ["v0.18", "SEAL-11", ["MONO-10/v0.18", "MONO-10/v0.19"], [31, 2087, 9, 0]],
    ];
    let f11 = null;
    ...
    const f14 = SI.inventory(BUNDLE, "MONO-10/v0.19");
    console.log("  INFO  cadre v0.19 mesure : " + f14.sealedHistoricalLots + " lots, " + f14.sealedReferences + " references, "
    ...
    check("SEAL-04. cadre v0.19 : memes 9 divergences, 0 lot non scelle, v0.18 devenu historique (32 lots)",
      f14.sealedDivergences === 9 && f14.unverifiableHistoricalLots.length === 0 && f14.sealedHistoricalLots === 32,
    ...
    const nr = docs["NON-REGRESSION.md"] || "";
    check("SEAL-06. les neuf cadres sont nommes dans NON-REGRESSION.md, et le MANIFEST porte le cadre v0.19 mesure",
      ["v0.11", "v0.12", "v0.13", "v0.14", "v0.15", "v0.16", "v0.17", "v0.18", "v0.19"].every(function (v) { return new RegExp("cadre v0\\." + v.slice(3)).test(nr); })
      && man.sealInventory && man.sealInventory.frameV019
      && man.sealInventory.frameV019.sealedHistoricalLots === f14.sealedHistoricalLots
      && man.sealInventory.frameV019.sealedReferences === f14.sealedReferences
      ...
      JSON.stringify(man.sealInventory && man.sealInventory.frameV019));
  }
  ...
  console.log("KNOWN_FALSE_STATEMENTS_REMAINING (mesure, DOC-01..DOC-29 hors DOC-17 qui la compare) = " + DETECTORS.reduce(function (n, d) {
