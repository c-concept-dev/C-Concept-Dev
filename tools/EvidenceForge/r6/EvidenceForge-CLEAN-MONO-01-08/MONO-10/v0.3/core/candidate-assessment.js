"use strict";
/**
 * MONO-10 v0.3 — core/candidate-assessment.js
 *
...
const { artifactRef } = require("./lineage.js");
...
function assessCandidates(input) {
  ...
  return {
    schema: "EvidenceForge.ProfessionalCandidateAssessment", schemaVersion: "MONO-10-v3",
    assessmentId: input.assessmentId || "candidate-assessment",
    missionId: discovery.missionId || null,
    missionLabels: missionLabels,
    assessesDiscoveryRef: artifactRef(discovery, "professional-discovery", discovery.schema),
    assessesVerificationRef: artifactRef(verification, "professional-verification", verification.schema),
...
function defaultIdentityExtractor(c, v) {
  const out = [];
  const pushE = (type, authority, family, id, binding, verif, contrib) => {
    if (!isNonEmptyStr(id)) return;
    out.push({ evidenceType: type, sourceAuthorityId: authority, sourceFamilyId: family,
      identifier: id, assertedValue: null, subjectBinding: binding,
      provenance: { origin: "DISCOVERY" }, verificationStatus: verif, confidenceContribution: contrib });
  };
  // L'autorite emettrice n'est PAS deduite du protocole de l'identifiant :
  // une URL ne demontre aucune independance. Elle reste nulle sauf declaration.
  pushE("provider-native-id", c.sourceAuthorityId || null, c.sourceFamilyId || null, c.candidateRef,
  ...
  (Array.isArray(c.identifiers) ? c.identifiers : []).forEach(function (x) {
    pushE(x.type || "external-id", x.sourceAuthorityId || null, x.sourceFamilyId || null, x.value,
  ...
  if (isNonEmptyStr(c.displayName)) {
    out.push({ evidenceType: "display-name", sourceAuthorityId: null, sourceFamilyId: null, identifier: null, assertedValue: c.displayName,
