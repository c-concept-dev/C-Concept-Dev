# MONO-10 v0.10 — non-régression

## 1. Ce qui est affirmé, et ce qui ne l'est pas

| Affirmation | Valeur |
|---|---|
| `MONO10_V010_CAUSED_REGRESSION` | **NO** |
| `AGGREGATE_BASELINE_IDENTICAL` | **22 / 22 lots** |
| `SEALED_REFERENCE_DIVERGENCES` | **0** (sur les 13 lots qui portent un sceau) |
| `FULL_HISTORY_BYTE_IDENTITY_CONFIRMED` | **NO** |
| `UNVERIFIABLE_HISTORICAL_LOTS` | **9** |

La dernière ligne est négative et elle n'est pas enfouie : **9 lots sur 22 ne
...
Résultat : **22 lots, 0 divergent.**

Mais une baseline que j'ai moi-même capturée n'est pas une preuve historique
suffisante. Le contrôle qui l'est :

**Fenêtre de construction de v0.10 : ouverte le 12/09/2026 à 19:15:35.**
**Aucun fichier situé hors de `MONO-10/v0.10/` n'a été écrit dans cette
fenêtre** — zéro, mesuré sur l'arborescence entière du paquet.

```
find . -path ./MONO-10/v0.10 -prune -o -type f -print \
  | xargs stat -f "%m %N" | awk '$1 >= 1789326935'     ->  0 ligne
```

Cette vérification est indépendante de mes empreintes : elle porte sur les dates
de modification du système de fichiers.

**Précision honnête sur la baseline.** La baseline de v0.10 a été recapturée
...
## 5. Ce que v0.10 a modifié dans son propre périmètre

v0.10 modifie **uniquement** `MONO-10/v0.10/`. Les changements de contrat qui
affectent un appelant sont énumérés dans `MIGRATION-v0.9-v0.10.md` §3. Les deux
modifications de comportement visibles dans le chemin nominal :

1. `assessCandidates` exige désormais `verifier` — sans lui, corpus vide ;
2. `PRODUCTION_LLM_CAPABILITY` exige une sonde réellement exécutée par
   `verifier.runLlmProbe`.

Aucun lot historique n'a été touché pour faire passer ces tests.

## 6. Vérification permanente

`NR-01` (447 références scellées) et `T33` (identité croisée MONO-01) sont
