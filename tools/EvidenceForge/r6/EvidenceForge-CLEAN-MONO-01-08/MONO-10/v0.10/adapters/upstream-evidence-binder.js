"use strict";
/**
 * MONO-10 v0.10 — adapters/upstream-evidence-binder.js   (§17 a §25, §32)
 *
 * FERMETURE B5 et B6.
 *
...
function createUpstreamEvidenceBinder(input) {
   ...
   */
  const expect = { operatorBoundaryId: manifest.operatorTrustBoundaryId || null,
    configBindingHash: manifest.configBindingHash || null, executionMode: manifest.executionMode };
  /**
   * §6 (v0.10) — le pont ne DETIENT plus l'autorite. Il ne peut que DEMANDER
   * une certification au verificateur de la frontiere qui a ouvert ce run.
   * Une `provenanceAuthority` passee en argument est signalee et ignoree.
   */
  const callerAuthoritySupplied = !!input.provenanceAuthority;
  const verifier = OTV.isOperatorTrustVerifier(input.verifier) ? input.verifier : null;
  ...
  function bind(descriptor) {
    ...
    let provenanceStatus = CC.PROVENANCE_STATUS.UNRESOLVED;
    let problems = ["aucun verificateur de frontiere operateur : la provenance n'est pas authentifiable"];
    if (verifier) {
      // La resolution ET l'emission de la capacite se font DANS la frontiere.
      const cert = verifier.certifyProvenance({ registry: registry, artifactId: artifactId });
      provenanceStatus = cert.certified ? CC.PROVENANCE_STATUS.AUTHENTICATED
        : ((cert.resolution && cert.resolution.status) || CC.PROVENANCE_STATUS.UNRESOLVED);
      problems = cert.problems || [];
    }
    return { artifactId: artifactId, ref: ref, provenanceStatus: provenanceStatus, problems: problems,
      callerAuthoritySupplied: callerAuthoritySupplied, callerAuthorityIgnored: true, reason: null };
  }

  return { bind: bind, artifactIdPrefix: prefix, provenanceAuthorityBound: !!verifier,
    callerAuthorityIgnored: callerAuthoritySupplied };
}
