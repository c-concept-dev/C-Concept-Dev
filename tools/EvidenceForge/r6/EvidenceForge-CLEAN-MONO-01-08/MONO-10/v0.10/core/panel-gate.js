"use strict";
/**
 * MONO-10 v0.10 — core/panel-gate.js   (§19, §20)
 *
...
const { HUMAN_EVIDENCE_RELATIONS } = require("./lineage.js");
...
/** §21/§24 — l'empreinte de l'acte PRECIS attendu pour un candidat donne. */
function panelDecisionHash(assessment, a, decision) {
  const rb = runBindingOf(assessment);
  return HAP.computeDecisionHash({ actionType: HAP.ACTION_TYPE.PANEL_DECISION,
    runId: (rb ? rb.runId : assessment.runId) || "", missionHash: (rb ? rb.missionHash : assessment.missionHash) || "",
    subjectId: a.candidateId, decision: decision,
    boundArtifactHash: assessmentHash(assessment), evidenceRefsHash: evidenceRefsHash(a.missionEvidenceRefs) });
}
...
function buildPanelValidationTemplate(assessment) {
  ...
  return {
    ...
    schema: "EvidenceForge.ProfessionalPanelValidation", schemaVersion: "MONO-10-v6",
...
function validatePanelValidation(validation, assessment, ctx) {
  ...
  (validation.decisions || []).forEach(function (d, i) {
    ...
    const a0 = byId.get(d.candidateId);
    // §21 — l'acte doit porter l'empreinte de CETTE decision, sur CE candidat.
    const expectedDecisionHash = a0 ? panelDecisionHash(assessment, a0, d.decision) : null;
    if (d.decisionHash !== expectedDecisionHash) {
      problems.push(tag + " : decisionHash absent ou ne portant pas sur cette decision et ce candidat — "
        + "l'appartenance d'un acteur au registre ne prouve pas qu'il a accompli CET acte."); return;
    }
    const auth = verifyHumanActAuthenticity(d, ctx, tag, { actionType: HAP.ACTION_TYPE.PANEL_DECISION,
      decisionHash: expectedDecisionHash, runId: validation.runId, missionHash: assessment.missionHash });
    ...
    if (AAR.isAuthenticatedRegistry(registry)) {
      ...
      if (refs.length) {
        const res = resolveLineage(refs, registry, {
          ...
          // §23 — classe de preuve admissible : un artefact de mission ne devient
          // pas une preuve documentaire par simple etiquette de relation.
          acceptedRelations: HUMAN_EVIDENCE_RELATIONS,
        ...
        const wrongRelation = res.resolvedRefs.filter((r) => HUMAN_EVIDENCE_RELATIONS.indexOf(r.relation) === -1);
        ...
        // §30 — SINK CRITIQUE « panel-gate:evidence-presented-to-human ».
        // Une preuve n'est presentable a un humain que si l'autorite de
        // provenance de l'exploitant a AUTHENTIFIE ses racines. Resoudre contre
        // le registre prouve la liaison au run, pas l'authenticite de l'origine.
        const required = AC.requirementOf("panel-gate:evidence-presented-to-human");
        const expectedBoundary = ctx.manifest ? { operatorBoundaryId: ctx.manifest.operatorTrustBoundaryId, configBindingHash: ctx.manifest.configBindingHash, executionMode: ctx.manifest.executionMode } : null;
        const unauthenticated = [];
        const crossBoundary = [];
        refs.forEach(function (r) {
          const e = registry.get(r.artifactId);
          if (!AC.hasCapability(e, required)) { unauthenticated.push(r.artifactId); return; }
          // §12 — la capacite doit avoir ete emise par la frontiere de CE run.
          try { AC.assertCapability(e, required, "panel-gate:evidence-presented-to-human", expectedBoundary); }
          catch (err) { crossBoundary.push(r.artifactId + " (" + String(err.message).split(":")[0] + ")"); }
        });
        if (crossBoundary.length) {
          problems.push(tag + " : preuve(s) dont la capacite vient d'une autre frontiere operateur ("
            + crossBoundary.slice(0, 3).join(", ") + ")."); return;
        }
        if (unauthenticated.length) {
          problems.push(tag + " : preuve(s) sans capacite \"" + required + "\" (" + unauthenticated.slice(0, 3).join(", ")
            + ") — les racines de provenance n'ont pas ete authentifiees par l'autorite de l'exploitant. "
            + "Une racine ecrite par l'appelant n'est pas une racine."); return;
        }
      }
    }
...
module.exports = { buildPanelValidationTemplate, validatePanelValidation, decisionsById, panelDecisionHash,
