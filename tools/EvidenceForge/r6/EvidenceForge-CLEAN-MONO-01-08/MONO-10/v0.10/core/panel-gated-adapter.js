"use strict";
/**
 * MONO-10 v0.10 — core/panel-gated-adapter.js  (§3, §4, §5, §30)
 *
...
const { deriveEffectiveEligibility, isEligible, isEligibleDecision, sanitizeEligibilityPolicy, ELIGIBILITY } = require("./effective-eligibility.js");
const AC = require("./artifact-capabilities.js");

const GATE_ID = "MONO-10-v9-panel-gate";

function createPanelGatedAdapter(baseAdapter, opts) {
  opts = opts || {};
  /**
   * FERMETURE B1 (§3, §5). En v0.6, `opts.eligibilityPolicy` traversait
   * l'adaptateur intacte jusqu'au recalcul du sink, ou la cle
   * `legacyVerificationGrantsEligibility` admettait au corpus un candidat que
   * le panel n'avait jamais vu. La politique est desormais assainie ICI par
   * liste blanche, et le refus est consigne dans l'artefact de sortie.
   */
  const eligibilityPolicy = sanitizeEligibilityPolicy(opts.eligibilityPolicy);
  ...
  function gate() {
    ...
    // §30 — SINK CRITIQUE « panel-gated-adapter:corpus-sink ». L'artefact de
    // decision humaine doit porter la capacite HUMAN_AUTHENTICATED, emise par
    // le mecanisme d'acte de l'exploitant apres verification des preuves
    // d'acte. Sans elle : fail closed, aucun corpus.
    const reg = (opts.runContext || {}).artifactRegistry;
    const panelArtifactId = isNonEmptyStr(opts.panelValidationArtifactId) ? opts.panelValidationArtifactId : "panel-validation";
    if (reg && typeof reg.get === "function") {
      const required = AC.requirementOf("panel-gated-adapter:corpus-sink");
      const entry = reg.get(panelArtifactId);
      if (!entry) {
        throw fail("PANEL_VALIDATION_UNREGISTERED",
          "la ProfessionalPanelValidation \"" + panelArtifactId + "\" n'est pas enregistree dans le registre authentifie du run.");
      }
      const man = (opts.runContext || {}).manifest;
      AC.assertCapability(entry, required, "panel-gated-adapter:corpus-sink",
        man ? { operatorBoundaryId: man.operatorTrustBoundaryId, configBindingHash: man.configBindingHash, executionMode: man.executionMode } : null);
    }
  ...
  async function verifyProfessionals(inputs, ctx) {
    ...
    return Object.assign({}, base, {
      ...
      panelGate: { gateId: GATE_ID, legacyStatusPreserved: true,
        eligibilityPolicyRefused: eligibilityPolicy.refused.slice(),
        eligibilityPolicyMotives: Object.assign({}, eligibilityPolicy.refusalMotives),
