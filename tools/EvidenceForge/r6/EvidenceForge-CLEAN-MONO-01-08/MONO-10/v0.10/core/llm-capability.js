"use strict";
/**
 * MONO-10 v0.10 — core/llm-capability.js  (§19)
 *
...
async function runActiveProbe(intent, ctx) {
  ...
  const base = {
    ...
    /** §11 — reference de la DECISION de sonde inscrite au registre de l'emetteur. */
    probeDerivationRef: null,
  ...
  /**
   * §11 (v0.10) — la sonde REELLEMENT executee est inscrite au registre de son
   * emetteur. C'est cette inscription, et elle seule, qui permettra plus tard
   * d'emettre la capacite `PRODUCTION_LLM_CAPABILITY`. Une chaine de caracteres
   * inventee par l'appelant ne s'y trouve pas.
   */
  ...
  if (typeof llmBoundary.recordProbe === "function") {
    ...
    base.probeDerivationRef = rec.derivationRef;
  } else {
...
function assertCapabilityUsable(capability, config, ctx) {
  ...
  /**
   * §12 (v0.10) — FAIL CLOSED SUR LES NEUF CONDITIONS.
   *
   * v0.9 decidait d'appliquer ou non le bloc de production par
   * `RM.isProductionContext(ctx)`, qui exige un verificateur marque dans le
   * contexte. Retirer `ctx.verifier` faisait donc SAUTER les neuf controles de
   * production d'un coup : l'appelant desactivait le controle en retirant ce
   * qui sert a le faire.
   *
   * v0.10 : ce que la capacite PRETEND est lu sur l'artefact et sur le
   * registre. Si elle pretend une capacite de production, le bloc s'applique ;
   * et si le contexte ne permet pas de le mener, c'est un REFUS, pas une
   * dispense.
   */
  const genuineProduction = RM.isProductionContext(ctx);
  const claimsProduction = capability.probeExecutionMode === RM.MODE.PRODUCTION
  ...
  if (production) {
    ...
    if (!AAR.isAuthenticatedRegistry(reg)) {
      ...
      const artifactId = isNonEmptyStr(ctx.llmCapabilityArtifactId) ? ctx.llmCapabilityArtifactId : "llm-capability";
      let entry = null;
  ...
  if (production) {
    if (!ctx.manifest) problems.push("aucun manifeste de run : une capacite non rattachee a un run ne vaut pas en production");
       ...
       */
      if (!isNonEmptyStr(capability.probeDerivationRef)) {
        problems.push("aucune reference de decision de sonde : une capacite de PRODUCTION non inscrite au registre "
          + "de son emetteur n'existe pas — fail closed.");
      } else if (!OTV.isOperatorTrustVerifier(ctx.verifier)) {
        problems.push("aucun verificateur issu d'une frontiere operateur pour verifier la decision de sonde "
          + capability.probeDerivationRef + " — fail closed.");
      } else {
        const vd = ctx.verifier.verifyIssuerDecision("PRODUCTION_LLM_CAPABILITY", capability.probeDerivationRef,
        ...
        if (!vd || vd.valid !== true) {
          problems.push("decision de sonde " + capability.probeDerivationRef + " non verifiable aupres de son "
            + "emetteur : " + ((vd && vd.problems) || ["registre muet"]).slice(0, 2).join(" ; "));
        }
      }
    }
  }
