"use strict";
/**
 * MONO-10 v0.11 — core/operator-trust-boundary.js   (§2, §3, §4, §6, §7, §8)
 *
 * ================================================================
...
/**
 * §6 (v0.10) — COFFRE PRIVE DES EMETTEURS.
 *
 * Une carte faible, non exportee, associant une frontiere scellee a ses
 * emetteurs de capacites critiques. Rien d'autre dans le lot ne peut la lire.
 * `verifierFor` est la seule fonction qui l'ouvre, et elle ne rend pas les
 * emetteurs : elle les passe au module de verification.
 */
const ISSUERS = new WeakMap();

/**
 * MONO-10 v0.11 — §3, §5, §6 : RACINE UNIQUE D'AUTORITE.
 *
...
function buildBoundary(cfg, namespace, provisionedFrom, sourceDescriptor) {
  ...
  const acceptanceBoundary = OAB.createFromBoundary(issuance, {
  ...
  const sealed = sealBoundary({
    schema: "EvidenceForge.OperatorTrustBoundary", schemaVersion: "MONO-10-v11",
    ...
    /** §3/§4 (v0.9) — la liaison de configuration est PUBLIEE pour etre COMPAREE. */
    configBindingHash: configBindingHash,
    ...
    /**
     * §6 (v0.10) — `humanAuthMechanism`, `llmCapabilityBoundary`,
     * `provenanceAuthority` et `historicalInputAuthority` NE SONT PLUS des
     * champs de cet objet. Ces emetteurs sont detenus dans `ISSUERS`, une carte
     * faible privee de ce module, et ne sortent que sous forme d'OPERATIONS
     * exposees par le verificateur (`verifierFor`). Un appelant qui detient la
     * frontiere peut demander une certification ; il ne recoit pas l'emetteur.
     */
    acceptanceBoundary: acceptanceBoundary,
    /** §6 — presence declarable sans remise de l'emetteur. */
    issuerInventory: Object.freeze({ humanAuth: !!humanAuth, llmCapability: !!llmBoundary,
      provenance: !!provenanceAuthority, historicalInput: !!historicalInputAuthority }),
  ...
  ISSUERS.set(sealed, Object.freeze({ humanAuth: humanAuth || null, llm: llmBoundary || null,
    provenance: provenanceAuthority || null, historical: historicalInputAuthority || null }));
  return sealed;
}

/**
 * §6 (v0.10) — SEULE fabrique de verificateur.
 *
 * Les emetteurs sont remis directement du coffre prive de ce module au module
 * de verification. Ils ne traversent jamais le code appelant. Une frontiere
 * fabriquee par un appelant n'a pas d'entree dans `ISSUERS` : elle produit un
 * verificateur sans emetteur, qui refuse toute certification — jamais un
 * verificateur permissif.
 */
function verifierFor(boundary) {
  if (!boundary || !isOperatorTrustBoundary(boundary)) {
    throw fail("TRUST_BOUNDARY_FORGED", "verifierFor : objet non issu de ce module — un appelant ne fabrique pas sa frontiere.");
  }
  const OTV = require("./operator-trust-verifier.js");
  return OTV.__build(boundary, ISSUERS.get(boundary) || {});
}
...
function provisionProductionTrustBoundary(opts) {
  opts = opts || {};
  /**
   * §24 (v0.9) — le NOM de la variable de confiance n'est pas un parametre
   * d'appel. Laisser l'appelant le choisir lui permettrait de designer une
   * autre racine de confiance. Un `opts.envVar` fourni est REFUSE, pas ignore
   * en silence.
   */
  if (isNonEmptyStr(opts.envVar) && opts.envVar !== ENV_VAR) {
    throw fail("TRUST_ENV_VAR_REFUSED",
      "le nom de la variable de configuration de confiance n'est pas choisi par l'appelant : "
      + ENV_VAR + " est fixe. Fournir \"" + opts.envVar + "\" designerait une autre racine de confiance.");
  }
...
module.exports = { provisionProductionTrustBoundary, provisionTestTrustBoundary, verifierFor,
