...
| Article | Exigence | Où elle s'exerce dans v0.11 |
|---|---|---|
...
| §24 | aucune dette bloquante silencieuse | `README.md` — cinq hypothèses déclarées ; le lot ne prétend pas prouver qu'un LLM réel a répondu |
...
| §15 | aucune capacité fabriquée : un fournisseur jamais sondé n'est jamais certifié | `LLM_SUBJECT_MISMATCH`, `LLM_SUBJECT_OUT_OF_ALLOWLIST`, `LLM_PROBE_ALREADY_CONSUMED` |
...
| §10 | *unknown remains unknown* jusque dans la mesure de non-régression | `NON-REGRESSION.md` — `UNVERIFIABLE_HISTORICAL_LOTS = 9` maintenu, non dissimulé |
