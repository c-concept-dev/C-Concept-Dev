# MONO-10 v0.18 — matrice de remédiation (v0.12 à v0.18 : lots documentaires)
...
## 11. v0.15 à v0.18 — ce que les audits finaux de v0.14 à v0.17 ont établi

Même constante : aucun bloqueur runtime, runtime byte-identique. Les quatre
...
verbatim**, pas contre des formes voisines (`DOC-19`).

| Constat | Établi par | Statut en v0.18 |
|---|---|---|
...
| B1 (v0.17) : `DOC-19` **non causal** — la faute B2 de v0.16 réinjectée verbatim donnait 0 hit : le marqueur générique « à l'époque » était évalué avant le couplage de version et la détection de « depuis » ; quatre documents affirmaient pourtant « même NN, depuis refusé » | audit final de v0.17 | **corrigé en v0.18** : ordre impératif continuité adjacente → couplage de version → citation, « à l'époque » n'exempte plus ; témoin obligatoire verbatim, sept variantes, trois contrôles ; claims réécrites (`MIGRATION-v0.17-v0.18.md` §1) |
| lacunes non bloquantes : `DOC-25` laissait passer le retrait de « plus `executionMode` » dans R1 et une dimension fantôme ; gloss « conçu en v0.7 » pour `LINEAGE.md` (existe depuis v0.1) ; `where` de B2 mêlant faute exacte et recadrages préventifs | audit final de v0.17 | **fermées en v0.18** (`MIGRATION-v0.17-v0.18.md` §2–§4) |
