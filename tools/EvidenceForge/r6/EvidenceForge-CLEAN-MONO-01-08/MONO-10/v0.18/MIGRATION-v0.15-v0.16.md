...
| statut exact de chaque réserve (`DOC-23`, `COH-01`) | chaque ligne R1..R5 du tableau de statut est vérifiée individuellement : `OPEN`, `NON_BLOCKING`, le token du lot courant (à l'époque de v0.16 : `NOT_FIXED_IN_V016`), et sa section `## Rn` existe | R5 passée à `CLOSED` / `FIXED_IN_V016`, les quatre autres inchangées → `FAIL` |
...
- il ne ferme **aucune** des cinq réserves R1 à R5 (à l'époque de v0.16 : `NOT_FIXED_IN_V016` ; le statut courant est dans `OPEN-FINDINGS.md`) ;
