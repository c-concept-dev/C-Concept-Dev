...
├── (émetteurs privés, jamais remis)  → verifier.verifyHumanAct / certifyHumanAuthenticated
...
## 4bis. Racine unique d'autorité (v0.8)

v0.7 exposait quatre constructeurs publics d'autorité qui n'exigeaient qu'un
chemin de fichier. Un appelant obtenait donc, sans toucher l'exploitant, une
autorité authentiquement marquée et les capacités qui en découlent. v0.8 impose :

> une capacité de sécurité n'est fiable que si son émetteur descend de
> l'`OperatorTrustBoundary` déjà provisionnée.

`isProvisioned*(a, expected)` ne répond plus « créé par ce module » mais « émis
par cette frontière », et chaque capacité porte l'identité de son émetteur.

## 4ter. Identité composite de frontière (v0.9)

`operatorTrustBoundaryId` est **déclaré dans la configuration** : deux
configurations distinctes peuvent porter le même. Une identité de frontière est
donc composite — identifiant + `configBindingHash` + espace d'exécution — et
`configBindingHash` est **comparé** par chaque consommateur, pas seulement
porté. Le manifeste de run l'engage, et `assertManifestAuthentic` le vérifie.

Corollaire appliqué partout : *une preuve portée dans un artefact n'a d'effet
que si son consommateur la vérifie*.
