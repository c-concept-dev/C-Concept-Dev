"use strict";
/**
 * MONO-10 v0.7 — core/key-lifecycle.js   (§9, §10, §11)
 *
...
/**
 * §36 — les statuts ne sont plus ecrits ici : ils sont IMPORTES de la source
 * canonique `contracts/canonical-contracts.json`, contre laquelle la
 * documentation est validee. Code et document ne peuvent plus diverger.
 */
const KEY_STATUS = require("./canonical-contracts.js").KEY_STATUS;
...
function makeKeyRecord(k) {
  ...
  // §30 — un statut inconnu ne devient jamais ACTIVE par defaut. L'absence est
  // tolerable (ACTIVE explicite par defaut contractuel) ; une valeur INCONNUE
  // est un echec ferme : on ne devine pas l'etat d'une cle.
  // §38 — un statut absent, nul ou inconnu est INVALIDE. v0.6 faisait valoir
  // ACTIVE par defaut : une cle sans statut devenait utilisable par accident.
  if (!isNonEmptyStr(k.status) || !KEY_STATUS[k.status]) {
    throw fail("KEY_STATUS_INVALID",
      "statut de cle " + JSON.stringify(k.status === undefined ? null : k.status) + " invalide : "
      + "un statut absent, nul ou inconnu n'est jamais ACTIVE par defaut. Valeurs admises : "
      + Object.keys(KEY_STATUS).join(", ") + ".");
  }
...
function evaluateKeyAt(key, signedAtIso, nowIso) {
  ...
  return { usable: false, reason: "statut de cle inconnu — fail closed" };
}
