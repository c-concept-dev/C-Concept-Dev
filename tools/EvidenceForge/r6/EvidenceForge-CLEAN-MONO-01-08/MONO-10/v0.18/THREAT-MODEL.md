# MONO-10 v0.18 — modèle de menace
...
| Attaque | Issue |
|---|---|
| deux configurations distinctes déclarant le même `operatorTrustBoundaryId` | `AUTHORITY_CONFIG_BINDING_MISMATCH` (V011-R1) |
...
| rejeu via un symlink ou un alias relatif du fichier de configuration | même ancre, même réserve : nonce refusé (V011-R2) |
| ancre de configuration non résolvable | `TRUST_CONFIG_ANCHOR_UNRESOLVABLE` — fail closed |
| `opts.verifier` fabriqué portant `provenanceAuthority()` | ignoré et consigné ; provenance `UNRESOLVED` (V011-R3). En v0.10 ce getter n'existe plus sur le vrai vérificateur |
...
| ré-étiqueter vers un triplet **hors liste blanche** | la capacité reste **inutilisable** (T10). Le code observé est `LLM_SUBJECT_MISMATCH` : la branche `LLM_SUBJECT_OUT_OF_ALLOWLIST` existe mais l'ordonnancement la rend inatteignable — garantie **comportementale**, pas garantie de code (R5, OPEN-FINDINGS.md) |
...
| `assertCapabilityUsable` sur un artefact non enregistré | `usable = false` (V011-R4) |
| `assertCapabilityUsable` hors registre authentifié | `usable = false` |
| champs de surface `namespace`, `provisionedFrom`, `transportOrigin` mensongers | sans effet **utile à l'attaquant**, mais la raison n'est pas celle qui était écrite : `assertCapabilityUsable` lit le champ **de l'artefact**, sans recoupement avec le descripteur. Ce qui bloque réellement est l'impossibilité d'obtenir une `probeRef` hors de la vraie frontière (R3, OPEN-FINDINGS.md) |
| `assertReadinessPhase` avec un registre de forme compatible | `READINESS_REGISTRY_INVALID` (V011-R5) |
...
## 7. Ce que ce modèle de menace ne couvre PAS (corrections v0.12 à v0.18)

### Note sur les étiquettes de réserve

Deux séries d'étiquettes coexistent dans ce document, et les confondre a
produit une ambiguïté que v0.13 a corrigée :

- **`V011-R1` … `V011-R5`** désignent les **résidus de v0.9 fermés par v0.10**.
  Ils sont **fermés**. Ils apparaissent dans les tableaux d'attaques ci-dessus à
  titre de traçabilité historique.
- **`R1` … `R5`**, toujours suivis de `OPEN-FINDINGS.md`, désignent les **cinq
  réserves ouvertes** du lot courant. Elles ne sont **pas** fermées.

Quatre affirmations de v0.11 promettaient plus que le code. Elles sont
corrigées ci-dessus et consignées en `OPEN-FINDINGS.md` :

| Affirmation de v0.11 | Ce qui est vrai |
|---|---|
| « l'appelant peut dégrader une préparation, il ne peut pas l'améliorer » | faux à la couche `assertReadinessPhase` ; vrai de bout en bout (`OPEN-FINDINGS.md` R2, et `READINESS.md`) |
| « champs de surface mensongers : le descripteur authentifié prime » | il n'y a **aucun recoupement** avec le descripteur ; ce qui bloque est ailleurs (R3, OPEN-FINDINGS.md) |
| « le fait qu'il ait essayé est inscrit dans l'artefact » (`callerTransportIgnored`) | contrôle **auto-défaisant** : l'effacer fait passer, l'honnêteté fait échouer. Traçabilité, pas garantie (R3, OPEN-FINDINGS.md) |
| `LLM_SUBJECT_OUT_OF_ALLOWLIST` présenté comme le code qui s'active | branche inatteignable ; garantie comportementale seulement (R5, OPEN-FINDINGS.md) |

**Douze champs de l'artefact de capacité LLM ne sont pas attestés**, et le
schéma n'est pas fermé à l'enregistrement. Aucun consommateur critique ne les
lit, mais le modèle de menace ne prétend plus qu'ils soient recoupés. Liste
canonique, dont ce nombre est dérivé : `LLM-CAPABILITY-BOUNDARY.md`, « Champs non attestés ».

**Neuf divergences de sceau historiques subsistent** dans `MONO-07` et
`MONO-08/v0.6`, antérieures à v0.11. Ce modèle de menace ne les traite pas :
elles relèvent d'une décision du propriétaire (`OPEN-FINDINGS.md` R4, et `NON-REGRESSION.md` §3).
