# MONO-10 v0.9 — non-régression

## 1. Ce qui est affirmé, et ce qui ne l'est pas

| Affirmation | Valeur |
|---|---|
| `MONO10_V09_CAUSED_REGRESSION` | **NO** |
| `AGGREGATE_BASELINE_IDENTICAL` | **21 / 21 lots** |
| `SEALED_REFERENCE_DIVERGENCES` | **0** (sur les 12 lots qui portent un sceau) |
| `FULL_HISTORY_BYTE_IDENTITY_CONFIRMED` | **NO** |
| `UNVERIFIABLE_HISTORICAL_LOTS` | **9** |

La dernière ligne est négative et elle n'est pas enfouie : **9 lots sur 21 ne
...
La baseline `/tmp/baseline-pre-v09.txt` a été capturée **avant** tout travail
...
v0.9. *Unknown remains unknown* — je ne convertis pas une archive d'une autre
...
`NR-01` (380 références scellées) et `NR-02` (identité croisée MONO-01) sont
