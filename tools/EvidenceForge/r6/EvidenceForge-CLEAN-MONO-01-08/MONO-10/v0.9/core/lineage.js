"use strict";
/**
 * MONO-10 v0.9 — core/lineage.js   (§38, §39, §40, §23)
 *
...
function resolveLineage(refs, registry, opts) {
  ...
  if (hiRequests.length) {
    /**
     * §5/§12 (v0.8) — l'autorite doit appartenir a LA FRONTIERE qui a ouvert le
     * run de destination. En v0.7, `isProvisionedHistoricalInputAuthority` ne
     * verifiait que la marque de module : une autorite construite ailleurs — y
     * compris une fabrique de TEST — ouvrait le franchissement de run.
     *
     * Sans identite de frontiere disponible, on refuse : on ne verifie pas une
     * origine contre rien.
     */
    // §4 (v0.9) — identite COMPOSITE. Un identifiant declare ne suffit pas.
    const src = (opts.verifier && opts.verifier.boundaryIdentity)
      || (opts.manifest ? { operatorBoundaryId: opts.manifest.operatorTrustBoundaryId,
        configBindingHash: opts.manifest.configBindingHash, executionMode: opts.manifest.executionMode } : null)
      || (registry && registry.boundaryIdentity) || null;
    const expectedIdentity = (src && isNonEmptyStr(src.operatorBoundaryId) && isNonEmptyStr(src.configBindingHash))
      ? src : null;
    if (!expectedIdentity) {
      problems.push("entree historique demandee sans identite de frontiere operateur COMPLETE (identifiant et "
        + "liaison de configuration) — refus par defaut.");
    } else if (!OHIA.isProvisionedHistoricalInputAuthority(hiAuthority, expectedIdentity)) {
      problems.push("l'OperatorHistoricalInputAuthority presentee n'a pas ete emise par la frontiere \""
        + expectedIdentity.operatorBoundaryId + "\" (liaison de configuration comparee) qui a ouvert ce run — refus.");
    } else {
