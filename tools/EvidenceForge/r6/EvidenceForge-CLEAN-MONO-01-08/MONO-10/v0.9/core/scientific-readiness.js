"use strict";
/**
 * MONO-10 v0.9 — core/scientific-readiness.js  (§17, §18)
 *
 * FERMETURE v0.3 B-6.
 *
...
function assertReadinessPhase(readiness, expectedPhase, opts, label) {
  ...
  if (opts.verifier && typeof opts.verifier.operatorTrustBoundaryId === "string"
    && opts.registry.operatorTrustBoundaryId !== opts.verifier.operatorTrustBoundaryId) {
    throw fail("READINESS_REGISTRY_BOUNDARY_MISMATCH", label + " : registre ouvert sous une autre frontiere operateur.");
  }
  ...
  if (opts.registry) {
    ...
    PHASE_DIMENSIONS[expectedPhase].forEach(function (n) {
      ...
      const ceiling = ceilingFn(entry && entry.artifact, entry,
        { executionMode: readiness.executionMode || (opts.manifest && opts.manifest.executionMode) || null });
