# MONO-10 v0.9 — modèle de confiance
...
| **v0.9** | ce qui précède **et** l'identité composite, physique et vérifiée à chaque consommation | hypothèse §7 ci-dessous |
...
- **un objet portant `provenanceAuthority()`** : seul un vérificateur marqué par
  `operator-trust-verifier` peut désigner l'autorité de provenance. Un callback
  d'appelant a zéro effet (fermeture v0.9, R3) ;
