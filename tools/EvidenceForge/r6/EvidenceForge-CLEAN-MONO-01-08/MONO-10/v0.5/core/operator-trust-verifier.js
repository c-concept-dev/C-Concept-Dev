"use strict";
/**
 * MONO-10 v0.5 — core/operator-trust-verifier.js   (§4, §5, §12)
 *
...
function createOperatorTrustVerifier(boundary) {
  ...
  const v = {
    schema: "EvidenceForge.OperatorTrustVerifier", schemaVersion: "MONO-10-v5",
    ...
    verifyRuntimeAttestation(req) {
      ...
      const key = boundary.lookupKey(att.authorityId, att.keyId);
      if (!key) {
      ...
      // §12 — anti-rejeu OBLIGATOIRE en production, fourni par la frontiere.
      ...
      if (boundary.policy.requireAntiReplay) {
        ...
        } else if (req.consumeNonce === true) {
          const c = store.checkAndConsume(att.authorityId, att.nonce, { runId: att.runId, attestationId: att.attestationId });
          nonceConsumed = c.consumed;
          if (!c.consumed) problems.push("attestation rejouee : " + c.reason);
        } else if (store.hasSeen(att.authorityId, att.nonce)) {
          // Reverification legitime : le nonce a ete consomme par CE run. Un
          // rejeu, lui, reutilise le nonce pour ouvrir un AUTRE run.
          const rec = typeof store.getSeen === "function" ? store.getSeen(att.authorityId, att.nonce) : null;
      ...
      } else if (RP.isProvisionedStore(store) && req.consumeNonce === true) {
        const c = store.checkAndConsume(att.authorityId, att.nonce, { runId: att.runId });
        nonceConsumed = c.consumed;
        if (!c.consumed) problems.push("attestation rejouee : " + c.reason);
      }
    ...
    verifyHumanAct(act, ctx) {
      ...
      const res = mech.verifyHumanAct(act, ctx || {});
