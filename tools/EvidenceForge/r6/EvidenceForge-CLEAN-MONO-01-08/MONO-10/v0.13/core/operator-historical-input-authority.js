"use strict";
/**
 * MONO-10 v0.11 — core/operator-historical-input-authority.js   (§7, §8)
 *
...
function build(descriptor, entries, sourcePath) {
  ...
  function verifyDecision(derivationRef, expectations) {
    ...
    if (!d) return { valid: false, problems: ["derivation absente du registre de decisions historiques"] };
    const problems = [];
    if (isNonEmptyStr(e.artifactId) && d.subject.artifactId !== e.artifactId) problems.push("artefact different");
    if (isNonEmptyStr(e.destinationRunId) && d.subject.destinationRunId !== e.destinationRunId) problems.push("run de destination different");
    if (isNonEmptyStr(e.executionMode) && d.executionMode !== e.executionMode) problems.push("mode d'execution different");
    if (d.result.status !== "AUTHORIZED") problems.push("decision non autorisante");
  ...
  function authorize(request) {
    ...
    const decision = Object.freeze({
      ...
      decisionKind: "HISTORICAL_INPUT_AUTHORIZATION",
      ...
      executionMode: descriptor.executionMode, issuerGeneration: descriptor.issuerGeneration,
      subject: Object.freeze({ artifactId: e.artifactId, artifactHash: e.artifactHash,
        destinationRunId: e.destinationRunId, destinationMissionHash: e.destinationMissionHash }),
      result: Object.freeze({ status: "AUTHORIZED" }), decidedAt: new Date().toISOString(),
    });
    const dref = "decision:historical_input_authorization:" + sha256Of(decision).slice(0, 40);
    ledger.set(dref, decision);
    authorization.derivationRef = dref;
  ...
  const authority = { schema: "EvidenceForge.OperatorHistoricalInputAuthority", schemaVersion: "MONO-10-v8",
    ...
    boundFields: Object.freeze(BOUND_FIELDS.slice()), authorize: authorize, verifyDecision: verifyDecision };
