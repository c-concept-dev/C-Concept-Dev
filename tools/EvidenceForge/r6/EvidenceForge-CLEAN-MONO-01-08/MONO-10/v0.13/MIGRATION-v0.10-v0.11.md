...
| la liste blanche s'appliquait à la sonde seule | elle est **ré-appliquée à la certification** — `LLM_SUBJECT_OUT_OF_ALLOWLIST`. *Corrigé depuis : ce code est **inatteignable** dans l'ordonnancement réel ; la garantie est comportementale (`OPEN-FINDINGS.md` R5).* |
