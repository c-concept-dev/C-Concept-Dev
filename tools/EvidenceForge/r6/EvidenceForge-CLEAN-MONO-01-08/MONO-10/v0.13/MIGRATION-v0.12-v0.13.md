...
| 4.5 | la section de correction de `THREAT-MODEL` était insérée après la liste à puces de §6 | elle devient **§7**, avec une note explicite sur les deux séries d'étiquettes |
...
2. **témoin négatif par snippet isolé** — il prouvait que la regex reconnaît la
   ...
   Désormais : **mutation contrôlée des documents livrés**, recopiés dans un
...
refus sur-promis (§5.4) et la cohérence du nombre de réserves.
