# MONO-10 v0.13 — constats ouverts
...
**non résolues par v0.13**. v0.13, comme v0.12, est un successeur strictement
documentaire : il ne change aucun comportement, il cesse de mentir sur celui
qu'il a.
...
| Réserve | Statut | Bloquant ? | Corrigée en v0.13 ? | Couche où elle est compensée |
|---|---|---|---|---|
| **R1** résultat de sonde déclaré non confronté | `OPEN` | `NON_BLOCKING` | `NOT_FIXED_IN_V013` | `assertCapabilityUsable`, puis `qualifyProcess`, puis autorisation aval |
| **R2** assertion de préparation plus permissive que son consommateur | `OPEN` | `NON_BLOCKING` | `NOT_FIXED_IN_V013` | `qualifyProcess` (recalcul depuis les sources), puis autorisation aval |
| **R3** champs informatifs non attestés ; schéma non fermé | `OPEN` | `NON_BLOCKING` | `NOT_FIXED_IN_V013` | aucune couche n'en a besoin : **aucun consommateur critique ne les lit** |
| **R4** 9 divergences de sceau historiques préexistantes | `OPEN` | `NON_BLOCKING` | `NOT_FIXED_IN_V013` | hors périmètre MONO-10 — décision du propriétaire sur `MONO-07` et `MONO-08/v0.6` |
| **R5** `LLM_SUBJECT_OUT_OF_ALLOWLIST` inatteignable | `OPEN` | `NON_BLOCKING` | `NOT_FIXED_IN_V013` | la garantie comportementale tient par le refus de sujet (`LLM_SUBJECT_MISMATCH`) et par `probe()` en amont |

`NOT_FIXED_IN_V013` n'est pas une négligence : fermer l'une de ces réserves
...
| | |
|---|---|
| **Fait** | corrige la vérité documentaire : mesures de non-régression, promesse de préparation formulée par couche, champs non attestés énumérés, garantie de liste blanche reformulée au niveau comportemental. **v0.13 y ajoute** : correction de `governance/README.md`, qualification en place de `callerTransportIgnored` dans `LLM-CAPABILITY-BOUNDARY.md` §3, désambiguïsation des étiquettes de réserve, et détecteurs documentaires rendus réellement causaux |
| **Ne fait pas** | ne change aucun comportement d'exécution ; ne ferme **aucune** des cinq réserves R1 à R5 ; ne corrige pas les divergences de `MONO-07` et `MONO-08/v0.6` |

`RUNTIME_BYTE_IDENTICAL_TO_V011 = YES` et
`RUNTIME_BYTE_IDENTICAL_TO_V012 = YES` sont les propriétés centrales de ce lot,
et elles sont vérifiables : voir `NON-REGRESSION.md` §1.
