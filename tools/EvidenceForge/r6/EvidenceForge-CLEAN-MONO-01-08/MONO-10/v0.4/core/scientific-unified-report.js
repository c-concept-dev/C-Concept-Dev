"use strict";
/**
 * MONO-10 v0.4 — core/scientific-unified-report.js  (§21)
 *
...
const { artifactRef, createArtifactRegistry, assertLineageResolved, assertRefMatches, RELATION } = require("./lineage.js");
...
function buildScientificUnifiedReport(input) {
  ...
  const priorRef = artifactRef(prior, "prior-unified-report", RELATION.MISSION);
  ...
  const registry = createArtifactRegistry(named.map((p) => ({ artifactId: p[0], relation: p[2], artifact: p[1] })));
  if (input.artifactRegistry && typeof input.artifactRegistry.forEach === "function") {
    input.artifactRegistry.forEach(function (e, id) { if (!registry.has(id)) registry.set(id, e); });
  }
  assertLineageResolved(refs, registry, { relation: RELATION.REPORT }, "lineage du rapport unifie");

  return {
    schema: "EvidenceForge.ScientificUnifiedReport", schemaVersion: "MONO-10-v4",
