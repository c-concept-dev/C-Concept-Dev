"use strict";
/**
 * MONO-10 v0.4 — core/downstream-authorization.js  (§5, §6, §23, §24)
 *
...
function resolveDownstreamUseAuthorization(input) {
  ...
  // ---- §23 — racine de confiance d'abord.
  let trustOk = false;
  if (ctx.manifest) {
    try { RM.assertManifestAuthentic(ctx.manifest, ctx.anchorSet, ctx.attestation, { now: ctx.now, seenNonces: ctx.seenNonces }); trustOk = true; }
...
function emit(status, reasons, policy, input, ctx, revalidation, refusedOverrides) {
  const out = {
    schema: "EvidenceForge.DownstreamUseAuthorization", schemaVersion: "MONO-10-v4",
