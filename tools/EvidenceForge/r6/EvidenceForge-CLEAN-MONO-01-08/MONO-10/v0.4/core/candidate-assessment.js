"use strict";
/**
 * MONO-10 v0.4 — core/candidate-assessment.js
 *
...
function assessCandidates(input) {
  ...
  const idOpts = Object.assign({ authorityRegistry: input.authorityRegistry }, policy.identity || {});
  ...
  const out = {
    schema: "EvidenceForge.ProfessionalCandidateAssessment", schemaVersion: "MONO-10-v4",
...
/**
 * Extracteur STRUCTUREL par defaut : identifiants opaques de meme rang.
 * Il ne connait ni ORCID, ni OpenAlex, ni aucun registre, et ne DEDUIT jamais
 * une autorite : `sourceAuthorityId` ne vient que d'une declaration explicite,
 * qui devra encore etre resolue au registre exterieur pour compter.
 */
function defaultIdentityExtractor(c, v) {
  const out = [];
  const push = (type, authority, family, id, prov, binding, verif, contrib) => {
    if (!isNonEmptyStr(id)) return;
    out.push({ evidenceType: type, sourceAuthorityId: authority || null, sourceFamilyId: family || null,
      identifier: id, assertedValue: null, provenanceRecordHash: prov || null,
      subjectBinding: binding, verificationStatus: verif, confidenceContribution: contrib });
  };
  push("provider-native-id", c.sourceAuthorityId, c.sourceFamilyId, c.candidateRef, c.provenanceRecordHash,
  ...
  (Array.isArray(c.identifiers) ? c.identifiers : []).forEach(function (x) {
    push(x.type || "external-id", x.sourceAuthorityId, x.sourceFamilyId, x.value, x.provenanceRecordHash,
  ...
  if (isNonEmptyStr(c.displayName)) {
    out.push({ evidenceType: DISPLAY_NAME_TYPE, sourceAuthorityId: null, sourceFamilyId: null, identifier: null,
      assertedValue: c.displayName, provenanceRecordHash: null,
