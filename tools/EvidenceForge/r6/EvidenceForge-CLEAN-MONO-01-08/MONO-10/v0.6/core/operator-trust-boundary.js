...
function buildBoundary(cfg, namespace, provisionedFrom, sourceDescriptor) {
  ...
  if (rp.kind === RP.KIND.FILE) {
    ...
    // §26/§27 — le store declare le PERIMETRE D'AUTORITES qu'il couvre.
    replayStore = RP.createFileReplayStore({ directory: rp.directory, authorityScope: rp.authorityScope || authorityIds });
    const notCovered = authorityIds.filter((a) => !replayStore.coversAuthority(a));
    if (notCovered.length) {
      throw fail("REPLAY_PROTECTION_SCOPE_INCOMPLETE", "le store de rejeu ne couvre pas " + notCovered.join(", ")
    ...
    replayStore = RP.createMemoryReplayStore({ namespaceId: cfg.operatorTrustBoundaryId || "test", authorityScope: authorityIds });
  ...
  return sealBoundary({
    schema: "EvidenceForge.OperatorTrustBoundary", schemaVersion: "MONO-10-v6",
