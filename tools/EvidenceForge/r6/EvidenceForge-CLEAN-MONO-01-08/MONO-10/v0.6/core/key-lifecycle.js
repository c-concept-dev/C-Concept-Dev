"use strict";
/**
 * MONO-10 v0.6 — core/key-lifecycle.js   (§9, §10, §11)
 *
...
function makeKeyRecord(k) {
  ...
  if (k.status !== undefined && k.status !== null && !KEY_STATUS[k.status]) {
    throw fail("KEY_STATUS_UNKNOWN", "statut de cle \"" + k.status + "\" inconnu — fail closed, jamais ACTIVE par defaut.");
  }
  const status = k.status ? k.status : KEY_STATUS.ACTIVE;
