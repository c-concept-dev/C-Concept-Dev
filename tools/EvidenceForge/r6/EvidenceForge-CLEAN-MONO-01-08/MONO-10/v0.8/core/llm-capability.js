"use strict";
/**
 * MONO-10 v0.8 — core/llm-capability.js  (§19)
 *
    ...
    if (entry) {
      ...
      try {
        AC.assertCapability(entry, required, "llm-capability:production-use",
          ctx.manifest ? ctx.manifest.operatorTrustBoundaryId : null);
      } catch (err) {
        problems.push("capacite LLM de production refusee : " + String(err.message).slice(0, 180));
      }
    }
  }
  if (production) {
