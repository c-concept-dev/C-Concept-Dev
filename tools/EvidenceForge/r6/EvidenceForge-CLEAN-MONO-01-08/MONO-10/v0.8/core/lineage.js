"use strict";
/**
 * MONO-10 v0.8 — core/lineage.js   (§38, §39, §40, §23)
 *
...
function resolveLineage(refs, registry, opts) {
  ...
  if (hiRequests.length) {
    ...
    const expectedBoundaryId = (opts.verifier && opts.verifier.operatorTrustBoundaryId)
      || (opts.manifest && opts.manifest.operatorTrustBoundaryId)
      || (registry && registry.operatorTrustBoundaryId) || null;
    if (!isNonEmptyStr(expectedBoundaryId)) {
      problems.push("entree historique demandee sans identite de frontiere operateur verifiable — refus par defaut.");
    } else if (!OHIA.isProvisionedHistoricalInputAuthority(hiAuthority,
      { operatorBoundaryId: expectedBoundaryId })) {
      problems.push("l'OperatorHistoricalInputAuthority presentee n'a pas ete emise par la frontiere \""
        + expectedBoundaryId + "\" qui a ouvert ce run — refus par defaut.");
    } else {
