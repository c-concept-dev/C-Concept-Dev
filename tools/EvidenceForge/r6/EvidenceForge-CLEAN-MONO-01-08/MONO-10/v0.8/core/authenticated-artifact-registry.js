"use strict";
/**
 * MONO-10 v0.8 — core/authenticated-artifact-registry.js   (§15–§19, §57)
 *
...
function openAuthenticatedArtifactRegistry(manifest, ctx) {
  ...
  const reg = {
    ...
    grantCapability(artifactId, grant) {
      ...
      if (grant.operatorBoundaryId !== manifest.operatorTrustBoundaryId) {
        throw fail("CAPABILITY_GRANT_CROSS_BOUNDARY",
          "concession emise sous la frontiere \"" + grant.operatorBoundaryId + "\" alors que ce run a ete ouvert sous \""
          + manifest.operatorTrustBoundaryId + "\" — refus.");
      }
