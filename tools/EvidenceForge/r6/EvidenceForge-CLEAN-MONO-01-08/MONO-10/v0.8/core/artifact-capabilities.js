"use strict";
/**
 * MONO-10 v0.8 — core/artifact-capabilities.js   (§26, §27, §28, §29, §30, §31)
 *
...
function mintCapabilityGrant(input) {
  ...
  AD.assertSameBoundary(issuerDescriptor, input.operatorBoundaryId, "autorite emettrice");
 ...
 */
function assertCapability(entry, capability, sinkLabel, expectedBoundaryId) {
  const have = (entry && Array.isArray(entry.capabilities)) ? entry.capabilities : [];
  if (have.indexOf(capability) !== -1 && isNonEmptyStr(expectedBoundaryId)) {
    ...
    if (g.operatorBoundaryId !== expectedBoundaryId) {
      throw fail("ARTIFACT_CAPABILITY_CROSS_BOUNDARY",
        (sinkLabel || "sink") + " : capacite \"" + capability + "\" emise sous la frontiere \""
        + g.operatorBoundaryId + "\", presentee sous \"" + expectedBoundaryId + "\" — refus.");
    }
  }
