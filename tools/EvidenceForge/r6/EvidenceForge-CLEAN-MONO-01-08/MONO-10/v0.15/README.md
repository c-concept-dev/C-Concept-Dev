# MONO-10 v0.15 — FINAL DOCUMENTARY FRAME CLARIFICATION & DETECTOR CLOSURE
...
Un successeur **strictement documentaire** de v0.14 — qui l'était de v0.13, de
v0.12 et de v0.11.
...
`RUNTIME_BYTE_IDENTICAL_TO_V013 = YES` **et** `RUNTIME_BYTE_IDENTICAL_TO_V014 = YES`,
vérifiables en quarante-huit `diff` (douze chemins × quatre lots,
`NON-REGRESSION.md` §1). La **Charte** reste byte-identique.

## Pourquoi il existe

Cinq audits indépendants se sont succédé. Les deux premiers ont conclu que
...
toujours **sans aucun bloqueur runtime**.
...
### Ce que v0.15 corrige, et que v0.14 avait laissé

| # | Où | Résidu de v0.14 | Ce que v0.15 fait |
|---|---|---|---|
| **B1** | `OPEN-FINDINGS.md` R4 | « 24 lots historiques scellés / 1 577 références » au présent, sans cadre — dans l'arbre livré, le cadre v0.14 mesurait 27 / 1 787 | R4 nomme le **cadre historique v0.11** (24 / 1 577) et le **cadre courant v0.15** (28 / 1 860, mesuré après scellement) ; `DOC-05` refuse désormais tout chiffre de cadre cité sans son cadre, en phrase, tableau, puce, parenthèse, gras ou JSON |
...
| `RUNTIME_BYTE_IDENTICAL_TO_V014` | **YES** (12 chemins) — 48 comparaisons vides au total |
...
| `SEALED_HISTORICAL_LOTS_COUNT` | **24** cadre v0.11 · **25** cadre v0.12 · **26** cadre v0.13 · **27** cadre v0.14 · **28** cadre v0.15 |
| `SEALED_REFERENCES_COUNT` | **1 577** cadre v0.11 · **1 644** cadre v0.12 · **1 715** cadre v0.13 · **1 787** cadre v0.14 · **1 860** cadre v0.15 |
| `ALL_SEALS_FILE_COUNT` / `ALL_SEALS_REFERENCE_COUNT` | **39** / **3 417**, arbre complet au scellement de v0.15, mesuré après scellement (`NON-REGRESSION.md` §2) |
| `BUILD_WINDOW_OPENED` (v0.15) | dérivé, jamais saisi : `NON-REGRESSION.md` §7, `MANIFEST.sealInventory.v015BuildWindowOpened`, contrôle `DOC-20` |
...
| Contrôles documentaires | **21 / 21**, chacun avec `PASS_CLEAN` / `FAIL_SINGLE_TARGET_FAULT` / `PASS_AFTER_REVERT` sur le document réel |
| Variantes de sur-promesse d'attestation attrapées | **15 / 15** (`ATTESTATION_OVERCLAIM_VARIANTS_CAUGHT = ALL`) |
| Réserves ouvertes consignées | **5** (R1–R5), toutes `OPEN` / `NON_BLOCKING` / `NOT_FIXED_IN_V015` |
...
4. **Neuf divergences de sceau subsistent** dans `MONO-07` et `MONO-08/v0.6`.
   v0.15 ne les corrige pas. (`OPEN-FINDINGS.md` R4)
...
4. La garde de contrats couvre des **énumérations**, pas la prose
   ...
   `test/test-mono10-v0.15-documentary.js` en couvre désormais vingt-et-un,
...
`OPEN-FINDINGS.md` **d'abord** → `MIGRATION-v0.14-v0.15.md` →
