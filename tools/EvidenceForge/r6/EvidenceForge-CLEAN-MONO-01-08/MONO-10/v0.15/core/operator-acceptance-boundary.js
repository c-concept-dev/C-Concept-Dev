"use strict";
/**
 * MONO-10 v0.11 — core/operator-acceptance-boundary.js   (§5, §6, §7, §8, §9)
 *
 ...
 */
function buildAcceptanceBoundary(input) {
  input = input || {};
...
/**
 * §22 (v0.9) — la fabrique publique est retiree de la surface. En v0.8,
 * `createAcceptanceBoundary({namespace, validate})` etait exportee : elle
 * n'emet aucune capacite de securite, mais laisser une fabrique d'objet marque
 * sur la surface publique est exactement le motif que v0.8 a ferme ailleurs.
 * Elle n'est desormais atteignable que depuis la frontiere.
 */
function createFromBoundary(issuance, input) {
  ...
  if (!OTB.isBoundaryIssuanceHandle(issuance)) {
    throw fail("AUTHORITY_ISSUER_NOT_BOUNDARY",
      "une frontiere d'acceptation ne se construit que depuis une OperatorTrustBoundary provisionnee.");
  }
  return buildAcceptanceBoundary(Object.assign({}, input || {}, { namespace: issuance.namespace,
    boundaryId: issuance.operatorBoundaryId }));
}

module.exports = { createFromBoundary, isProvisionedAcceptanceBoundary,
