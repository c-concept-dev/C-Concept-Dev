# MONO-10 v0.15 — constats ouverts
...
**non résolues par v0.15**. v0.15, comme v0.12 à v0.14, est un successeur
...
| Réserve | Statut | Bloquant ? | Corrigée en v0.15 ? | Couche où elle est compensée |
|---|---|---|---|---|
| **R1** résultat de sonde déclaré non confronté | `OPEN` | `NON_BLOCKING` | `NOT_FIXED_IN_V015` | `assertCapabilityUsable`, puis `qualifyProcess`, puis autorisation aval |
| **R2** assertion de préparation plus permissive que son consommateur | `OPEN` | `NON_BLOCKING` | `NOT_FIXED_IN_V015` | `qualifyProcess` (recalcul depuis les sources), puis autorisation aval |
| **R3** champs informatifs non attestés ; schéma non fermé | `OPEN` | `NON_BLOCKING` | `NOT_FIXED_IN_V015` | aucune couche n'en a besoin : **aucun consommateur critique ne les lit** |
| **R4** 9 divergences de sceau historiques préexistantes | `OPEN` | `NON_BLOCKING` | `NOT_FIXED_IN_V015` | hors périmètre MONO-10 — décision du propriétaire sur `MONO-07` et `MONO-08/v0.6` |
| **R5** `LLM_SUBJECT_OUT_OF_ALLOWLIST` inatteignable | `OPEN` | `NON_BLOCKING` | `NOT_FIXED_IN_V015` | la garantie comportementale tient par le refus de sujet (`LLM_SUBJECT_MISMATCH`) et par `probe()` en amont |

`NOT_FIXED_IN_V015` n'est pas une négligence : fermer l'une de ces réserves
...
- **dans le cadre historique v0.11** (v0.11 à v0.15 exclus) : 24 lots scellés /
...
- **dans le cadre courant v0.15** (v0.15 seul exclu, mesuré après scellement) :
  28 lots scellés / 1 860 références / 9 divergences / 0 lot non scellé.

Les neuf divergences sont les mêmes dans tous les cadres (`NON-REGRESSION.md`
§2 pour les cinq cadres et leurs commandes rejouées) :
...
**Elles ne sont imputables ni à v0.11, ni à v0.12, ni à v0.13, ni à v0.14, ni à v0.15.** Leurs dates de
...
séparément. Ni v0.12, ni v0.13, ni v0.14, ni v0.15 ne les corrigent, ni ne prétendent le faire.
