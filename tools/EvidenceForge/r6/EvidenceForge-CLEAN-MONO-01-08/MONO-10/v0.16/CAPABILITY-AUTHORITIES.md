# MONO-10 v0.16 — autorités de capacité (contenu de v0.11, corrigé en v0.13)
...
| `LlmCapabilityAuthority` | `recordProbe` puis `certifyProbedArtifact` — la décision n'est inscrite qu'après confrontation du sujet **reconstruit depuis l'artefact** au sujet réellement sondé | `LLM_PROBE_EXECUTION` |
...
| `certifyLlmCapability({registry, artifactId})` | un verdict, sur le **sujet exact** de l'artefact. Codes réellement observés : `LLM_SUBJECT_MISMATCH`, `LLM_PROBE_ALREADY_CONSUMED`. `LLM_SUBJECT_OUT_OF_ALLOWLIST` existe mais est **inatteignable** dans l'ordonnancement actuel (`OPEN-FINDINGS.md` R5) |
| `verifyLlmSubject({registry, artifactId})` | lecture seule : cet artefact décrit-il ce qui a été réellement sondé ? |
...
## Complément v0.11 — une décision réelle ne suffit pas

v0.10 exigeait qu'une dérivation soit **vérifiable**. L'audit A a montré qu'une
dérivation vérifiable pouvait ne pas **couvrir** l'affirmation portée par la
concession : la décision existait, elle était authentique, et elle ne disait
rien du fournisseur que l'artefact déclarait.

> **Une décision réelle ne suffit pas si elle ne lie pas le sujet exact qui est
> certifié.**

`verifyDecision` de la frontière de capacité LLM exige désormais sept
dimensions — `providerId`, `modelId`, `workerBindingId`, `artifactId`,
`artifactHash`, `runId`, `missionHash` — plus `executionMode`. **Une attente
absente est un refus**, jamais une dispense. La concession porte
`certifiedSubject` et `decisionSubjectHash` : elle nomme ce qu'elle certifie.
