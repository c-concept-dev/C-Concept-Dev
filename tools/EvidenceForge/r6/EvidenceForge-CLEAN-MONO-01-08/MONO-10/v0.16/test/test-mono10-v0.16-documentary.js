...
"use strict";
/**
 * MONO-10 v0.16 — test/test-mono10-v0.16-documentary.js
 *
 ...
 *
 * CE QUE v0.16 AJOUTE (audit final de v0.15 : bloqueurs B1, B2 et lacunes
 * non bloquantes) :
 ...
 *
 * Usage : node test/test-mono10-v0.16-documentary.js [racineDuPaquet]
 */
...
const VERSION_TAG = (path.basename(__filename).match(/v0\.\d+/) || ["v0.16"])[0];   // "v0.16"
...
detector("DOC-05", "aucune mesure de sceau fausse ou perimee n'est AFFIRMEE comme courante, ni cadree avec un chiffre d'un autre cadre",
  ...
  [
    ...
    mut("OPEN-FINDINGS.md R4 : « cadre historique v0.11 … : 24 lots » devient « 25 lots »", "OPEN-FINDINGS.md", function (t) {
      return mustReplace(t, /(cadre historique v0\.11\*\* \(v0\.11 à v0\.16 exclus\) : )24 lots/, "$125 lots", "cadre historique v0.11");
    }),
    mut("AUDIT-VERDICTS.v0.12-v0.16.md (fichier racine a nom pointe) : « 24 lots / 1 577 références » sans cadre", "AUDIT-VERDICTS.v0.12-v0.16.md", function (t) {
...
detector("DOC-13", "chaque commande imprimee avec un « # attendu » reproduit exactement le resultat imprime",
  ...
  [
    mut("NON-REGRESSION.md : le chiffre attendu du cadre v0.11 est altere (24 -> 26)", "NON-REGRESSION.md", function (t) {
      return mustReplace(t, /(MONO-10\/v0\.11,MONO-10\/v0\.12,MONO-10\/v0\.13,MONO-10\/v0\.14,MONO-10\/v0\.15,MONO-10\/v0\.16\n# attendu : SEALED_HISTORICAL_LOTS_COUNT = )24/, "$126", "attendu cadre v0.11");
    }),
    mut("NON-REGRESSION.md : la commande imprimee du cadre v0.11 n'exclut plus que v0.11 (chiffre inchange)", "NON-REGRESSION.md", function (t) {
      return mustReplace(t, /node tools\/seal-inventory\.js <racine> MONO-10\/v0\.11,MONO-10\/v0\.12,MONO-10\/v0\.13,MONO-10\/v0\.14,MONO-10\/v0\.15,MONO-10\/v0\.16\n/, "node tools/seal-inventory.js <racine> MONO-10/v0.11\n", "commande cadre v0.11");
    }),
...
detector("DOC-20", "MANIFEST.sealInventory.v016BuildWindowOpened est derive de la methode canonique et re-mesurable",
  function (docs) {
    let man;
    try { man = JSON.parse(docs["MANIFEST.json"] || ""); } catch (e) { return ["MANIFEST.json illisible"]; }
    const hits = [];
    const declared = (man.sealInventory || {}).v016BuildWindowOpened;
    const unix = typeof declared === "number" ? declared : parseInt(String(declared || "").replace(/.*unix\s*/, ""), 10);
    if (!(unix > 0)) return ["v016BuildWindowOpened absent ou illisible : " + declared];
    ...
    else if (ceilEven(Math.min.apply(null, mt)) !== unix) hits.push("v016BuildWindowOpened = " + unix + " ; derive de min(files[].mtime) = " + ceilEven(Math.min.apply(null, mt)));
  ...
  [
    mut("MANIFEST.json : v016BuildWindowOpened est decale d'une seconde", "MANIFEST.json", function (t) {
      const man = JSON.parse(t); man.sealInventory.v016BuildWindowOpened = man.sealInventory.v016BuildWindowOpened + 1; return JSON.stringify(man, null, 2);
    }),
...
detector("DOC-22", "dans un registre multi-versions, aucune ligne ne decrit une correction d'une version posterieure a son heading / sa colonne de statut",
    ...
    Object.keys(docs).forEach(function (f) {
      ...
      docs[f].split("\n").forEach(function (l, i) {
        ...
        if (/^\s*\|/.test(l)) {
          ...
          if (/^\|\s*-+/.test(l)) return;
          const cap = colMax !== null ? colMax : secMax;
          if (cap === null) return;
          ...
          later.forEach(function (m) {
            ...
            if (n > cap) hits.push(f + ":" + (i + 1) + " :: ligne decrivant une correction de v0." + n + " sous un heading / une colonne limites a v0." + cap);
          });
          ...
          if (audit && parseInt(audit[1], 10) + 1 > cap) hits.push(f + ":" + (i + 1) + " :: constat de l'audit final de v0." + audit[1] + " (corrige au plus tot en v0." + (parseInt(audit[1], 10) + 1) + ") sous un heading / une colonne limites a v0." + cap);
        }
      });
    });
    return hits;
  ...
  [
    mut("AUDIT-REMEDIATION-MATRIX.md §11 : la colonne « Statut en v0.16 » redevient « Statut en v0.15 » au-dessus de lignes corrigees en v0.16", "AUDIT-REMEDIATION-MATRIX.md", function (t) {
      return mustReplace(t, /\| Statut en v0\.16 \|/, "| Statut en v0.15 |", "colonne Statut en v0.16");
    }),
...
detector("DOC-23", "le tableau de statut de OPEN-FINDINGS porte, pour chacune de R1..R5, OPEN / NON_BLOCKING / NOT_FIXED_IN_V016",
  function (docs) {
    const of = docs["OPEN-FINDINGS.md"] || "";
  ...
  [mut("OPEN-FINDINGS.md : la ligne R5 passe a CLOSED / FIXED_IN_V016, les quatre autres inchangees", "OPEN-FINDINGS.md", function (t) {
    return mustReplace(t, /(^\|\s*\*\*R5\*\*[^\n]*?)`OPEN`([^\n]*?)`NOT_FIXED_IN_V016`/m, "$1`CLOSED`$2`FIXED_IN_V016`", "ligne R5");
  })]);

/* ----- DOC-24 — aucun titre de version perime sur un document modifie depuis v0.11 ----- */
/**
 * Convention : un document modifie par un lot documentaire (v0.12+) porte dans
 * son H1 la version COURANTE (et, entre parentheses, sa version d'origine). Un
 * document byte-identique a v0.11 garde son titre d'origine ; la liste de ces
 * documents est DECLAREE (MANIFEST.originTitledDocuments) et, quand v0.11 est
 * joignable, verifiee octet pour octet (RT-08).
 */
detector("DOC-24", "aucun H1 « MONO-10 v0.NN » perime hors des documents declares identiques a v0.11",
  function (docs) {
    let man = null;
    ...
    if (!origin) return ["MANIFEST.originTitledDocuments absent"];
    const hits = [];
    Object.keys(docs).forEach(function (f) {
      if (isJson(f) || f === CHARTE) return;
      const h1 = (docs[f].match(/^# MONO-10 (v0\.\d+)\b/m) || [])[1];
      if (!h1) return;
      if (h1 === VERSION_TAG) return;
      if (origin[f] && origin[f] === h1) return;
      hits.push(f + ":1 :: titre « " + h1 + " » perime : document modifie depuis v0.11, ou version d'origine non declaree");
    });
    return hits;
  },
  [mut("README.md : le H1 redevient « MONO-10 v0.15 »", "README.md", function (t) {
    return mustReplace(t, /^# MONO-10 v0\.16/m, "# MONO-10 v0.15", "H1 README");
  })]);
...
detector("DOC-25", "champs du sujet enregistre et dimensions exigees par verifyDecision : enumerations et nombres derives du code",
  function (docs) {
    ...
    c.subjectFields.forEach(function (x) { if (r1.indexOf("`" + x + "`") === -1) hits.push("OPEN-FINDINGS.md R1 n'enumere pas le champ de sujet " + x); });
    ...
    c.required.forEach(function (x) { if (sec.indexOf("`" + x + "`") === -1) hits.push("LLM-CAPABILITY-BOUNDARY.md « Vérification du sujet complet » n'enumere pas la dimension exigee " + x); });
...
detector("DOC-19", "aucun statut NOT_FIXED_IN_V0xx / resolvedByV0xx d'une version anterieure n'est presente comme courant",
  ...
  [mut("OPEN-FINDINGS.md : un NOT_FIXED_IN_V016 redevient NOT_FIXED_IN_V016", "OPEN-FINDINGS.md", function (t) {
    return mustReplace(t, /NOT_FIXED_IN_V016/, "NOT_FIXED_IN_V015", "NOT_FIXED_IN_V016");
  })]);

const EXTRA_CHECK_IDS = ["COH-01", "COH-02", "COH-03", "COH-04", "COH-05", "COH-06", "RT-01", "RT-02", "RT-03", "RT-04", "RT-05", "RT-06", "RT-07", "RT-08",
  "SEAL-01", "SEAL-02", "SEAL-03", "SEAL-04", "SEAL-05", "SEAL-06", "SEAL-07", "SEAL-08", "MAN-01", "MAN-02", "MAN-03", "SCAN-01", "SCAN-02", "SCAN-03", "SCAN-04"];
...
/* ============================ execution ============================ */
(function () {
  console.log("MONO-10 v0.16 — controles de verite documentaire\n");
  ...
  check("SCAN-04. le balayage atteint un fichier documentaire racine a nom pointe (AUDIT-VERDICTS.v0.12-v0.16.md)",
    md.indexOf("AUDIT-VERDICTS.v0.12-v0.16.md") !== -1);
  ...
  const man = manifest || {};
  const of = docs["OPEN-FINDINGS.md"] || "";
  check("COH-01. OPEN-FINDINGS.md : chaque ligne R1..R5 du tableau de statut porte OPEN / NON_BLOCKING / NOT_FIXED_IN_V016 (verification individuelle, DOC-23)",
    ["R1", "R2", "R3", "R4", "R5"].every(function (r) {
      const row = (of.match(new RegExp("^\\|\\s*\\*\\*" + r + "\\*\\*[^\\n]*$", "m")) || [""])[0];
      return /`OPEN`/.test(row) && /`NON_BLOCKING`/.test(row) && /`NOT_FIXED_IN_V016`/.test(row);
    }));
  check("COH-02. aucune reserve n'est presentee comme resolue (MANIFEST.knownOpenFindings)",
    (man.knownOpenFindings || []).length === 5
    && (man.knownOpenFindings || []).every(function (r) { return r.resolvedByV016 === false && r.status === "OPEN" && r.blocking === false; }),
    JSON.stringify((man.knownOpenFindings || []).map(function (r) { return r.id + ":" + r.status + ":" + r.resolvedByV016; })));
  ...
  check("COH-04. MIGRATION v0.15 -> v0.16 existe et affirme l'absence de changement runtime",
    !!docs["MIGRATION-v0.15-v0.16.md"] && /Aucune API ne change/.test(docs["MIGRATION-v0.15-v0.16.md"] || ""));
  check("COH-05. le MANIFEST declare la nature documentaire et les cinq identites runtime",
    /DOCUMENTAIRE/i.test(man.natureOfLot || "")
    && ["MONO-10-v0.11", "MONO-10-v0.12", "MONO-10-v0.13", "MONO-10-v0.14", "MONO-10-v0.15"].every(function (v) { return (man.runtimeByteIdenticalTo || []).indexOf(v) !== -1; }),
  ...
  const staleKeys = Object.keys(man.measurements || {}).filter(function (k) { return /V01[0-5]\b/.test(k) && !/runtimeByteIdenticalToV01[0-5]/.test(k); });
  check("MAN-03. MANIFEST : reserves R1-R5 OPEN / NON_BLOCKING / NOT_FIXED_IN_V016, closesFindings vide, aucune mesure indexee sur une version anterieure recopiee comme courante",
    (man.closesFindings || []).length === 0 && (man.knownOpenFindings || []).length === 5
    && (man.knownOpenFindings || []).every(function (r) { return r.status === "OPEN" && r.blocking === false && r.resolvedByV016 === false && r.fixedIn === "NOT_FIXED_IN_V016"; })
    && staleKeys.length === 0 && man.correctsFalseStatementsOf === "MONO-10-v0.15",
  ...
  [["v0.11", "RT-01"], ["v0.12", "RT-02"], ["v0.13", "RT-03"], ["v0.14", "RT-06"], ["v0.15", "RT-07"]].forEach(function (pair) {
  ...
  check("RT-05. les outils et tests documentaires ajoutes n'ecrivent rien (aucun appel fs d'ecriture)",
    ["tools/seal-inventory.js", "test/test-mono10-v0.16-documentary.js"].every(function (t) {
  ...
  /** RT-08 — les documents declares « titre d'origine » sont byte-identiques a v0.11 */
  (function () {
    const v11 = path.resolve(LOT, "..", "v0.11");
    const origin = man.originTitledDocuments || {};
    if (!fs.existsSync(v11)) { skip("RT-08. documents a titre d'origine identiques a v0.11", "v0.11 non joignable"); return; }
    const bad = Object.keys(origin).filter(function (f) {
      const a = path.join(LOT, f), b = path.join(v11, f);
      return !fs.existsSync(a) || !fs.existsSync(b) || !fs.readFileSync(a).equals(fs.readFileSync(b));
    });
    check("RT-08. les " + Object.keys(origin).length + " documents declares a titre d'origine (MANIFEST.originTitledDocuments) sont byte-identiques a v0.11",
      Object.keys(origin).length > 0 && bad.length === 0, JSON.stringify(bad));
  })();

  /* ----- sceaux : cadres, mesures, jamais postules ----- */
  console.log("\n  -- inventaire des sceaux : six cadres --");
  if (!fs.existsSync(path.join(BUNDLE, "MONO-10"))) {
    skip("SEAL-01..08. inventaire", "paquet non joignable depuis cette extraction");
  } else {
    const SI = require(path.join(LOT, "tools", "seal-inventory.js"));
    const frames = [
      ["v0.11", "SEAL-01", ["MONO-10/v0.11", "MONO-10/v0.12", "MONO-10/v0.13", "MONO-10/v0.14", "MONO-10/v0.15", "MONO-10/v0.16"], [24, 1577, 9, 0]],
      ["v0.12", "SEAL-02", ["MONO-10/v0.12", "MONO-10/v0.13", "MONO-10/v0.14", "MONO-10/v0.15", "MONO-10/v0.16"], [25, 1644, 9, 0]],
      ["v0.13", "SEAL-03", ["MONO-10/v0.13", "MONO-10/v0.14", "MONO-10/v0.15", "MONO-10/v0.16"], [26, 1715, 9, 0]],
      ["v0.14", "SEAL-07", ["MONO-10/v0.14", "MONO-10/v0.15", "MONO-10/v0.16"], [27, 1787, 9, 0]],
      ["v0.15", "SEAL-08", ["MONO-10/v0.15", "MONO-10/v0.16"], [28, 1860, 9, 0]],
    ];
    let f11 = null;
    ...
    const f14 = SI.inventory(BUNDLE, "MONO-10/v0.16");
    console.log("  INFO  cadre v0.16 mesure : " + f14.sealedHistoricalLots + " lots, " + f14.sealedReferences + " references, "
    ...
    check("SEAL-04. cadre v0.16 : memes 9 divergences, 0 lot non scelle, v0.15 devenu historique (29 lots)",
      f14.sealedDivergences === 9 && f14.unverifiableHistoricalLots.length === 0 && f14.sealedHistoricalLots === 29,
    ...
    const nr = docs["NON-REGRESSION.md"] || "";
    check("SEAL-06. les six cadres sont nommes dans NON-REGRESSION.md, et le MANIFEST porte le cadre v0.16 mesure",
      ["v0.11", "v0.12", "v0.13", "v0.14", "v0.15", "v0.16"].every(function (v) { return new RegExp("cadre v0\\." + v.slice(3)).test(nr); })
      && man.sealInventory && man.sealInventory.frameV016
      && man.sealInventory.frameV016.sealedHistoricalLots === f14.sealedHistoricalLots
      && man.sealInventory.frameV016.sealedReferences === f14.sealedReferences
      ...
      JSON.stringify(man.sealInventory && man.sealInventory.frameV016));
  }
  ...
  console.log("KNOWN_FALSE_STATEMENTS_REMAINING (mesure, DOC-01..DOC-25 hors DOC-17 qui la compare) = " + DETECTORS.reduce(function (n, d) {
