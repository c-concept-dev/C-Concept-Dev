# MONO-10 v0.16 — non-régression

## 1. La propriété centrale de ce lot : le runtime est inchangé

v0.16 est un successeur **strictement documentaire** de v0.15, qui l'était de
v0.14, de v0.13, de v0.12 et de v0.11. La seule affirmation forte qu'il porte
est celle-ci, et elle est vérifiable en **soixante comparaisons** (douze
chemins × cinq lots de référence) :
...
| `MONO10_V16_CAUSED_REGRESSION` | **NO** |
| `MONO10_V16_CHANGED_ANY_API` | **NO** |
| `MONO10_V16_CHANGED_ANY_BEHAVIOUR` | **NO** |
...
|---|---|
| `core/` `adapters/` `validators/` `schemas/` `contracts/` | `governance/README.md` — document, corrigé en v0.13, v0.14, v0.15 et v0.16 |
...
| | `test/test-mono10-v0.16-documentary.js` (ajout, lecture seule) |
...
```
cd MONO-10
for prev in v0.11 v0.12 v0.13 v0.14 v0.15; do
  for d in core adapters validators schemas contracts; do diff -r $prev/$d v0.16/$d; done
  for f in test/fixture-chain.js test/test-mono10-v0.11.js \
    ...
    diff $prev/$f v0.16/$f; done
done
# attendu : comparaisons = 60 ; VIDES = 60
```

Sortie attendue : **rien**, sur les soixante comparaisons. La ligne
`# attendu` est **rejouée** par `test/test-mono10-v0.16-documentary.js`
...
- `test/test-mono10-v0.16-documentary.js` — contrôles documentaires, réécrits en
  v0.14, durcis en v0.15 et v0.16 : balayage récursif des `.md` **et** des `.json`,
...
```
node tools/seal-inventory.js <racine> MONO-10/v0.11,MONO-10/v0.12,MONO-10/v0.13,MONO-10/v0.14,MONO-10/v0.15,MONO-10/v0.16
...
l'historique. **Six cadres, six jeux de chiffres, tous vrais, tous
rejoués :**

| Cadre | Lots exclus | Lots scellés | Références | Divergences | Non scellés |
|---|---|---|---|---|---|
| **cadre v0.11** | v0.11, v0.12, v0.13, v0.14, v0.15, v0.16 | 24 | 1 577 | 9 | 0 |
| **cadre v0.12** | v0.12, v0.13, v0.14, v0.15, v0.16 | 25 | 1 644 | 9 | 0 |
| **cadre v0.13** | v0.13, v0.14, v0.15, v0.16 | 26 | 1 715 | 9 | 0 |
| **cadre v0.14** | v0.14, v0.15, v0.16 | 27 | 1 787 | 9 | 0 |
| **cadre v0.15** | v0.15, v0.16 | 28 | 1 860 | 9 | 0 |
| **cadre v0.16** | v0.16 | 29 | 1 934 | 9 | 0 |

```
node tools/seal-inventory.js <racine> MONO-10/v0.12,MONO-10/v0.13,MONO-10/v0.14,MONO-10/v0.15,MONO-10/v0.16
# attendu : SEALED_HISTORICAL_LOTS_COUNT = 25 ; SEALED_REFERENCES_COUNT = 1644 ; SEALED_DIVERGENCES = 9 ; UNVERIFIABLE_HISTORICAL_LOTS = 0
```

```
node tools/seal-inventory.js <racine> MONO-10/v0.13,MONO-10/v0.14,MONO-10/v0.15,MONO-10/v0.16
# attendu : SEALED_HISTORICAL_LOTS_COUNT = 26 ; SEALED_REFERENCES_COUNT = 1715 ; SEALED_DIVERGENCES = 9 ; UNVERIFIABLE_HISTORICAL_LOTS = 0
```

```
node tools/seal-inventory.js <racine> MONO-10/v0.14,MONO-10/v0.15,MONO-10/v0.16
# attendu : SEALED_HISTORICAL_LOTS_COUNT = 27 ; SEALED_REFERENCES_COUNT = 1787 ; SEALED_DIVERGENCES = 9 ; UNVERIFIABLE_HISTORICAL_LOTS = 0
```

```
node tools/seal-inventory.js <racine> MONO-10/v0.15,MONO-10/v0.16
# attendu : SEALED_HISTORICAL_LOTS_COUNT = 28 ; SEALED_REFERENCES_COUNT = 1860 ; SEALED_DIVERGENCES = 9 ; UNVERIFIABLE_HISTORICAL_LOTS = 0
```

```
node tools/seal-inventory.js <racine> MONO-10/v0.16
...
valeurs mesurées portées par `MANIFEST.sealInventory`. Le cadre v0.16 est
**mesuré après scellement** de v0.16, jamais postulé : la valeur « 0 non
scellé » n'est vraie qu'une fois `SHA256SUMS.txt` écrit.

### Le total, tous sceaux confondus — cadre : l'arbre complet au scellement de v0.16
...
de v0.14 ; 39 / 3 417 dans l'arbre de v0.15). Le total ne se recopie pas : il
se mesure, dans l'arbre où on le publie.

```
node tools/seal-inventory.js <racine>
# attendu : ALL_SEALS_FILE_COUNT = 40 ; ALL_SEALS_REFERENCE_COUNT = 3493 ; ALL_SEALS_DIVERGENCES = 9
```

| Population (arbre complet, v0.16 scellé) | Sceaux | Références | Divergences |
|---|---|---|---|
| lots historiques distincts (cadre v0.16) | 29 | 1 934 | 9 |
| copies imbriquées sous `dependencies/` | 10 | 1 483 | 0 |
| lot candidat v0.16 | 1 | 76 | 0 |
| **tous sceaux confondus** | **40** | **3 493** | **9** |
...
à v0.15, ni à v0.16.** Leurs dates de modification vont du 31/08/2026 au 03/09/2026, soit neuf à douze
...
Ce n'est **pas** une vérité générale des arbres v0.12 à v0.16 : chaque lot
...
dans aucune des six fenêtres de construction** (chaque fenêtre est définie en
§7).
...
`MONO10_V15_CAUSED_REGRESSION = NO` et `MONO10_V16_CAUSED_REGRESSION = NO` le
...
- `test/test-mono10-v0.16-documentary.js` — échoue si une fausseté documentaire
  corrigée par ce lot ou par ses prédécesseurs réapparaît. **Vingt-cinq**
...
**dérivée** et **rejouée** ; v0.16 applique la même méthode :
...
```
find MONO-10/v0.16 -type f ! -name '*.zip' -exec stat -f %m {} + | sort -n | head -1 | awk '{print $1 + ($1 % 2)}'
# attendu : BUILD_WINDOW_OPENED = 1789306460
```
...
`sealInventory.v016BuildWindowOpened` est le minimum de ces mtimes, arrondi.
...
Les fenêtres de v0.11 à v0.15 (`sealInventory.v01xBuildWindowOpened`) sont
...
dans un lot historique ; pour v0.15 la valeur (`1789295186`) est dérivée par
cette méthode et a été reproduite par l'audit final de v0.15.
