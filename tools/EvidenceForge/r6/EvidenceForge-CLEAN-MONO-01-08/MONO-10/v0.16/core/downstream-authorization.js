...
function resolveDownstreamUseAuthorization(input) {
  ...
  const lin = resolveLineage(input.lineageRefs || (input.report && input.report.lineage), input.artifactRegistry, {
    ...
    // par un contrat redige par l'appelant. §6 (v0.10) : on transmet le
    // VERIFICATEUR et le manifeste, jamais l'emetteur.
    verifier: OTV.isOperatorTrustVerifier(ctx.verifier) ? ctx.verifier : null,
