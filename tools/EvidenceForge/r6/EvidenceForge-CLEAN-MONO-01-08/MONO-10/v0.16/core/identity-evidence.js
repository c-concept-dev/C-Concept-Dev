"use strict";
/**
 * MONO-10 v0.7 — core/identity-evidence.js   (§25, §26)
 *
...
const INDEPENDENCE = {
  ...
  SAME_AUTHORITY_ROOT: "SAME_AUTHORITY_ROOT", SAME_FAMILY_ROOT: "SAME_FAMILY_ROOT",
  SAME_CONTENT: "SAME_CONTENT", UNKNOWN: "INDEPENDENCE_UNKNOWN",
};
...
function independenceBetween(a, b) {
  ...
  // §34 — l'independance EXIGE des racines AUTHENTIFIEES. Une provenance
  // inconnue, non resolue ou ambigue ne corrobore rien.
  if (pa.status !== PROVENANCE_STATUS.AUTHENTICATED || pb.status !== PROVENANCE_STATUS.AUTHENTICATED) return INDEPENDENCE.UNKNOWN;
  if (pa.sourceRootId === pb.sourceRootId) return INDEPENDENCE.SAME_SOURCE_ROOT;
  // §33 — meme contenu reetiquete : une seule source, quels que soient les libelles.
  if (isNonEmptyStr(pa.contentHash) && pa.contentHash === pb.contentHash) return INDEPENDENCE.SAME_CONTENT;
...
const MIN_INDEPENDENT_FOR_STRONG = 2;
const STRONG_THRESHOLD = 1.0;

/**
 * deriveIdentityConfidence(evidences, ctx) — ctx.registry est OBLIGATOIRE.
 *
 * §9 (v0.10) — RESSERREMENT SEULEMENT. Les deux seuils sont des PLANCHERS du
 * lot. Un appelant peut les DURCIR ; il ne peut pas les abaisser. v0.9 lisait
 * `ctx.minIndependentForStrong` et `ctx.strongThreshold` tels quels : les
 * mettre a 0 suffisait a obtenir STRONG sans aucune corroboration
 * independante, donc sans aucune preuve.
 */
function deriveIdentityConfidence(evidences, ctx) {
  ctx = ctx || {};
  const askedIndep = typeof ctx.minIndependentForStrong === "number" && isFinite(ctx.minIndependentForStrong)
    ? ctx.minIndependentForStrong : MIN_INDEPENDENT_FOR_STRONG;
  const askedThreshold = typeof ctx.strongThreshold === "number" && isFinite(ctx.strongThreshold)
    ? ctx.strongThreshold : STRONG_THRESHOLD;
  const minIndep = Math.max(MIN_INDEPENDENT_FOR_STRONG, askedIndep);
  const strongThreshold = Math.max(STRONG_THRESHOLD, askedThreshold);
  /** true si l'appelant n'a fait que RESSERRER (ou n'a rien demande). */
  const policyNarrowedOnly = (askedIndep >= MIN_INDEPENDENT_FOR_STRONG && askedThreshold >= STRONG_THRESHOLD);
  ...
  const none = (c, why) => ({ confidence: c, reasons: [why], independentProviders: 0, independenceRelations: [],
    minIndependentForStrong: minIndep, strongThreshold: strongThreshold, policyNarrowedOnly: policyNarrowedOnly });
  ...
  const reasons = [];
  const withPolicy = (o) => Object.assign(o, { minIndependentForStrong: minIndep,
    strongThreshold: strongThreshold, policyNarrowedOnly: policyNarrowedOnly });
  if (indep.size >= minIndep && anyVerified && total >= strongThreshold) {
    ...
    return { confidence: CONFIDENCE.STRONG, reasons: reasons, independentProviders: indep.size,
      independenceRelations: indep.relations, minIndependentForStrong: minIndep,
      strongThreshold: strongThreshold, policyNarrowedOnly: policyNarrowedOnly };
  }
  ...
  say(INDEPENDENCE.SAME_CONTENT, "au moins deux preuves portent le MEME CONTENU sous des libelles differents");
  ...
  if (confirmed.length >= 1 && anyVerified) { reasons.push("corroboration independante insuffisante pour STRONG"); return withPolicy({ confidence: CONFIDENCE.MODERATE, reasons: reasons, independentProviders: indep.size, independenceRelations: allRel }); }
  if (confirmed.length >= 1 || substantive.some((e) => e.subjectBinding === SUBJECT_BINDING.ASSERTED)) { reasons.push("preuves presentes mais non verifiees"); return withPolicy({ confidence: CONFIDENCE.MODERATE, reasons: reasons, independentProviders: indep.size, independenceRelations: allRel }); }
  reasons.push("aucune liaison au sujet etablie");
  return withPolicy({ confidence: CONFIDENCE.WEAK, reasons: reasons, independentProviders: 0, independenceRelations: allRel });
}
...
module.exports = { makeIdentityEvidence, deriveIdentityConfidence, independenceBetween, independentSet,
  ...
  MIN_INDEPENDENT_FOR_STRONG, STRONG_THRESHOLD };
