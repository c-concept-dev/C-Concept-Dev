"use strict";
/**
 * MONO-10 v0.7 — test/fixture-chain.js
 *
...
function provisionOperator(o) {
  ...
  const cfg = {
    ...
    // §10 — l'exploitant declare une RACINE ; le repertoire est derive du
    // namespace d'autorite, il n'est plus choisi.
    replayProtection: o.replayProtection || { kind: "FILE", replayRoot: replayRoot, authorityScope: [auth.authorityId] },
...
async function buildChain(o) {
   ...
   */
  const provenanceAuthority = verifier.provenanceAuthority();
  function grantProvenance(artifactId) {
    const e = registry.get(artifactId);
    if (!e || !OPA.isProvisionedProvenanceAuthority(provenanceAuthority)) return null;
    const res = provenanceAuthority.resolveRoots({ sourceRootId: e.artifact.sourceRootId,
      locator: e.artifact.locator || null, contentHash: e.artifact.contentHash || null });
    if (res.status !== "AUTHENTICATED") return res.status;
    registry.grantCapability(artifactId, AC.mintCapabilityGrant({ issuer: provenanceAuthority,
      capability: AC.CAPABILITY.AUTHENTICATED_PROVENANCE, artifactId: artifactId, artifactHash: e.hash,
      artifactSchema: e.artifactType, runId: manifest.runId, missionHash: manifest.missionHash,
      issuerId: provenanceAuthority.authorityId, derivationRef: res.derivationRef }));
    return "AUTHENTICATED";
  }
  /** §30 — PRODUCTION_LLM_CAPABILITY est emise par la frontiere de capacite. */
  function grantLlmCapability(artifactId, derivationRef) {
    const e = registry.get(artifactId);
    const lb = boundary.llmCapabilityBoundary;
    if (!e || !lb || lb.namespace !== "PRODUCTION") return null;
    registry.grantCapability(artifactId, AC.mintCapabilityGrant({ issuer: lb,
      capability: AC.CAPABILITY.PRODUCTION_LLM_CAPABILITY, artifactId: artifactId, artifactHash: e.hash,
      artifactSchema: e.artifactType, runId: manifest.runId, missionHash: manifest.missionHash,
      issuerId: lb.llmBoundaryId, derivationRef: derivationRef || "probe:" + e.hash.slice(0, 24) }));
    return true;
  }
  /** §30 — la capacite HUMAN_AUTHENTICATED est emise par le mecanisme d'acte. */
  function grantHumanAuthenticated(artifactId, actProofHashes) {
    const e = registry.get(artifactId);
    const mech = boundary.humanAuthMechanism;
    if (!e || !HAB.isProvisionedHumanAuth(mech)) return null;
    registry.grantCapability(artifactId, AC.mintCapabilityGrant({ issuer: mech,
      capability: AC.CAPABILITY.HUMAN_AUTHENTICATED, artifactId: artifactId, artifactHash: e.hash,
      artifactSchema: e.artifactType, runId: manifest.runId, missionHash: manifest.missionHash,
      issuerId: mech.mechanismId, derivationRef: "human-acts:" + sha256Of(actProofHashes || []) }));
    return true;
  }
  ...
  grantHumanAuthenticated("panel-validation",
    (validation.decisions || []).map((d) => (d.humanActProof && d.humanActProof.proofValue) || d.decisionHash));
  ...
  return {
    ...
    async complete() {
      ...
      grantLlmCapability("llm-capability", capability.requestId ? "probe:" + capability.requestId : null);
