"use strict";
/**
 * MONO-10 v0.7 — core/llm-capability.js  (§19)
 *
...
function assertCapabilityUsable(capability, config, ctx) {
  ...
  // §30 — SINK CRITIQUE « llm-capability:production-use ». Si l'artefact de
  // capacite est enregistre, il doit porter PRODUCTION_LLM_CAPABILITY, emise
  // par la frontiere de capacite de l'exploitant. Une sonde qui a « repondu
  // 200 » n'est pas une capacite de production.
  if (production && ctx.artifactRegistry && typeof ctx.artifactRegistry.get === "function") {
    const artifactId = isNonEmptyStr(ctx.llmCapabilityArtifactId) ? ctx.llmCapabilityArtifactId : "llm-capability";
    const entry = ctx.artifactRegistry.get(artifactId);
    if (entry) {
      const required = AC.requirementOf("llm-capability:production-use");
      if (!AC.hasCapability(entry, required)) {
        problems.push("l'artefact de capacite LLM enregistre ne porte pas \"" + required
          + "\" : la frontiere de capacite de l'exploitant ne l'a pas emise.");
      }
    }
  }
  if (production) {
