"use strict";
/**
 * MONO-10 v0.7 — core/replay-protection.js   (§9, §10, §11, §12)
 *
 ...
 *
 * LIMITE DECLAREE (voir REPLAY-PROTECTION.md §6) : un exploitant qui ecrit une
 * seconde configuration avec une AUTRE `replayRoot` cree une seconde reserve
 * physique. Aucun mecanisme interne au processus ne peut l'en empecher ; c'est
 * la meme hypothese d'environnement d'exploitation que la racine de confiance,
 * et elle est ecrite, pas dissimulee.
 */
...
function deriveReplayNamespaceId(input) {
  ...
  return sha256Of({ namespace: ns, authorities: list });
}
...
function createFileReplayStore(o) {
  o = o || {};
  if (!isNonEmptyStr(o.replayRoot)) {
    throw fail("REPLAY_ROOT_MISSING",
      "une reserve persistante exige une racine provisionnee (replayRoot) : le repertoire est DERIVE du namespace d'autorite, jamais choisi.");
  }
  ...
  const dir = path.join(path.resolve(o.replayRoot), o.replayNamespaceId);
  ...
  return brand({
    ...
    storeId: "file:" + sha256Of({ root: path.resolve(o.replayRoot), ns: o.replayNamespaceId }).slice(0, 16),
...
module.exports = { createMemoryReplayStore, createFileReplayStore, assertStoreContract, isProvisionedStore,
  nonceKey, deriveReplayNamespaceId, assertNamespaceConsistency, __resetProcessNamespaceRegistry, KIND, MARKER };
