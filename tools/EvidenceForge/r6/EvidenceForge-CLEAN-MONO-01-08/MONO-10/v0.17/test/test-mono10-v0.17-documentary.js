...
"use strict";
/**
 * MONO-10 v0.17 — test/test-mono10-v0.17-documentary.js
 *
 ...
 *
 * CE QUE v0.17 AJOUTE (audit final de v0.16 : verite d'origine, statut perime) :
 ...
 *
 * Usage : node test/test-mono10-v0.17-documentary.js [racineDuPaquet]
 */
...
const VERSION_TAG = (path.basename(__filename).match(/v0\.\d+/) || ["v0.17"])[0];   // "v0.17"
...
detector("DOC-05", "aucune mesure de sceau fausse ou perimee n'est AFFIRMEE comme courante, ni cadree avec un chiffre d'un autre cadre",
  ...
  [
    ...
    mut("OPEN-FINDINGS.md R4 : « cadre historique v0.11 … : 24 lots » devient « 25 lots »", "OPEN-FINDINGS.md", function (t) {
      return mustReplace(t, /(cadre historique v0\.11\*\* \(v0\.11 à v0\.17 exclus\) : )24 lots/, "$125 lots", "cadre historique v0.11");
    }),
    mut("AUDIT-VERDICTS.v0.12-v0.17.md (fichier racine a nom pointe) : « 24 lots / 1 577 références » sans cadre", "AUDIT-VERDICTS.v0.12-v0.17.md", function (t) {
...
detector("DOC-13", "chaque commande imprimee avec un « # attendu » reproduit exactement le resultat imprime",
  ...
  [
    mut("NON-REGRESSION.md : le chiffre attendu du cadre v0.11 est altere (24 -> 26)", "NON-REGRESSION.md", function (t) {
      return mustReplace(t, /(MONO-10\/v0\.11,MONO-10\/v0\.12,MONO-10\/v0\.13,MONO-10\/v0\.14,MONO-10\/v0\.15,MONO-10\/v0\.16,MONO-10\/v0\.17\n# attendu : SEALED_HISTORICAL_LOTS_COUNT = )24/, "$126", "attendu cadre v0.11");
    }),
    mut("NON-REGRESSION.md : la commande imprimee du cadre v0.11 n'exclut plus que v0.11 (chiffre inchange)", "NON-REGRESSION.md", function (t) {
      return mustReplace(t, /node tools\/seal-inventory\.js <racine> MONO-10\/v0\.11,MONO-10\/v0\.12,MONO-10\/v0\.13,MONO-10\/v0\.14,MONO-10\/v0\.15,MONO-10\/v0\.16,MONO-10\/v0\.17\n/, "node tools/seal-inventory.js <racine> MONO-10/v0.11\n", "commande cadre v0.11");
    }),
...
detector("DOC-20", "MANIFEST.sealInventory.v017BuildWindowOpened est derive de la methode canonique et re-mesurable",
  function (docs) {
    let man;
    try { man = JSON.parse(docs["MANIFEST.json"] || ""); } catch (e) { return ["MANIFEST.json illisible"]; }
    const hits = [];
    const declared = (man.sealInventory || {}).v017BuildWindowOpened;
    const unix = typeof declared === "number" ? declared : parseInt(String(declared || "").replace(/.*unix\s*/, ""), 10);
    if (!(unix > 0)) return ["v017BuildWindowOpened absent ou illisible : " + declared];
    ...
    else if (ceilEven(Math.min.apply(null, mt)) !== unix) hits.push("v017BuildWindowOpened = " + unix + " ; derive de min(files[].mtime) = " + ceilEven(Math.min.apply(null, mt)));
  ...
  [
    mut("MANIFEST.json : v017BuildWindowOpened est decale d'une seconde", "MANIFEST.json", function (t) {
      const man = JSON.parse(t); man.sealInventory.v017BuildWindowOpened = man.sealInventory.v017BuildWindowOpened + 1; return JSON.stringify(man, null, 2);
    }),
...
detector("DOC-22", "dans un registre multi-versions, aucune ligne ne decrit une correction d'une version posterieure a son heading / sa colonne de statut",
  ...
  [
    mut("AUDIT-REMEDIATION-MATRIX.md §11 : la colonne « Statut en v0.17 » redevient « Statut en v0.15 » au-dessus de lignes corrigees en v0.16 / v0.17", "AUDIT-REMEDIATION-MATRIX.md", function (t) {
      return mustReplace(t, /\| Statut en v0\.17 \|/, "| Statut en v0.15 |", "colonne Statut en v0.17");
    }),
    mut("AUDIT-REMEDIATION-MATRIX.md §11 : le HEADING « v0.15 à v0.17 » redevient « v0.14 et v0.15 », la colonne « Statut en v0.17 » restant juste", "AUDIT-REMEDIATION-MATRIX.md", function (t) {
      return mustReplace(t, /^## 11\. v0\.15 à v0\.17 — /m, "## 11. v0.14 et v0.15 — ", "heading §11");
    }),
...
detector("DOC-23", "le tableau de statut de OPEN-FINDINGS porte, pour chacune de R1..R5, OPEN / NON_BLOCKING / NOT_FIXED_IN_V017",
  function (docs) {
    const of = docs["OPEN-FINDINGS.md"] || "";
  ...
  [mut("OPEN-FINDINGS.md : la ligne R5 passe a CLOSED / FIXED_IN_V016, les quatre autres inchangees", "OPEN-FINDINGS.md", function (t) {
    return mustReplace(t, /(^\|\s*\*\*R5\*\*[^\n]*?)`OPEN`([^\n]*?)`NOT_FIXED_IN_V017`/m, "$1`CLOSED`$2`FIXED_IN_V017`", "ligne R5");
  })]);
...
detector("DOC-24", "chaque document a titre d'origine declare une origine de CONTENU vraie (empreintes, lot nomme, premier lot identique) ; aucun autre H1 n'est perime",
  ...
  [
    ...
    mut("README.md : le H1 redevient « MONO-10 v0.16 »", "README.md", function (t) {
      return mustReplace(t, /^# MONO-10 v0\.17/m, "# MONO-10 v0.16", "H1 README");
    }),
  ]);
...
detector("DOC-27", "MANIFEST.documentHistory est present pour chaque .md du lot et egale l'historique re-mesure sur les lots ; aucun H1 ne porte de parenthese de version ecrite a la main",
  ...
  [
    ...
    mut("TRUST-MODEL.md : le H1 reprend une parenthese « (contenu de v0.12, corrigé en v0.14) »", "TRUST-MODEL.md", function (t) {
      return mustReplace(t, /^# MONO-10 v0\.17 — modèle de confiance/m, "# MONO-10 v0.17 — modèle de confiance (contenu de v0.12, corrigé en v0.14)", "H1 TRUST-MODEL");
    }),
  ]);
...
detector("DOC-19", "aucun ancien token NOT_FIXED_IN_V0NN / resolvedByV0NN n'est presente comme courant : tolere seulement dans la phrase « a l'epoque de v0.NN » (meme NN) ou cite ; jamais suivi de « depuis »",
  function (docs) {
    const cur = VERSION_TAG.replace(/^v0\./, "");   // numero de la version courante
    const hits = [];
    Object.keys(docs).forEach(function (f) {
      blocksOf(f, docs[f]).forEach(function (b) {
        if (b.kind === "code") return;
        sentencesOf(b).forEach(function (sent) {
          ...
          while ((m = re.exec(sent)) !== null) {
            const v = m[1] || m[2];
            if (v === cur) continue;
            /** « à l'époque de v0.NN » doit nommer le MEME NN, dans la MEME phrase */
            /** « à l'époque de v0.MM » avec MM ≠ NN juste avant le token : cadre FAUX, pas une citation */
            const before = sent.slice(Math.max(0, m.index - 60), m.index);
            const ep = before.match(/[aà] l'[eé]poque de v0\.(\d+)\s*:?\s*`?$/i);
            if (ep && ep[1] !== v) { hits.push(f + ":" + b.line + " :: " + m[0] + " cadre « à l'époque de v0." + ep[1] + " » : mauvaise version"); continue; }
            if (isCitedBefore(b, sent, m.index)) continue;                    // « … » ou marqueur de citation adjacent
            const epoque = new RegExp("[aà] l'[eé]poque de v0\\." + v + "\\b", "i").test(sent);
            const depuis = new RegExp(m[0] + "`?\\s*(depuis|désormais|desormais|aujourd)", "i").test(sent);
            if (depuis) { hits.push(f + ":" + b.line + " :: " + m[0] + " suivi de « depuis » : ancien token presente comme courant"); continue; }
            if (epoque) continue;
  ...
  [
    mut("OPEN-FINDINGS.md : un NOT_FIXED_IN_V017 redevient NOT_FIXED_IN_V015", "OPEN-FINDINGS.md", function (t) {
      return mustReplace(t, /NOT_FIXED_IN_V017/, "NOT_FIXED_IN_V015", "NOT_FIXED_IN_V017");
    }),
    ...
    mut("MIGRATION-v0.15-v0.16.md : « à l'époque de v0.16 : NOT_FIXED_IN_V016 » redevient « NOT_FIXED_IN_V016 depuis » (residu B2 de v0.16)", "MIGRATION-v0.15-v0.16.md", function (t) {
      return mustReplace(t, /à l'époque de v0\.16 : `NOT_FIXED_IN_V016`/, "`NOT_FIXED_IN_V016` depuis", "a l'epoque de v0.16");
    }),
    mut("MIGRATION-v0.13-v0.14.md : « à l'époque de v0.14 : NOT_FIXED_IN_V014 » recadre sur la mauvaise version (« à l'époque de v0.15 »)", "MIGRATION-v0.13-v0.14.md", function (t) {
      return mustReplace(t, /à l'époque de v0\.14 : `NOT_FIXED_IN_V014`/, "à l'époque de v0.15 : `NOT_FIXED_IN_V014`", "a l'epoque de v0.14");
    }),
  ]);

const EXTRA_CHECK_IDS = ["COH-01", "COH-02", "COH-03", "COH-04", "COH-05", "COH-06", "RT-01", "RT-02", "RT-03", "RT-04", "RT-05", "RT-06", "RT-07", "RT-08", "RT-09",
  "SEAL-01", "SEAL-02", "SEAL-03", "SEAL-04", "SEAL-05", "SEAL-06", "SEAL-07", "SEAL-08", "SEAL-09", "MAN-01", "MAN-02", "MAN-03", "SCAN-01", "SCAN-02", "SCAN-03", "SCAN-04"];
...
/* ============================ execution ============================ */
(function () {
  console.log("MONO-10 v0.17 — controles de verite documentaire\n");
  ...
  check("SCAN-04. le balayage atteint un fichier documentaire racine a nom pointe (AUDIT-VERDICTS.v0.12-v0.17.md)",
    md.indexOf("AUDIT-VERDICTS.v0.12-v0.17.md") !== -1);
  ...
  DETECTORS.forEach(function (d) {
    ...
    const ok = passClean && allBite && allRevert && d.mutations.length - witnessSkips > 0;
    if (ok) caught++;
    check(d.id + ". " + d.label
      + "\n        PASS_CLEAN=" + (passClean ? "YES" : "NO")
      + "  FAIL_SINGLE_TARGET_FAULT=" + (allBite ? "YES" : "NO") + " (" + (d.mutations.length - witnessSkips) + " temoin" + (d.mutations.length - witnessSkips > 1 ? "s" : "") + (witnessSkips ? ", " + witnessSkips + " SKIP" : "") + ")"
  ...
  const man = manifest || {};
  const of = docs["OPEN-FINDINGS.md"] || "";
  check("COH-01. OPEN-FINDINGS.md : chaque ligne R1..R5 du tableau de statut porte OPEN / NON_BLOCKING / NOT_FIXED_IN_V017 (verification individuelle, DOC-23)",
    ["R1", "R2", "R3", "R4", "R5"].every(function (r) {
      const row = (of.match(new RegExp("^\\|\\s*\\*\\*" + r + "\\*\\*[^\\n]*$", "m")) || [""])[0];
      return /`OPEN`/.test(row) && /`NON_BLOCKING`/.test(row) && /`NOT_FIXED_IN_V017`/.test(row);
    }));
  check("COH-02. aucune reserve n'est presentee comme resolue (MANIFEST.knownOpenFindings)",
    (man.knownOpenFindings || []).length === 5
    && (man.knownOpenFindings || []).every(function (r) { return r.resolvedByV017 === false && r.status === "OPEN" && r.blocking === false; }),
    JSON.stringify((man.knownOpenFindings || []).map(function (r) { return r.id + ":" + r.status + ":" + r.resolvedByV017; })));
  ...
  check("COH-04. MIGRATION v0.16 -> v0.17 existe et affirme l'absence de changement runtime",
    !!docs["MIGRATION-v0.16-v0.17.md"] && /Aucune API ne change/.test(docs["MIGRATION-v0.16-v0.17.md"] || ""));
  check("COH-05. le MANIFEST declare la nature documentaire et les six identites runtime",
    /DOCUMENTAIRE/i.test(man.natureOfLot || "")
    && ["MONO-10-v0.11", "MONO-10-v0.12", "MONO-10-v0.13", "MONO-10-v0.14", "MONO-10-v0.15", "MONO-10-v0.16"].every(function (v) { return (man.runtimeByteIdenticalTo || []).indexOf(v) !== -1; }),
  ...
  const staleKeys = Object.keys(man.measurements || {}).filter(function (k) { return /V01[0-6]\b/.test(k) && !/runtimeByteIdenticalToV01[0-6]/.test(k); });
  check("MAN-03. MANIFEST : reserves R1-R5 OPEN / NON_BLOCKING / NOT_FIXED_IN_V017, closesFindings vide, aucune mesure indexee sur une version anterieure recopiee comme courante",
    (man.closesFindings || []).length === 0 && (man.knownOpenFindings || []).length === 5
    && (man.knownOpenFindings || []).every(function (r) { return r.status === "OPEN" && r.blocking === false && r.resolvedByV017 === false && r.fixedIn === "NOT_FIXED_IN_V017"; })
    && staleKeys.length === 0 && man.correctsFalseStatementsOf === "MONO-10-v0.16",
  ...
  [["v0.11", "RT-01"], ["v0.12", "RT-02"], ["v0.13", "RT-03"], ["v0.14", "RT-06"], ["v0.15", "RT-07"], ["v0.16", "RT-09"]].forEach(function (pair) {
  ...
  check("RT-05. les outils et tests documentaires ajoutes n'ecrivent rien (aucun appel fs d'ecriture)",
    ["tools/seal-inventory.js", "test/test-mono10-v0.17-documentary.js"].every(function (t) {
  ...
  console.log("\n  -- inventaire des sceaux : sept cadres --");
  if (!fs.existsSync(path.join(BUNDLE, "MONO-10"))) {
    skip("SEAL-01..09. inventaire", "paquet non joignable depuis cette extraction");
  } else {
    const SI = require(path.join(LOT, "tools", "seal-inventory.js"));
    const frames = [
      ["v0.11", "SEAL-01", ["MONO-10/v0.11", "MONO-10/v0.12", "MONO-10/v0.13", "MONO-10/v0.14", "MONO-10/v0.15", "MONO-10/v0.16", "MONO-10/v0.17"], [24, 1577, 9, 0]],
      ["v0.12", "SEAL-02", ["MONO-10/v0.12", "MONO-10/v0.13", "MONO-10/v0.14", "MONO-10/v0.15", "MONO-10/v0.16", "MONO-10/v0.17"], [25, 1644, 9, 0]],
      ["v0.13", "SEAL-03", ["MONO-10/v0.13", "MONO-10/v0.14", "MONO-10/v0.15", "MONO-10/v0.16", "MONO-10/v0.17"], [26, 1715, 9, 0]],
      ["v0.14", "SEAL-07", ["MONO-10/v0.14", "MONO-10/v0.15", "MONO-10/v0.16", "MONO-10/v0.17"], [27, 1787, 9, 0]],
      ["v0.15", "SEAL-08", ["MONO-10/v0.15", "MONO-10/v0.16", "MONO-10/v0.17"], [28, 1860, 9, 0]],
      ["v0.16", "SEAL-09", ["MONO-10/v0.16", "MONO-10/v0.17"], [29, 1934, 9, 0]],
    ];
    let f11 = null;
    ...
    const f14 = SI.inventory(BUNDLE, "MONO-10/v0.17");
    console.log("  INFO  cadre v0.17 mesure : " + f14.sealedHistoricalLots + " lots, " + f14.sealedReferences + " references, "
    ...
    check("SEAL-04. cadre v0.17 : memes 9 divergences, 0 lot non scelle, v0.16 devenu historique (30 lots)",
      f14.sealedDivergences === 9 && f14.unverifiableHistoricalLots.length === 0 && f14.sealedHistoricalLots === 30,
    ...
    const nr = docs["NON-REGRESSION.md"] || "";
    check("SEAL-06. les sept cadres sont nommes dans NON-REGRESSION.md, et le MANIFEST porte le cadre v0.17 mesure",
      ["v0.11", "v0.12", "v0.13", "v0.14", "v0.15", "v0.16", "v0.17"].every(function (v) { return new RegExp("cadre v0\\." + v.slice(3)).test(nr); })
      && man.sealInventory && man.sealInventory.frameV017
      && man.sealInventory.frameV017.sealedHistoricalLots === f14.sealedHistoricalLots
      && man.sealInventory.frameV017.sealedReferences === f14.sealedReferences
      ...
      JSON.stringify(man.sealInventory && man.sealInventory.frameV017));
  }
