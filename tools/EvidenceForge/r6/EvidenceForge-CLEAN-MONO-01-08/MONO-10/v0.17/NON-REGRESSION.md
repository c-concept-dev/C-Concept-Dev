# MONO-10 v0.17 — non-régression

## 1. La propriété centrale de ce lot : le runtime est inchangé

v0.17 est un successeur **strictement documentaire** de v0.16, qui l'était de
v0.15, de v0.14, de v0.13, de v0.12 et de v0.11. La seule affirmation forte
qu'il porte est celle-ci, et elle est vérifiable en **soixante-douze
comparaisons** (douze chemins × six lots de référence) :
...
| `MONO10_V17_CAUSED_REGRESSION` | **NO** |
| `MONO10_V17_CHANGED_ANY_API` | **NO** |
| `MONO10_V17_CHANGED_ANY_BEHAVIOUR` | **NO** |
...
| | `test/test-mono10-v0.17-documentary.js` (ajout, lecture seule) |
...
```
cd MONO-10
for prev in v0.11 v0.12 v0.13 v0.14 v0.15 v0.16; do
  for d in core adapters validators schemas contracts; do diff -r $prev/$d v0.17/$d; done
  for f in test/fixture-chain.js test/test-mono10-v0.11.js \
    ...
    diff $prev/$f v0.17/$f; done
done
# attendu : comparaisons = 72 ; VIDES = 72
```

Sortie attendue : **rien**, sur les soixante-douze comparaisons. La ligne
`# attendu` est **rejouée** par `test/test-mono10-v0.17-documentary.js`
...
- `test/test-mono10-v0.17-documentary.js` — contrôles documentaires, réécrits en
...
```
node tools/seal-inventory.js <racine> MONO-10/v0.11,MONO-10/v0.12,MONO-10/v0.13,MONO-10/v0.14,MONO-10/v0.15,MONO-10/v0.16,MONO-10/v0.17
...
l'historique. **Sept cadres, sept jeux de chiffres, tous vrais, tous
rejoués :**

| Cadre | Lots exclus | Lots scellés | Références | Divergences | Non scellés |
|---|---|---|---|---|---|
| **cadre v0.11** | v0.11, v0.12, v0.13, v0.14, v0.15, v0.16, v0.17 | 24 | 1 577 | 9 | 0 |
| **cadre v0.12** | v0.12, v0.13, v0.14, v0.15, v0.16, v0.17 | 25 | 1 644 | 9 | 0 |
| **cadre v0.13** | v0.13, v0.14, v0.15, v0.16, v0.17 | 26 | 1 715 | 9 | 0 |
| **cadre v0.14** | v0.14, v0.15, v0.16, v0.17 | 27 | 1 787 | 9 | 0 |
| **cadre v0.15** | v0.15, v0.16, v0.17 | 28 | 1 860 | 9 | 0 |
| **cadre v0.16** | v0.16, v0.17 | 29 | 1 934 | 9 | 0 |
| **cadre v0.17** | v0.17 | 30 | 2 010 | 9 | 0 |

```
node tools/seal-inventory.js <racine> MONO-10/v0.12,MONO-10/v0.13,MONO-10/v0.14,MONO-10/v0.15,MONO-10/v0.16,MONO-10/v0.17
# attendu : SEALED_HISTORICAL_LOTS_COUNT = 25 ; SEALED_REFERENCES_COUNT = 1644 ; SEALED_DIVERGENCES = 9 ; UNVERIFIABLE_HISTORICAL_LOTS = 0
```

```
node tools/seal-inventory.js <racine> MONO-10/v0.13,MONO-10/v0.14,MONO-10/v0.15,MONO-10/v0.16,MONO-10/v0.17
# attendu : SEALED_HISTORICAL_LOTS_COUNT = 26 ; SEALED_REFERENCES_COUNT = 1715 ; SEALED_DIVERGENCES = 9 ; UNVERIFIABLE_HISTORICAL_LOTS = 0
```

```
node tools/seal-inventory.js <racine> MONO-10/v0.14,MONO-10/v0.15,MONO-10/v0.16,MONO-10/v0.17
# attendu : SEALED_HISTORICAL_LOTS_COUNT = 27 ; SEALED_REFERENCES_COUNT = 1787 ; SEALED_DIVERGENCES = 9 ; UNVERIFIABLE_HISTORICAL_LOTS = 0
```

```
node tools/seal-inventory.js <racine> MONO-10/v0.15,MONO-10/v0.16,MONO-10/v0.17
# attendu : SEALED_HISTORICAL_LOTS_COUNT = 28 ; SEALED_REFERENCES_COUNT = 1860 ; SEALED_DIVERGENCES = 9 ; UNVERIFIABLE_HISTORICAL_LOTS = 0
```

```
node tools/seal-inventory.js <racine> MONO-10/v0.16,MONO-10/v0.17
# attendu : SEALED_HISTORICAL_LOTS_COUNT = 29 ; SEALED_REFERENCES_COUNT = 1934 ; SEALED_DIVERGENCES = 9 ; UNVERIFIABLE_HISTORICAL_LOTS = 0
```

```
node tools/seal-inventory.js <racine> MONO-10/v0.17
...
valeurs mesurées portées par `MANIFEST.sealInventory`. Le cadre v0.17 est
**mesuré après scellement** de v0.17, jamais postulé : la valeur « 0 non
scellé » n'est vraie qu'une fois `SHA256SUMS.txt` écrit.

### Le total, tous sceaux confondus — cadre : l'arbre complet au scellement de v0.17
...
v0.16). Le total ne se recopie pas : il se mesure, dans l'arbre où on le
publie.

```
node tools/seal-inventory.js <racine>
# attendu : ALL_SEALS_FILE_COUNT = 41 ; ALL_SEALS_REFERENCE_COUNT = 3570 ; ALL_SEALS_DIVERGENCES = 9
```

| Population (arbre complet, v0.17 scellé) | Sceaux | Références | Divergences |
|---|---|---|---|
| lots historiques distincts (cadre v0.17) | 30 | 2 010 | 9 |
| copies imbriquées sous `dependencies/` | 10 | 1 483 | 0 |
| lot candidat v0.17 | 1 | 77 | 0 |
| **tous sceaux confondus** | **41** | **3 570** | **9** |
...
à v0.15, ni à v0.16, ni à v0.17.** Leurs dates de modification vont du 31/08/2026 au 03/09/2026, soit neuf à douze
...
Ce n'est **pas** une vérité générale des arbres v0.12 à v0.17 : chaque lot
...
dans aucune des sept fenêtres de construction** (chaque fenêtre est définie en
§7).
...
`MONO10_V15_CAUSED_REGRESSION = NO`, `MONO10_V16_CAUSED_REGRESSION = NO` et
`MONO10_V17_CAUSED_REGRESSION = NO` le sont de la même façon, chacun mesuré
...
- `test/test-mono10-v0.17-documentary.js` — échoue si une fausseté documentaire
...
**dérivée** et **rejouée** ; v0.16 et v0.17 appliquent la même méthode :
...
```
find MONO-10/v0.17 -type f ! -name '*.zip' -exec stat -f %m {} + | sort -n | head -1 | awk '{print $1 + ($1 % 2)}'
# attendu : BUILD_WINDOW_OPENED = 1789313214
```
...
`sealInventory.v017BuildWindowOpened` est le minimum de ces mtimes, arrondi.
...
Les fenêtres de v0.11 à v0.16 (`sealInventory.v01xBuildWindowOpened`) sont
...
dans un lot historique ; pour v0.15 (`1789295186`) et v0.16 (`1789306460`) les
valeurs sont dérivées par cette méthode et ont été reproduites par les audits
finaux de v0.15 et de v0.16.
...
Depuis v0.17, la provenance est **mesurée** par le constructeur sur les seize
lots scellés joignables (v0.1 à v0.16), et re-mesurée par le test :
...
- `MANIFEST.originTitledDocuments[fichier]` pour les neuf documents dont le H1
  ...
  (le lot qui l'a conçu — le H1), `currentContentOriginVersion` (le **premier**
  lot scellé où le contenu courant apparaît à l'identique), `originPath`,
...
| Document | H1 (origine conceptuelle) | Origine de contenu (premier lot identique) |
|---|---|---|
| `CONTRACT-GUARD.md`, `HUMAN-ACT-AUTHENTICATION.md`, `KEY-MANAGEMENT.md`, `RUN-ORDER.md`, `UPSTREAM-EVIDENCE-BINDING.md` | v0.7 | v0.7 |
| `HISTORICAL-INPUT-AUTHORITY.md`, `LINEAGE.md` | v0.7 — **ancêtre conceptuel** : le document a été conçu en v0.7 | **v0.10** — le contenu courant est celui de v0.10, byte-identique de v0.10 à v0.17 ; v0.7 n'est pas l'origine du contenu |
| `ARCHITECTURE.md`, `CONTRACT-MAPPING.md` | v0.11 | v0.11 |

Les deux documents à origine de contenu v0.10 ne sont **pas réécrits** par
v0.17 : les retitrer aurait fait de v0.17 l'origine de leur contenu, et
effacé la mesure. Leur H1 reste « v0.7 » ; ce que « v0.7 » veut dire est
