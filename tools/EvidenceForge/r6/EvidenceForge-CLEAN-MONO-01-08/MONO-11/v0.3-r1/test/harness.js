...
async function mountRun(missionKey, opts) {
  ...
  return { mission, F, M10, op, boundary, verifier, manifest, ctx, registry, runId, missionHash, bind, R, discovery, verification, boundDiscovery, assessment, assessmentRef,
    ...
    async runPanel(extra) {
      return require("../core/autonomous-run.js").runAutonomousPanel(Object.assign({
    ...
    async runDownstream(res, extra) {
      return require("../core/autonomous-run.js").runDownstream(Object.assign({ frozen: F, ledger, panel: res.panel, gateInputs: res.gateInputs, corpusSetAll: res.corpusSetAll,
        ...
        targetDocuments: [{ targetId: "target-01", label: "document cible synthetique", role: "review_target_not_evidence", content: opts.targetContent || "Passage un du document cible synthetique. Passage deux, egalement synthetique." }],
        onValidation: opts.onValidation,
...
/** LLM simule pour l'aval : couverture EF-02D3, revues EF-03B, classification EF-03C, et REPRISES INFORMEES v0.2 — cite des references EXACTES. */
function makeDownstreamFakeLlm(opts) {
  opts = opts || {};
  const calls = [];
  const fn = async function (prompt) {
    calls.push(prompt);
    if (prompt.indexOf("REPRISE (passe") === 0) {
      /* reprise informee : le prompt porte tout ce qu'il faut pour repondre juste */
      if (prompt.indexOf("WORKS_USED") !== -1) {
        const dims = /dans l'ordre ([^\]]+) \]\}/.exec(prompt)[1].split(",").map((x) => x.trim());
        const works = JSON.parse(/WORKS_USED \(workRef exacts\) : (\[[^\n]*\])/.exec(prompt)[1]);
        const doc = prompt.slice(prompt.indexOf("DOCUMENT CIBLE (targetId="), prompt.indexOf("\n\nWORKS_USED"));
        const passage = (/\) :\n([^\n.]+\.)/.exec(doc) || [null, null])[1];
        return JSON.stringify({ findings: dims.map((id) => ({ dimensionId: id, disposition: "concern", epistemicStatus: works.length ? "documented" : "not_determinable", finding: "constat repare", rationale: "reprise",
          targetEvidenceRefs: opts.reviewTamper === "invented_ref_forever" ? ["passage invente"] : (passage ? [passage] : []), twinBasisWorkRefs: works.slice(0, 1), confidenceQualitative: "low", limitations: ["fixture"] })) });
      }
      const refs = JSON.parse(/REFERENCES ADMISSIBLES[^:]*: (\[[^\n]*\])/.exec(prompt)[1]);
      const dims = /ids dans l'ordre ([^\]]+) \]/.exec(prompt)[1].split(",").map((x) => x.trim());
      const ref = /"professionalRef":"([^"]+)"/.exec(prompt);
      return JSON.stringify({ professionalRef: ref ? ref[1] : null, dimensions: dims.map((id, i) => ({ id, level: i === 0 ? "strong" : "moderate", evidenceWorks: refs.slice(0, 1), rationale: "reprise", contradictionWithMissing: false })), overallNote: "reprise" });
    }
    if (prompt.indexOf("EF-02D3") !== -1) {
      ...
      const topic = works[0] && works[0].topics && works[0].topics[0];
      const ev = opts.coverageTamper === "topic_as_work" && topic && calls.filter((c) => c.indexOf("EF-02D3") !== -1).length === 1 ? [topic] : works.slice(0, 1).map((w) => w.title);
      return JSON.stringify({ professionalRef: m[1], dimensions: dims.map((id, i) => ({ id, level: i === 0 ? "strong" : "moderate", evidenceWorks: ev, rationale: "fixture", contradictionWithMissing: false })), overallNote: "fixture" });
    }
    if (prompt.indexOf("EF-03B") !== -1) {
      ...
      const docBlock = prompt.slice(prompt.indexOf("DOCUMENT CIBLE RÉEL"), prompt.indexOf("\n\nBASE DOCUMENTAIRE"));
      const passage = (/\)\n([^\n.]+\.)/.exec(docBlock) || [null, "Passage un du document cible synthetique."])[1];
      const nReview = calls.filter((c) => c.indexOf("EF-03B") !== -1).length;
      let findings = dims.map((id) => ({ dimensionId: id, disposition: "concern", epistemicStatus: worksUsed.length ? "documented" : "not_determinable", finding: "constat fixture", rationale: "fixture",
        targetEvidenceRefs: [passage], twinBasisWorkRefs: worksUsed.slice(0, 1).map((w) => w.workRef), confidenceQualitative: "low", limitations: ["fixture"] }));
      if (opts.reviewTamper === "duplicate_dimension" && nReview === 1) findings = findings.concat([Object.assign({}, findings[0])]);
      if (opts.reviewTamper === "ascii_apostrophe" && nReview === 1) findings[0].targetEvidenceRefs = [passage.replace(/\u2019/g, "'")];
      if (opts.reviewTamper === "extra_key" && nReview === 1) findings[0].extra = "x";
      if (opts.reviewTamper === "bad_disposition" && nReview === 1) findings[0].disposition = "cautious_inference";
      if (opts.reviewTamper === "invented_ref_forever") findings[0].targetEvidenceRefs = ["passage invente"];
      return JSON.stringify({ findings: findings });
    }
