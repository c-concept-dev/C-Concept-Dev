...
async function runEnforcedReview(input) {
  ...
  for (let p = 1; p <= maxPasses; p++) {
    ...
    if (p === 1) { strategy = STRATEGY.BASE; prompt = basePrompt; }
      ...
      prompt = targetedRepairPrompt({ targetDoc: doc, faultyDimensions: dims, rejectedRefs: rejected, fragments: fragments, pass: p, strategyChange: change, exactRepeat: exactRepeatSeen || repeatedFaultyRef, forbiddenRefs: forbiddenRefs, context: ctx });
      targeted = { dims: dims, rejected: rejected, fragments: fragments, repeatedFaultyRef: repeatedFaultyRef, context: ctx };
    } else {
