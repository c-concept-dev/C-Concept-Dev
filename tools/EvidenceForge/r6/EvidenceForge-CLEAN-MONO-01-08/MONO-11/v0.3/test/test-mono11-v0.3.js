...
test("T9 (v0.3) validateur inchange : le meme corpus de citations invalides est refuse par validateReviewCandidate v0.2 et v0.3 (byte-identite de la fonction) ; TARGET_REF_NOT_LITERAL reste TARGET_REF_NOT_LITERAL", async () => {
  const c = await reviewContext(); const v02 = require(path.join(__dirname, "..", "..", "v0.2", "core", "review-enforcer.js"));
  assert.equal(v02.validateReviewCandidate.toString(), RE.validateReviewCandidate.toString(), "fonction de validation byte-identique");
  ...
  corpus.forEach((ref) => { const f = c.validFindings(); f[0].targetEvidenceRefs = [ref]; const t = JSON.stringify({ findings: f }); const a = v02.validateReviewCandidate(t, c.twin, c.doc, c.schema), b = RE.validateReviewCandidate(t, c.twin, c.doc, c.schema); assert.equal(a.ok, false); assert.equal(b.ok, false); assert.deepEqual(a.errors, b.errors); assert.ok(b.errors.some((e) => e.code === "TARGET_REF_NOT_LITERAL")); });
  const ok = JSON.stringify({ findings: c.validFindings() }); assert.equal(v02.validateReviewCandidate(ok, c.twin, c.doc, c.schema).ok, true); assert.equal(RE.validateReviewCandidate(ok, c.twin, c.doc, c.schema).ok, true);
});
...
test("T12 (v0.3) erreurs autres que TARGET_REF_NOT_LITERAL : comportement v0.2 conserve (reprise informee, meme prompt informedRepairPrompt) ; mixte (TARGET_REF + autre) : v0.2 aussi", async () => {
  const c = await reviewContext(); const v02 = require(path.join(__dirname, "..", "..", "v0.2", "core", "review-enforcer.js")); assert.equal(v02.informedRepairPrompt.toString(), RE.informedRepairPrompt.toString());
