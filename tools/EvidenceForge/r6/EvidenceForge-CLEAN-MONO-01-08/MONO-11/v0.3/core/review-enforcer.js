...
function rejectedRefsOf(errors, parsed) {
  /* citations rejetees, relues dans la reponse parsee (le detail d'erreur est tronque a 160 caracteres) */
  const out = {}; const byDim = {}; arr(parsed && parsed.findings).forEach((f) => { if (f && isStr(f.dimensionId)) byDim[f.dimensionId] = f; });
  faultyDimensions(errors).forEach((d) => { out[d] = arr(byDim[d] && byDim[d].targetEvidenceRefs).filter(isStr); });
  return out;
}
...
function targetedRepairPrompt(input) {
  ...
  dims.forEach(function (d) {
    lines.push("- dimension " + d + " — champ targetEvidenceRefs — citation(s) rejetee(s) : " + JSON.stringify(rejected[d] || []));
...
async function runEnforcedReview(input) {
  ...
  for (let p = 1; p <= maxPasses; p++) {
    ...
    if (p === 1) { strategy = STRATEGY.BASE; prompt = basePrompt; }
      ...
      const dims = faultyDimensions(errors); const rejected = rejectedRefsOf(errors, parsed);
      ...
      prompt = targetedRepairPrompt({ targetDoc: doc, faultyDimensions: dims, rejectedRefs: rejected, fragments: fragments, pass: p, strategyChange: change, exactRepeat: exactRepeatSeen || repeatedFaultyRef, forbiddenRefs: forbiddenRefs });
      targeted = { dims: dims, rejected: rejected, fragments: fragments, repeatedFaultyRef: repeatedFaultyRef };
    } else {
    ...
    if (targeted) {
      ...
      if (rp.ok) { const rc = recompose(parsed, rp.repairs); candidateText = JSON.stringify(rc); v = validateReviewCandidate(candidateText, twin, doc, schema); }
    ...
    const rec = { pass: p, informed: p > 1, strategy: strategy, promptSha256: sha(prompt), responseSha256: sha(candidateText), rawResponseSha256: rawSha, previousResponseSha256: prevRaw === null ? null : sha(prevRaw),
      ...
      repair: repairInfo, repeatedFaultyRef: targeted ? targeted.repeatedFaultyRef === true : false, callId: r && (r.callId || r.providerRequestId || r.localRequestId) || null, valid: v.ok, errors: v.errors.slice(0, 20), errorCodes: Array.from(new Set(v.errors.map((e) => e.code))).concat(repairInfo && !repairInfo.parsed ? repairInfo.errors.map((e) => e.code) : []) };
...
async function buildEnforcedReviewSet(input) {
  ...
  return {
    reviewSet: { schema: "EvidenceForge.DocumentaryReviewSet", schemaVersion: "EF-03B-v1", missionId: twinSet.missionId || null, reviewSchemaHash: schema.schemaHash,
      ...
      producedBy: "MONO-11 v0.3 review-enforcer : prompt EF-03B gele + validation locale a schema ferme + reprise informee / reparation ciblee TARGET_REF_NOT_LITERAL + anti-repetition + validateur EF-03B gele", testMode: true },
    traces: traces,
  };
}
