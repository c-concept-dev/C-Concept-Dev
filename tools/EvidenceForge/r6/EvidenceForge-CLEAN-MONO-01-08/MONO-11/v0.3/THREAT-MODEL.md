...
| run exécuté sur code non scellé (F1) | `assertSealedRuntime` avant toute action : `RUN_ON_UNSEALED_CODE` ; sceau/code/manifeste/zip épinglés dans l'état du run et l'ancre du ledger | T37, T41 |
| réutilisation d'une réponse refusée (ou jamais validée) présentée comme valide | politique par réponse : REUSE_VALID seulement ; INVALID/UNKNOWN ⇒ appel réel ; un reuse ne compte jamais comme REAL_LLM_CALL | T38 |
| tolérance glissée dans `reviews_complete` | égalité stricte `accepted == expected` ; mutants 99/90/80 % détectés | T39, T40 |
| citation altérée par normalisation (changement de sens) | règles fermées, sans lettre, idempotentes ; original conservé et haché ; `verifyNormalizationRecord` | T32, T33 |
