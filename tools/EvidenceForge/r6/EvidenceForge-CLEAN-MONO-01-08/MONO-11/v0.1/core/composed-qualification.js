"use strict";
/**
 * MONO-11 v0.1 — core/composed-qualification.js   (audit §8 : readiness / qualification composees)
 *
...
function evaluateReadiness(input) {
  ...
  if (phase === "FULL") {
    ...
    const reviews = arr(input.reviewSet && input.reviewSet.reviews);
    const complete = reviews.filter((r) => r.reviewStatus === "complete").length;
    d("reviews", reviews.length && complete === reviews.length ? DIM.SATISFIED : DIM.UNSATISFIED, [complete + "/" + reviews.length + " revue(s) complete(s)"]);
...
function qualifyProcess(input) {
  ...
  const reviews = arr(input.reviewSet.reviews);
  crit("reviews_present", reviews.length > 0, reviews.length + " revue(s)");
  crit("reviews_complete", reviews.length > 0 && reviews.every((r) => r.reviewStatus === "complete"), reviews.filter((r) => r.reviewStatus === "complete").length + "/" + reviews.length);
...
module.exports = { evaluateReadiness, qualifyProcess, admitted, DIM, READINESS, PRE, FULL_ONLY };
