"use strict";
/**
 * MONO-11 v0.1 — core/autonomous-run.js
 *
  ...
  for (const a of arr(input.assessment.assessments)) {
    ...
    if (corpusEvidence.status === "SUFFICIENT") {
      relevanceEvidence = await SRO.evaluateRelevanceEvidence({ frozen: F, corpus: corpus, attributedWorkRefs: corpusEvidence.attribution.attributedWorkRefs,
        ...
        opts: input.oracleOpts /* ex. { maxWorks } : nombre d'oeuvres soumises a EF-02D2 (defaut gele : 10, dans l'ordre du corpus) */ });
...
async function runDownstream(input) {
  ...
  const coverageMatrix = await F.M01.D3.buildCoverageMatrix(usable, corpusByRef, input.dimensionSet, input.missionQuestion, llm);
  ...
  const eligibilityRelevanceSet = { schema: "EvidenceForge.DocumentaryEligibilityRelevanceSet", schemaVersion: "EF-02D-v2", missionId: input.missionId, missionQuestion: input.missionQuestion,
    ...
    records: admittedIds.map(function (id) {
      ...
      return { schema: "EvidenceForge.DocumentaryEligibilityRelevance", schemaVersion: "EF-02D-v2", professionalRef: id,
        eligibility: g && g.corpusEvidence.eligibility ? g.corpusEvidence.eligibility : { status: "eligible_documentary", source: "human-override" },
        dimensionRelevance: (relByRef.get(id) && relByRef.get(id).judgments) || [] };
    }) };
  ...
  const targetDocumentSet = await F.M01.TDS.buildTargetDocumentSet(input.missionId, arr(input.targetDocuments));
  ...
  let reviewSet = await F.M01.RR.buildDocumentaryReviewSet({ reviewSchema: reviewSchema, twinSet: twinSet, targetDocumentSet: targetDocumentSet, workerCallFn: llm });
  /* Reprise CIBLEE et BORNEE (capacite generique gelee d'EF-03B) : seules les revues en erreur — sorties refusees
     par la validation anti-fabrication — sont rejouees, une fois par passe ; les revues valides sont conservees
     octet pour octet. Le nombre de passes est un parametre d'exploitation (defaut 1) ; chaque passe est consignee.
     Une revue encore en erreur apres les passes RESTE en erreur : rien n'est force, rien n'est masque. */
  const passes = Number.isInteger(input.reviewResumePasses) ? Math.max(0, input.reviewResumePasses) : 1;
  const resumeTrace = [];
  for (let i = 0; i < passes && reviewSet.summary.reviewsError > 0; i++) {
    reviewSet = await F.M01.RR.resumeDocumentaryReviewSet(reviewSet, { reviewSchema: reviewSchema, twinSet: twinSet, targetDocumentSet: targetDocumentSet, workerCallFn: llm });
    resumeTrace.push(Object.assign({ pass: i + 1 }, reviewSet.resumeSummary));
    log({ event: "reviews_resume_pass", pass: i + 1, summary: reviewSet.resumeSummary });
  }
  reviewSet = Object.assign({}, reviewSet, { resumePasses: resumeTrace });
  ...
  return { panel: panel, corpusSet: corpusSet, coverageMatrix: coverageBound, panelSelection: panelSelection, twinSet: twinsBound, reviewSchema: reviewSchema,
    targetDocumentSet: targetDocumentSet, reviewSet: reviewsBound, aggregation: aggBound, llmCalls: calls };
}
