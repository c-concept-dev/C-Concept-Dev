...
"use strict";
/**
 * MONO-11 v0.4 — benchmark/replay.js (v0.4 : rejectedRefsOf avec document, prompt cible avec conservees, correction historique fusionnee) : REJEU DETERMINISTE HORS LIGNE des sequences de revues reelles (fixtures anonymisees,
...
function run() {
  ...
  fixtures.forEach(function (fx) {
    ...
    fx.passes.forEach(function (p, i) {
      ...
      const rejected = v.parsed ? RE.rejectedRefsOf(v.errors, v.parsed, doc.content) : {}; const frags = {};   /* v0.4 : non litterales seulement */
    ...
    if (acc && parsed1) { const pv = RE.validateReviewCandidate(acc.responseText, twin, doc, schema); const dims = RE.faultyDimensions(RE.validateReviewCandidate(fx.passes[0].responseText, twin, doc, schema).errors);
      if (pv.ok && pv.parsed && dims.length) { const repairs = {}; pv.parsed.findings.forEach((f) => { if (dims.indexOf(f.dimensionId) !== -1) repairs[f.dimensionId] = f.targetEvidenceRefs; }); const mg = RE.mergeRepairLineage(parsed1, repairs, doc.content); const rc = RE.recompose(parsed1, mg.applied); const rv = RE.validateReviewCandidate(JSON.stringify(rc), twin, doc, schema); seq.historicalCorrectionAsTargetedRepair = { faultyDimensions: dims, accepted: rv.ok, errors: rv.errors.map((e) => e.code), lineage: mg.dimensions, unresolvedDimensions: mg.unresolvedDimensions };   /* v0.4 : fusion de lignee avant recomposition */ if (rv.ok) summary.historicalCorrectionAcceptedByRecomposition++; } }
    /* prompt cible que v0.3 aurait construit apres la passe 1 (deterministe) */
    const e1 = seq.passes[0]; if (e1 && e1.targetedEligible) { const v1 = RE.validateReviewCandidate(fx.passes[0].responseText, twin, doc, schema); const dims = RE.faultyDimensions(v1.errors); const part = RE.partitionRefsOf(v1.errors, v1.parsed, doc.content); const rejected = part.rejected; const fragments = {}; dims.forEach((d) => { fragments[d] = rejected[d].map((ref) => Object.assign({ ref }, RE.literalFragments(ref, doc.content))); });
      const ctx = RE.repairContext({ twin, targetDoc: doc, parsed: v1.parsed, faultyDimensions: dims });   /* v0.3-r1 : empreinte de contexte portee par le prompt */
      const prompt = RE.targetedRepairPrompt({ targetDoc: doc, faultyDimensions: dims, rejectedRefs: rejected, preservedRefs: part.preserved, fragments, pass: 2, strategyChange: false, context: ctx }); seq.targetedPromptPass2 = { sha256: sha(prompt), chars: prompt.length, mentionsFragments: /NE SONT PAS CONTIGUS/.test(prompt), mentionsEmptyFallback: /rendez \[\]/.test(prompt), containsDocument: prompt.indexOf(doc.content) !== -1, reinjectsFullPreviousResponse: prompt.indexOf(fx.passes[0].responseText) !== -1, repairContextFingerprint: ctx.fingerprint, carriesFingerprint: prompt.indexOf(ctx.fingerprint) !== -1 }; }
    rows.push(seq);
  });
  /* v0.3-r1 : deux sequences eligibles ne partagent jamais un prompt cible (donc jamais une cle de reuse) */
  const prompts = rows.filter((r) => r.targetedPromptPass2).map((r) => r.targetedPromptPass2.sha256); const fps = rows.filter((r) => r.targetedPromptPass2).map((r) => r.targetedPromptPass2.repairContextFingerprint);
  summary.targetedPromptsBuilt = prompts.length; summary.targetedPromptCollisions = prompts.length - new Set(prompts).size; summary.distinctRepairContexts = new Set(fps).size;
  return { summary, rows };
}
