"use strict";
/**
 * MONO-11 v0.4 — test/test-mono11-v0.4.js (= 69 tests v0.3-r1 conserves, 3 assertions adaptees au contrat v3, + TR-1..TR-14 v0.4)
 ...
 * v0.4 — TARGETED REPAIR LINEAGE PRESERVATION : TR-1..TR-14 (LLM factice scripte, 0 fournisseur), voir fin de fichier.
 */
...
test("T2 (v0.3) citation non litterale simple : passe 1 refusee, passe 2 CIBLEE (schema {repairs}), correction acceptee ; validateur gele EF-03B accepte", async () => {
  ...
  const p2 = s.calls[1].prompt; assert.ok(p2.indexOf("REPRISE CIBLEE (passe 2)") === 0 && p2.indexOf("dimension " + c.dims[1] + " — champ targetEvidenceRefs") !== -1 && p2.indexOf("citation(s) rejetee(s) (REFS_TO_REPAIR) : [\"passage reformule qui n'existe pas\"]") !== -1 && /PRESERVED_VALID_REFS/.test(p2));   /* v0.4 */
...
test("T4 (v0.4, ex-v0.3) aucune citation valide et reparation [] : REFS_PRESENT_BUT_UNRESOLVED — jamais un [] silencieux accepte ; les refs invalides restent, la politique bornee continue puis fail-closed (contrat v3, CAS B)", async () => {
  ...
  const r = await RE.runEnforcedReview({ frozen: c.F, twin: c.twin, targetDoc: c.doc, reviewSchema: c.schema, llmCall: s.llmCall, maxPasses: 3 });
  assert.equal(r.review.reviewStatus, "error"); assert.equal(r.trace.acceptedPass, null); const p2 = r.trace.passes[1]; assert.deepEqual(p2.repairUnresolvedDimensions, [c.dims[2]]); assert.equal(p2.repair.lineage[c.dims[2]].state, RE.REPAIR_STATE.UNRESOLVED); assert.equal(p2.valid, false); assert.ok(p2.errorCodes.includes(RE.TARGET_REF_CODE));
});
...
test("r1-T10 aucune regression : validateur, preambule, reprise informee, extractJson byte-identiques a la reference v0.2 ; maxPasses 3 ; strategies v0.3 inchangees ; contrat MONO-11-v3 (v0.4) ; fail-closed apres 3 passes", async () => {
  ...
  const contracts = require("../contracts/mono11-contracts.json"); assert.equal(contracts.contractVersion, "MONO-11-v3"); assert.equal(contracts.retryPolicy.version, "MONO-11-v0.4"); assert.ok(contracts.lineageMonotonicityDuringRepair && contracts.lineageMonotonicityDuringRepair.properties.P1);   /* v0.4 */
  const c = await twoTwinContext(); const t = reusingTransport((prompt, meta) => meta.pass === 1 ? JSON.stringify({ findings: c.findingsFor(c.A, "CONSTAT-A") }) : JSON.stringify({ repairs: [{ dimensionId: c.dims[0], targetEvidenceRefs: [SPLICE] }] }));
...
/* ===================== v0.4 — TARGETED REPAIR LINEAGE PRESERVATION (contrat MONO-11-v3 : LINEAGE_MONOTONICITY_DURING_REPAIR) ===================== */
/* Fixtures synthetiques : VALID_A / VALID_B / VALID_C sont des sous-chaines exactes de DOC3 ; INVALID_B / INVALID_C n'en sont pas. Aucun jeton de cas. */
const VALID_A = "passage exact a citer", VALID_B = "autre passage citable", VALID_C = "Seconde phrase, distincte", INVALID_B = "passage reformule qui n'existe pas", INVALID_C = "encore une citation absente";
const refsOf = (r, id) => r.review.findings.find((f) => f.dimensionId === id).targetEvidenceRefs;
const withRefs = (c, byDim) => { const f = c.validFindings(); f.forEach((x) => { if (byDim[x.dimensionId]) x.targetEvidenceRefs = byDim[x.dimensionId]; }); return f; };
const repairResp = (repairs) => JSON.stringify({ repairs: Object.keys(repairs).map((d) => ({ dimensionId: d, targetEvidenceRefs: repairs[d] })) });
const run4 = (c, s, max) => RE.runEnforcedReview({ frozen: c.F, twin: c.twin, targetDoc: c.doc, reviewSchema: c.schema, llmCall: s.llmCall, maxPasses: max || 3 });
const sameExceptRefs = (before, after) => before.every((b) => { const a = after.find((x) => x.dimensionId === b.dimensionId); return a && Object.keys(b).every((k) => k === "targetEvidenceRefs" || JSON.stringify(b[k]) === JSON.stringify(a[k])); });

test("TR-1 (v0.4) original valide + original invalide + repair valide -> [validA, validB] : originale conservee, reparee ajoutee apres, acceptee passe 2", async () => {
  const c = await reviewContext(); const d = c.dims[1]; const raw = withRefs(c, { [d]: [VALID_A, INVALID_B] });
  const s = scripted([JSON.stringify({ findings: raw }), repairResp({ [d]: [VALID_B] })]); const r = await run4(c, s);
  assert.equal(r.review.reviewStatus, "complete"); assert.equal(r.trace.acceptedPass, 2); assert.deepEqual(refsOf(r, d), [VALID_A, VALID_B]);
  const lg = r.trace.passes[1].repair.lineage[d]; assert.equal(lg.state, RE.REPAIR_STATE.RESOLVED); assert.deepEqual(lg.preservedRefs, [VALID_A]); assert.deepEqual(lg.rejectedRefs, [INVALID_B]); assert.deepEqual(lg.validRepairedRefs, [VALID_B]); assert.equal(lg.repairOutcome, "REPAIRED");
  assert.ok(sameExceptRefs(raw, r.review.findings));
});
test("TR-2 (v0.4) toutes originales valides -> aucune reparation ciblee, refs byte-identiques ; fonction pure : repair [] => aucune perte", async () => {
  const c = await reviewContext(); const d = c.dims[1]; const raw = withRefs(c, { [d]: [VALID_A, VALID_B] });
  const s = scripted([JSON.stringify({ findings: raw })]); const r = await run4(c, s); assert.equal(r.trace.acceptedPass, 1); assert.equal(s.calls.length, 1); assert.deepEqual(refsOf(r, d), [VALID_A, VALID_B]);
  const mg = RE.mergeRepairLineage({ findings: raw }, { [d]: [] }, DOC3); assert.deepEqual(mg.repairs[d], [VALID_A, VALID_B]); assert.equal(mg.dimensions[d].state, RE.REPAIR_STATE.RESOLVED); assert.deepEqual(mg.unresolvedDimensions, []);
});
test("TR-3 (v0.4) repair omet volontairement une originale valide -> l'originale valide est preservee par le CODE (le modele ne la reemet pas) ; elle n'est ni rejetee ni interdite, elle est listee CONSERVEE dans le prompt", async () => {
  const c = await reviewContext(); const d = c.dims[0]; const raw = withRefs(c, { [d]: [VALID_A, INVALID_B] });
  const s = scripted([JSON.stringify({ findings: raw }), repairResp({ [d]: [VALID_B] })]); const r = await run4(c, s);
  assert.equal(r.review.reviewStatus, "complete"); assert.deepEqual(refsOf(r, d), [VALID_A, VALID_B]);
  const p2 = s.calls[1].prompt; assert.ok(p2.indexOf("citation(s) rejetee(s) (REFS_TO_REPAIR) : " + JSON.stringify([INVALID_B])) !== -1, "seule l'invalide est rejetee"); assert.ok(p2.indexOf("PRESERVED_VALID_REFS, deja litterales, acquises : elles restent quoi que vous rendiez, ne les reecrivez pas) : " + JSON.stringify([VALID_A])) !== -1, "la valide est listee conservee");
  assert.deepEqual(r.trace.passes[1].preservedRefs[d], [VALID_A]); assert.equal(r.trace.passes[1].proposedFragments[0].refs.length, 1, "un seul fragment propose : celui de l'invalide");
});
test("TR-4 (v0.4) repair renvoie doublon -> dedup stable, premiere occurrence gagnante ; doublon du brut lui-meme dedoublonne", async () => {
  const c = await reviewContext(); const d = c.dims[1]; const raw = withRefs(c, { [d]: [VALID_A, INVALID_B] });
  const s = scripted([JSON.stringify({ findings: raw }), repairResp({ [d]: [VALID_A, VALID_B, VALID_B, VALID_A] })]); const r = await run4(c, s);
  assert.equal(r.review.reviewStatus, "complete"); assert.deepEqual(refsOf(r, d), [VALID_A, VALID_B]);
  assert.deepEqual(RE.mergeRepairLineage({ findings: withRefs(c, { [d]: [VALID_A, VALID_A, INVALID_B] }) }, { [d]: [VALID_A] }, DOC3).repairs[d], [VALID_A]);
});
test("TR-5 (v0.4) repair renvoie une ref non litterale -> rejetee sans supprimer les originales valides ; tracee droppedRepairedRefs ; jamais rendue valide", async () => {
  const c = await reviewContext(); const d = c.dims[1]; const raw = withRefs(c, { [d]: [VALID_A, INVALID_B] });
  const s = scripted([JSON.stringify({ findings: raw }), repairResp({ [d]: ["", INVALID_C] })]); const r = await run4(c, s);
  assert.equal(r.review.reviewStatus, "complete"); assert.equal(r.trace.acceptedPass, 2); assert.deepEqual(refsOf(r, d), [VALID_A]);
  const lg = r.trace.passes[1].repair.lineage[d]; assert.deepEqual(lg.droppedRepairedRefs, ["", INVALID_C]); assert.deepEqual(lg.unresolvedInvalidRefs, [INVALID_B]); assert.equal(lg.repairOutcome, "INVALID_DROPPED_NO_REPLACEMENT"); assert.equal(lg.state, RE.REPAIR_STATE.RESOLVED);
});
test("TR-6 (v0.4) multi-dimension : seule la dimension ciblee est potentiellement modifiee sur ses refs ; les autres dimensions sont byte-identiques (refs comprises) ; targetedDimensions = [dim]", async () => {
  const c = await reviewContext(); const [d0, d1, d2] = c.dims; const raw = withRefs(c, { [d0]: [VALID_A, VALID_C], [d1]: [VALID_A, INVALID_B], [d2]: [VALID_C] });
  const s = scripted([JSON.stringify({ findings: raw }), repairResp({ [d1]: [VALID_B] })]); const r = await run4(c, s);
  assert.equal(r.review.reviewStatus, "complete"); assert.deepEqual(r.trace.passes[1].targetedDimensions, [d1]);
  assert.deepEqual(refsOf(r, d0), [VALID_A, VALID_C]); assert.deepEqual(refsOf(r, d2), [VALID_C]); assert.deepEqual(refsOf(r, d1), [VALID_A, VALID_B]);
  const mg = RE.mergeRepairLineage({ findings: raw }, { [d1]: [VALID_B] }, DOC3); assert.deepEqual(Object.keys(mg.repairs), [d1]);
});
test("TR-7 (v0.4) non-regression semantique : tous les champs hors targetEvidenceRefs byte-identiques (disposition, epistemicStatus, finding, rationale, confidenceQualitative, limitations, twinBasisWorkRefs) sur 3 dimensions reparees ; empreinte de contexte inchangee par la fusion", async () => {
  const c = await reviewContext(); const [d0, d1, d2] = c.dims; const raw = withRefs(c, { [d0]: [VALID_C, INVALID_B], [d1]: [VALID_A, INVALID_B], [d2]: [INVALID_C] });
  const s = scripted([JSON.stringify({ findings: raw }), repairResp({ [d0]: [VALID_B], [d1]: [VALID_B], [d2]: [VALID_B] })]); const r = await run4(c, s);
  assert.equal(r.review.reviewStatus, "complete"); assert.ok(sameExceptRefs(raw, r.review.findings));
  raw.forEach((b) => { const a = r.review.findings.find((x) => x.dimensionId === b.dimensionId); Object.keys(b).forEach((k) => { if (k !== "targetEvidenceRefs") assert.deepEqual(a[k], b[k], b.dimensionId + "." + k); }); });
  assert.deepEqual(refsOf(r, d0), [VALID_C, VALID_B]); assert.deepEqual(refsOf(r, d1), [VALID_A, VALID_B]); assert.deepEqual(refsOf(r, d2), [VALID_B]);
  const ctx = RE.repairContext({ twin: c.twin, targetDoc: c.doc, parsed: { findings: raw }, faultyDimensions: [d0, d1, d2] }); assert.equal(r.trace.passes[1].repairContext.fingerprint, ctx.fingerprint, "l'empreinte ne depend pas des refs (constat hors targetEvidenceRefs)");
});
test("TR-8 (v0.4) fixture reelle anonymisee (reparation ciblee, tentative 10) : 2 originales litterales + 1 non litterale ; resultat final identique quel que soit le comportement du modele (rend [], ou les 3, ou les 2 valides, ou le seul remplacement) — jamais dependant de la reemission des valides", async () => {
  const fx = JSON.parse(fs.readFileSync(path.join(__dirname, "fixtures", "tr8-targeted-repair-case.json"), "utf8")); assert.equal(sha256(fx.content), fx.contentSha256, "fixture intacte");
  const { h, r: r0 } = await full("domainA"); const { makeDownstreamFakeLlm } = require("./harness.js"); const llm0 = makeDownstreamFakeLlm({}); const dn = await h.runDownstream(r0, { downstreamLlm: llm0, llmCall: llm0, targetContent: fx.content });
  const twin = dn.twinSet.twins[0]; const doc = { targetId: "target-01", role: "review_target_not_evidence", content: fx.content }; const schema = dn.reviewSchema; const dims = schema.dimensions.map((d) => d.id); const work = twin.documentaryBasis.worksUsed[0] ? twin.documentaryBasis.worksUsed[0].workRef : null;
  const findings = dims.map((id, i) => ({ dimensionId: id, disposition: "concern", epistemicStatus: work ? "documented" : "not_determinable", finding: "constat " + id, rationale: "r", targetEvidenceRefs: i === 0 ? fx.raw.slice() : [fx.validOriginal[0]], twinBasisWorkRefs: work ? [work] : [], confidenceQualitative: "low", limitations: [] }));
  const valid = fx.validOriginal; assert.equal(valid.length, 2); assert.ok(valid.every((x) => fx.content.indexOf(x) !== -1) && fx.content.indexOf(fx.invalidOriginal) === -1, "fixture : 2 litterales, 1 non litterale");
  const variants = { "modele rend []": [], "modele reemet les 3 (dont l'invalide)": fx.raw.slice(), "modele rend les 2 valides (historique)": valid.slice(), "modele rend le seul remplacement": [fx.replacement] };
  for (const label of Object.keys(variants)) {
    const s = scripted([JSON.stringify({ findings }), repairResp({ [dims[0]]: variants[label] })]);
    const r = await RE.runEnforcedReview({ frozen: h.F, twin, targetDoc: doc, reviewSchema: schema, llmCall: s.llmCall, maxPasses: 3 });
    assert.equal(r.review.reviewStatus, "complete", label); assert.equal(r.trace.acceptedPass, 2, label);
    const expected = label === "modele rend le seul remplacement" ? valid.concat([fx.replacement]) : valid.slice(); assert.deepEqual(refsOf(r, dims[0]), expected, label);
    const p2 = s.calls[1].prompt; assert.ok(p2.indexOf("(REFS_TO_REPAIR) : " + JSON.stringify([fx.invalidOriginal])) !== -1 && p2.indexOf("ne les reecrivez pas) : " + JSON.stringify(valid)) !== -1, label + " : prompt = 1 rejetee, 2 conservees");
  }
});
test("TR-9 (v0.4) sondes de non-regression (audit v1.0.11 P4/P5) : runEnforcedReview n'accepte JAMAIS un candidat ayant perdu une ref litterale valide ; une reparation [] ou sous-ensemble ne supprime rien", async () => {
  const c = await reviewContext(); const d = c.dims[0]; const raw = withRefs(c, { [d]: [VALID_A, VALID_C, INVALID_B] });
  for (const rep of [[], [VALID_C], [VALID_B]]) { const s = scripted([JSON.stringify({ findings: raw }), repairResp({ [d]: rep })]); const r = await run4(c, s); assert.equal(r.review.reviewStatus, "complete", JSON.stringify(rep)); const out = refsOf(r, d); assert.ok(out.indexOf(VALID_A) !== -1 && out.indexOf(VALID_C) !== -1, "aucune perte : " + JSON.stringify(out)); assert.deepEqual(out.slice(0, 2), [VALID_A, VALID_C], "ordre brut conserve"); }
});
test("TR-10 (v0.4) rejeu R1..R6 / S1..S5 : decomptes identiques a v0.3-r1 (12 occurrences, 12 collages, 5 repetitions, 5 eligibles, 0 fausse acceptation, 0 regression, 2 corrections acceptees) ; corrections historiques fusionnees = aucune dimension non resolue, aucune originale valide perdue", () => {
  const rr = REPLAY.run(); const s = rr.summary; assert.equal(s.occurrencesTargetRef, 12); assert.equal(s.splices, 12); assert.equal(s.exactRepeatsDetected, 5); assert.equal(s.targetedEligibleSequences, 5); assert.equal(s.falseAcceptances, 0); assert.equal(s.regressions, 0); assert.equal(s.historicalCorrectionAcceptedByRecomposition, 2);
  rr.rows.filter((x) => x.historicalCorrectionAsTargetedRepair).forEach((x) => { const h = x.historicalCorrectionAsTargetedRepair; assert.deepEqual(h.unresolvedDimensions, [], x.id); Object.keys(h.lineage).forEach((d) => { const lg = h.lineage[d]; lg.preservedRefs.forEach((ref) => assert.ok(lg.finalRefs.indexOf(ref) !== -1, x.id + "/" + d)); }); });
  rr.rows.forEach((row) => { if (row.targetedPromptPass2) assert.ok(row.targetedPromptPass2.carriesFingerprint, row.id); });
});
test("TR-11 (v0.4) legitimate empty : RAW_REFS = [] -> [] conserve, aucune reparation, aucune citation inventee (CAS A)", async () => {
  const c = await reviewContext(); const d = c.dims[2]; const raw = withRefs(c, { [d]: [] });
  const s = scripted([JSON.stringify({ findings: raw })]); const r = await run4(c, s); assert.equal(r.review.reviewStatus, "complete"); assert.equal(r.trace.acceptedPass, 1); assert.equal(s.calls.length, 1); assert.deepEqual(refsOf(r, d), []);
  const mg = RE.mergeRepairLineage({ findings: raw }, { [d]: [] }, DOC3); assert.equal(mg.dimensions[d].state, RE.REPAIR_STATE.NO_REFS_EXPECTED); assert.deepEqual(mg.repairs[d], []);
});
test("TR-12 (v0.4) unresolved invalid refs : RAW_REFS = [invalidA], repair [] -> aucune citation inventee, etat REFS_PRESENT_BUT_UNRESOLVED, [] jamais traite comme succes ; refs invalides conservees dans le candidat ; passe 3 (changement de strategie) puis fail-closed", async () => {
  const c = await reviewContext(); const d = c.dims[1]; const raw = withRefs(c, { [d]: [INVALID_B] });
  const s = scripted([JSON.stringify({ findings: raw }), repairResp({ [d]: [] }), repairResp({ [d]: [] })]); const r = await run4(c, s);
  assert.equal(r.review.reviewStatus, "error"); assert.equal(r.trace.acceptedPass, null); assert.equal(s.calls.length, 3);
  const p2 = r.trace.passes[1]; assert.deepEqual(p2.repairUnresolvedDimensions, [d]); assert.equal(p2.repair.lineage[d].state, RE.REPAIR_STATE.UNRESOLVED); assert.deepEqual(p2.repair.lineage[d].finalRefs, []); assert.deepEqual(p2.repair.lineage[d].unresolvedInvalidRefs, [INVALID_B]); assert.equal(p2.valid, false); assert.ok(p2.errorCodes.includes(RE.TARGET_REF_CODE));
  assert.equal(r.trace.passes[2].strategy, RE.STRATEGY.TARGETED_CHANGE, "la passe necessaire n'est pas supprimee par un [] syntaxiquement acceptable");
  const p3 = s.calls[2].prompt; assert.ok(p3.indexOf("(REFS_TO_REPAIR) : " + JSON.stringify([INVALID_B])) !== -1 && p3.indexOf("ne les reecrivez pas) : []") !== -1, "passe 3 : l'invalide reste a reparer, aucune conservee");
  /* variante : la passe 3 trouve un remplacement -> RESOLVED, accepte */
  const s2 = scripted([JSON.stringify({ findings: raw }), repairResp({ [d]: [] }), repairResp({ [d]: [VALID_B] })]); const r2 = await run4(c, s2); assert.equal(r2.review.reviewStatus, "complete"); assert.equal(r2.trace.acceptedPass, 3); assert.deepEqual(refsOf(r2, d), [VALID_B]);
});
test("TR-13 (v0.4) mixed preserved + failed repair : RAW_REFS = [validA, invalidB], repair [] -> FINAL = [validA] ; validA preservee ; invalidB tracee comme reparation non resolue ; aucune suppression de validA", async () => {
  const c = await reviewContext(); const d = c.dims[1]; const raw = withRefs(c, { [d]: [VALID_A, INVALID_B] });
  const s = scripted([JSON.stringify({ findings: raw }), repairResp({ [d]: [] })]); const r = await run4(c, s);
  assert.equal(r.review.reviewStatus, "complete"); assert.equal(r.trace.acceptedPass, 2); assert.deepEqual(refsOf(r, d), [VALID_A]);
  const lg = r.trace.passes[1].repair.lineage[d]; assert.equal(lg.state, RE.REPAIR_STATE.RESOLVED); assert.deepEqual(lg.preservedRefs, [VALID_A]); assert.deepEqual(lg.unresolvedInvalidRefs, [INVALID_B]); assert.equal(lg.repairOutcome, "INVALID_DROPPED_NO_REPLACEMENT"); assert.deepEqual(r.trace.passes[1].repairUnresolvedDimensions, []);
});
test("TR-14 (v0.4) model deliberately omits valid original : le LLM de test rend uniquement un remplacement ; validA demeure dans FINAL_REFS ; a la passe 3, la ref a reparer n'est jamais une valide et une valide n'est jamais interdite", async () => {
  const c = await reviewContext(); const d = c.dims[0]; const raw = withRefs(c, { [d]: [VALID_A, INVALID_B] });
  const s = scripted([JSON.stringify({ findings: raw }), repairResp({ [d]: [VALID_B] })]); const r = await run4(c, s); assert.deepEqual(refsOf(r, d), [VALID_A, VALID_B]);
  /* reparation non litterale puis remplacement valide : validA conservee des la passe 2 (RESOLVED par ses conservees), jamais rejetee */
  const s3 = scripted([JSON.stringify({ findings: raw }), repairResp({ [d]: [INVALID_C] })]); const r3 = await run4(c, s3);
  assert.equal(r3.review.reviewStatus, "complete"); assert.equal(r3.trace.acceptedPass, 2); assert.deepEqual(refsOf(r3, d), [VALID_A]);
  /* sans conservee : passe 2 non litterale -> non resolue ; passe 3 : REFS_TO_REPAIR = derniere invalide, INTERDIT ne porte que sur des invalides */
  const s4 = scripted([JSON.stringify({ findings: withRefs(c, { [d]: [INVALID_B] }) }), repairResp({ [d]: [INVALID_C] }), repairResp({ [d]: [VALID_B] })]); const r4 = await run4(c, s4);
  assert.equal(r4.review.reviewStatus, "complete"); assert.equal(r4.trace.acceptedPass, 3); const p3 = s4.calls[2].prompt; assert.ok(/INTERDIT de reutiliser textuellement/.test(p3));
  assert.ok(p3.indexOf("(REFS_TO_REPAIR) : " + JSON.stringify([INVALID_B])) !== -1, "passe 3 : la dimension non resolue conserve ses refs brutes (INVALID_B) — la tentative non litterale du modele n'est pas substituee au brut"); assert.deepEqual(r4.trace.passes[1].repair.lineage[d].droppedRepairedRefs, [INVALID_C]); assert.equal(r4.trace.passes[2].repeatedFaultyRef, true, "la citation invalide resoumise est signalee ; INVALID_C ecartee compte comme deja rejetee"); assert.ok(p3.indexOf(JSON.stringify([VALID_A])) === -1);
});
