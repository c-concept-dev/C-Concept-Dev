...
const t = cp.spawnSync("node", ["--test", "test/test-mono11-v0.4.js"], { cwd: LOT, encoding: "utf8", env: process.env });
...
const m = {
  lot: "MONO-11", version: "v0.4", name: "Autonomous Panel Successor — TARGETED REPAIR LINEAGE PRESERVATION (contrat MONO-11-v3 : LINEAGE_MONOTONICITY_DURING_REPAIR) ; successeur de v0.3-r1", predecessor: { version: "v0.3-r1", zipSha256: "3c44b397dda0997d0fff6ab4c8e8a8b311430e6a5ac7aeee38149f8349296b0b", intact: true, note: "v0.3-r1 conserve intact comme historique (dossier et zip canonique)", lineage: { version: "v0.3", zipSha256: "bec558412832293221261ab2f481afc2e3a2c8d7e0bae43769837400af81758d", lineage: { version: "v0.2", zipSha256: "5c208cbbd13231e732fe20475347f9f7cbe6f890f3ce4823581093c7eba60005" } } }, builtAt: new Date().toISOString(),
  status: "IMPLEMENTE / TESTE — GELABLE propose par le chantier (v0.4), NON GELE (gel = decision proprietaire apres audit independant) ; NON INTEGRE au monolithe (integration = MONOLITH-v1.0.12, apres gel)",
  ...
  contracts: "contracts/mono11-contracts.json", contractVersion: require("../contracts/mono11-contracts.json").contractVersion, contractVersionPolicy: "MONO-11-v3 (decision proprietaire 2026-09-22, Q3) : v2 garantissait forme + litteralite des references presentes + [] admissible ; v3 ajoute LINEAGE_MONOTONICITY_DURING_REPAIR (contracts.lineageMonotonicityDuringRepair : P1..P6, etats de reparation NO_REFS_EXPECTED / RESOLVED / REFS_PRESENT_BUT_UNRESOLVED) ; consommateurs a aligner en MONOLITH-v1.0.12 (VALIDATION_CONTRACT, registre)",
  auditFindingsAddressed: ["v0.4 : audit independant MONOLITH-v1.0.11 § 4 (LINEAGE_PRESERVATION_GAP du chemin de reparation ciblee : rejectedRefsOf rendait toutes les refs, forbiddenRefs et INTERDIT passe 3 visaient des refs valides, recompose remplacait le tableau, aucun validateur ne detectait la perte ; run reel t.10 : conservation par le modele seul) -> partitionRefsOf / mergeRepairLineage / prompt PRESERVED_VALID_REFS vs REFS_TO_REPAIR ; Q6 proprietaire : [] jamais un succes silencieux (REFS_PRESENT_BUT_UNRESOLVED)", "v0.3-r1 : audit independant v0.3 P2-07 (reuse inter-jumeaux d'une reparation ciblee : reproduit) -> empreinte de contexte deterministe portee par le prompt cible ; P2-02 (suite non auto-suffisante) -> reference d'empreintes v0.2 embarquee ; P2-01/P2-04 (provenance des mesures, decomptes, patch) -> corriges", "v0.3 : retry cible TARGET_REF_NOT_LITERAL (benchmark H1/H1-v1.0.1/P0.1 : 12 occurrences, 5 repetitions exactes) -> reparation ciblee + fragments deterministes + anti-repetition ; validateur/contrat/100 % inchanges", "3 revues rejetees (2 FORMAT_NORMALIZATION_ONLY, 1 SCHEMA_DRIFT) -> normalisation a l'ingestion + enforcement local + reprise informee", "F1 run sur code pre-sceau -> run-seal-guard (RUN_ON_UNSEALED_CODE) + sceau epingle dans l'ancre du ledger", "F2 no-replay par prompt -> politique de reuse par reponse avec validationStatus", "F3 worksSubmitted -> worksAttributed + worksSubmittedToOracle", "F6 preuves non persistees -> ledger.exportArtifacts", "F7/M9 regle 100 % faiblement epinglee -> T39/T40 (N=100, mutants 99/90/80 %)", "F10 eligibilite fabriquee sur override -> retiree", "F13 topics cites comme oeuvres -> coverage-enforcer"],
  debts: [
