  ...
  const p = (async function () {
    ...
    try {
      ...
      if (!isDone("QUALIFICATION")) {
        ...
        /* v1.0.15 (R1, R2) — IDENTITE CIBLE <-> DOCUMENT : le pipeline ne fabrique plus de table d'autorite et ne fournit plus de lambda.
           Il remet a l'adaptateur EXACTEMENT le meme jeu de documents cibles (`targetDocs`) que celui remis au lot gele par
           SP.runDownstreamFromCheckpoint ci-dessous ; l'adaptateur en derive lui-meme la carte canonique des documents NORMALISES
           (lib/target-identity.js, module gele MONO-11 core/target-normalizer.js) et refuse toute autorite qui n'est pas le document
           normalise de la cible demandee. Les identifiants de cible (target-NN) ont desormais UNE SEULE derivation (TI.assignTargetIds),
           partagee avec lib/stage-professionals.js et lib/stage-report.js : le decalage d'indice et l'autorite d'une autre cible ne sont
           plus exprimables. Le contenu BRUT n'est jamais une autorite de litteralite. */
        ...
        const ef03b = adapter.stats(); r.summary.ef03b = ef03b; store.saveJson("ef03b-resilience.json", Object.assign({ schema: "EvidenceForge.EF03BResilienceSummary", schemaVersion: "MONOLITH-v1.0.16", attemptId: state.attempts }, ef03b));   /* v1.0.10 : reuse registre / troncatures / completions / litteralisations / cout cumule des revues */
