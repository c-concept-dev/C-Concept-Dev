 ...
 * v1.0.15 — AUTHORITY IDENTITY HARDENING (R1, R2) : l'autorite documentaire n'est plus FOURNIE a l'adaptateur par une lambda opaque, elle est
 *   DERIVEE par lui de la carte canonique des documents cibles (lib/target-identity.js ; normalisation par le module GELE du lot). Trois gardes
 *   d'IDENTITE, toutes fail closed : (a) l'autorite servie pour une cible est IDENTIQUE au document normalise canonique de CETTE cible
 *   (DOCUMENT_AUTHORITY_IDENTITY_MISMATCH) ; (b) l'identifiant de cible lu dans le PROMPT gele est celui de la passe en cours
 *   (DOCUMENT_AUTHORITY_TARGET_MISMATCH) ; (c) le contexte reconstruit depuis le prompt gele est un PREFIXE de l'autorite
 *   (DOCUMENT_AUTHORITY_CONTEXT_DIVERGENCE) — invariant structurel du prompt EF-03B, verification INDEPENDANTE de la carte. La garde de point
 *   fixe de v1.0.14 est CONSERVEE en assertion secondaire : elle prouve « normalise », jamais « le bon document ». Registre lie a la cible :
 *   une entree produite pour target-01 n'est jamais servie pour target-02.
...
const TI = require("./target-identity.js");   /* v1.0.15 (R1) : SOURCE UNIQUE de l'identite cible <-> document et de la carte canonique des documents normalises */   /* v1.0.14 : module GELE de normalisation du lot — seule reference pour prouver qu'une autorite est bien le contenu normalise */   /* fonctions PURES du lot gele, utilisees telles quelles (jamais modifiees) */
...
function createReviewRegistry(opts) {
  ...
  function find(basePromptSha256, sealHash, contract, documentAuthoritySha256, targetId) { const all = read().filter((e) => e.basePromptSha256 === basePromptSha256 && e.validationContract === contract && (e.sourceSealHash || null) === (sealHash || null) && e.status === "VALID"
    && (documentAuthoritySha256 ? e.documentAuthoritySha256 === documentAuthoritySha256 : true)
    && (targetId ? e.targetId === targetId : true)); return all.length ? all[all.length - 1] : null; }   /* v1.0.15 (R1) : liaison a la cible — une entree produite pour une autre cible (ou sans cible, ere <= v1.0.14) n'est jamais servie quand la cible est exigee */
  function put(entry) { fs.mkdirSync(opts.runDir, { recursive: true }); fs.appendFileSync(file, JSON.stringify(Object.assign({ schema: "EvidenceForge.ValidReviewRecord", schemaVersion: "MONOLITH-v1.0.16", status: "VALID", recordedAt: new Date().toISOString() }, entry)) + "\n"); return entry; }
...
function createReviewAdapter(opts) {
  ...
  /* v1.0.15 (R1, R2) — AUTORITE DOCUMENTAIRE DERIVEE ET LIEE A LA CIBLE. L'adaptateur construit lui-meme la carte canonique des documents
     NORMALISES a partir des documents cibles du run (opts.targetDocuments, les MEMES objets que ceux remis au lot gele par le pipeline),
     avec le module GELE MONO-11. Le contenu BRUT n'est jamais une autorite ; une reconstruction du prompt non plus. `opts.documentAuthority`
     reste accepte (simulation, outils) mais n'est plus une source : ce qu'il rend est CONFRONTE a la carte canonique et rejete s'il en differe. */
  const canonical = Array.isArray(opts.targetDocuments) && opts.targetDocuments.length ? TI.buildCanonicalAuthorityMap(opts.targetDocuments) : null;
  const overrideOf = (function (src) {
  ...
  if (!canonical && !overrideOf) throw Object.assign(new Error("DOCUMENT_AUTHORITY_REQUIRED: l'adaptateur EF-03B exige les documents cibles du run (targetDocuments) pour deriver l'autorite documentaire NORMALISEE ; la litteralite ne peut etre jugee ni sur le brut ni sur une reconstruction du prompt"), { code: "DOCUMENT_AUTHORITY_REQUIRED", fatal: true });
  const authErr = (code, message, extra) => Object.assign(new Error(code + ": " + message), Object.assign({ code: code, fatal: true }, extra || {}));
  /**
   * requireAuthority(targetId, ctx) -> contenu NORMALISE d'autorite de CETTE cible.
   * Ordre des gardes (toutes fail closed, aucune correction silencieuse) :
   *   1. cible connue de la carte canonique (DOCUMENT_AUTHORITY_UNKNOWN_TARGET) ;
   *   2. contenu present et non vide (DOCUMENT_AUTHORITY_REQUIRED / _EMPTY) ;
   *   3. point fixe des regles gelees — assertion SECONDAIRE, conservee de v1.0.14 (DOCUMENT_AUTHORITY_NOT_NORMALIZED) ;
   *   4. IDENTITE au document canonique de la cible (DOCUMENT_AUTHORITY_IDENTITY_MISMATCH) — c'est la garde de v1.0.15 : un autre document,
   *      un document tronque ou l'autorite d'une autre cible est un point fixe et etait ACCEPTE en v1.0.14 ;
   *   5. identifiant de cible du prompt gele == cible de la passe (DOCUMENT_AUTHORITY_TARGET_MISMATCH) ;
   *   6. contexte reconstruit depuis le prompt gele = PREFIXE de l'autorite (DOCUMENT_AUTHORITY_CONTEXT_DIVERGENCE) : verification
   *      INDEPENDANTE de la carte, fondee sur le prompt construit par le lot a partir du document reellement examine.
   */
  function requireAuthority(targetId, ctx) {
    const canon = canonical ? canonical.byTarget[targetId] : null;
    if (canonical && !canon) throw authErr("DOCUMENT_AUTHORITY_UNKNOWN_TARGET", "la cible " + JSON.stringify(targetId) + " n'appartient pas au jeu de cibles du run (" + canonical.targetIds.join(", ") + ")", { targetId: targetId || null, knownTargetIds: canonical.targetIds });
    let c = null;
    if (overrideOf) { try { c = overrideOf(targetId); } catch (e) { c = null; }
      if (c && typeof c === "object" && typeof c.content === "string") { if (isStr(c.targetId) && c.targetId !== targetId) throw authErr("DOCUMENT_AUTHORITY_TARGET_MISMATCH", "autorite de la cible " + JSON.stringify(c.targetId) + " servie pour la cible " + JSON.stringify(targetId), { targetId: targetId || null, authorityTargetId: c.targetId }); c = c.content; } }
    else c = canon.content;
    if (typeof c !== "string") throw authErr("DOCUMENT_AUTHORITY_REQUIRED", "aucun contenu documentaire d'autorite pour la cible " + JSON.stringify(targetId), { targetId: targetId || null });
    if (!c.length) throw authErr("DOCUMENT_AUTHORITY_EMPTY", "contenu documentaire d'autorite vide pour la cible " + JSON.stringify(targetId), { targetId: targetId || null });
    /* PARITE DE NORMALISATION (v1.0.14, conservee) : condition NECESSAIRE, jamais suffisante — un document tronque ou un autre document la satisfont. */
    const again = TN.normalizeText(c).normalized;
    if (again !== c) throw authErr("DOCUMENT_AUTHORITY_NOT_NORMALIZED", "l'autorite documentaire de la cible " + JSON.stringify(targetId) + " n'est pas le contenu normalise examine par MONO-11 (regles " + TN.RULE_SET_ID + ") — brut ou reconstruction refuses", { targetId: targetId || null, ruleSetId: TN.RULE_SET_ID, authoritySha256: sha(c), normalizedSha256: sha(again) });
    /* IDENTITE (v1.0.15, R2) : le point fixe ne prouve que « normalise » ; seule l'egalite au document canonique de la cible prouve « le bon document ». */
    if (canon && c !== canon.content) throw authErr("DOCUMENT_AUTHORITY_IDENTITY_MISMATCH", "l'autorite servie pour la cible " + JSON.stringify(targetId) + " n'est pas le document normalise canonique de cette cible (attendu " + canon.sha256.slice(0, 12) + "…, " + canon.content.length + " caracteres ; recu " + sha(c).slice(0, 12) + "…, " + c.length + " caracteres)", { targetId: targetId || null, expectedAuthoritySha256: canon.sha256, receivedAuthoritySha256: sha(c), expectedLength: canon.content.length, receivedLength: c.length, ruleSetId: TN.RULE_SET_ID });
    if (ctx) {
      if (isStr(ctx.targetId) && ctx.targetId !== targetId) throw authErr("DOCUMENT_AUTHORITY_TARGET_MISMATCH", "le prompt gele porte la cible " + JSON.stringify(ctx.targetId) + " alors que la passe declare " + JSON.stringify(targetId), { targetId: targetId || null, promptTargetId: ctx.targetId });
      /* INVARIANT STRUCTUREL du prompt EF-03B : le contexte reconstruit est le document examine, tronque au premier marqueur de section —
         donc TOUJOURS un prefixe du document. Une autorite qui ne le prolonge pas n'est pas le document examine par le lot. */
      if (typeof ctx.content === "string" && ctx.content.length && c.indexOf(ctx.content) !== 0) { stats.authorityContextDivergences++; throw authErr("DOCUMENT_AUTHORITY_CONTEXT_DIVERGENCE", "le contexte reconstruit depuis le prompt gele de la cible " + JSON.stringify(targetId) + " n'est pas un prefixe de l'autorite documentaire : l'autorite n'est pas le document examine par MONO-11", { targetId: targetId || null, promptParsedContentSha256: sha(ctx.content), promptParsedContentLength: ctx.content.length, documentAuthoritySha256: sha(c), documentAuthorityLength: c.length }); }
    }
    stats.authorityIdentityVerified++;
    return c;
  }
  ...
  const reviews = {}; const byCallId = {}; const order = []; const stats = { reviewsSeen: 0, registryReuses: 0, truncations: 0, completions: 0, literalizations: 0, realCalls: 0, cumulativeReviewCostUsd: 0, validated: 0, contextUnparsed: 0, contextContentMismatch: 0, registryRefusedForeignAuthority: 0, registryRefusedForeignTarget: 0, authorityIdentityVerified: 0, authorityContextDivergences: 0 };   /* v1.0.15 (R1) : verifications d'identite reussies, divergences de contexte, refus de registre par cible etrangere */
  ...
  function withAuthority(st, ctx, meta) {
    const authority = requireAuthority(meta && meta.targetId, ctx);   /* v1.0.15 : le contexte du prompt participe aux gardes d'identite (cible + prefixe) */
  ...
  /* v1.0.16 (PB-B7) : tout appel de transport EF-03B porte l'identite COURANTE de la revue — cible de la passe et empreinte de l'autorite
     CANONIQUE normalisee (st.authoritySha256, etablie par requireAuthority ; jamais l'empreinte du contenu relu dans le prompt). Le magasin
     generique (lib/llm.js checkEf03bReuseIdentity) n'accepte une reponse que si son entree porte exactement cette identite : un refus
     d'identite du registre ne peut plus etre neutralise par le magasin (meme contrainte des deux cotes). */
  async function call(st, prompt, meta, extraMeta) { const m = Object.assign({}, meta, { maxTokens: cfg.maxTokens }, extraMeta || {}, { targetId: st.targetId, documentAuthoritySha256: st.authoritySha256 || null }); const r = await llm.llmCall(prompt, m); if (!r.reused) stats.realCalls++; return r; }
  ...
  async function reviewCall(prompt, meta) {
    ...
    if (isBase) { st.basePromptSha256 = sha(prompt); st.ctx = withAuthority(st, parseReviewPromptContext(prompt), meta); if (!st.ctx) stats.contextUnparsed++;   /* v1.0.14 : forme du prompt, CONTENU = autorite normalisee (fail closed si absente / non normalisee) */
      ...
      const hit = registry.find(st.basePromptSha256, opts.sealHash || null, CONTRACT, st.authoritySha256, st.targetId);   /* v1.0.14 : meme autorite documentaire normalisee exigee ; v1.0.15 (R1) : ET meme cible */
      if (!hit) { const foreign = registry.find(st.basePromptSha256, opts.sealHash || null, CONTRACT); if (foreign) { if ((foreign.targetId || null) !== st.targetId) stats.registryRefusedForeignTarget++; stats.registryRefusedForeignAuthority++; trace({ twinId: st.twinId, targetId: st.targetId, reviewIndex: st.index, event: "ef03b_pass", strategy: "REGISTRY_REFUSED", state: (foreign.targetId || null) !== st.targetId ? "FOREIGN_TARGET" : "FOREIGN_DOCUMENT_AUTHORITY", entryTargetId: foreign.targetId || null, entryAuthoritySha256: foreign.documentAuthoritySha256 || null, documentAuthoritySha256: st.authoritySha256 }); } }   /* v1.0.14 : entree d'une autre autorite (ou sans empreinte) : jamais servie, refus trace */
    ...
    if (isTargeted || !st.ctx) {   /* reparation ciblee gelee (petite sortie) ou contexte illisible : transport seul */
      ...
      const r = await call(st, prompt, meta); if (isTargeted && st.ctx && st.parsed) { const faulty = RE.faultyDimensions(st.lastErrors || []); const rp = RE.parseRepair(r.text, faulty.length ? faulty : Object.keys(st.parsed.findings.reduce((o, f) => { o[f.dimensionId] = 1; return o; }, {})));   /* v1.0.14 (R-A1) : memes dimensions fautives que le lot gele */
    ...
    const dims = st.ctx.dims; const sent = cfg.enableOutputBudget ? prompt + outputBudgetPreamble(dims, cfg.budget) : prompt; const r = await call(st, sent, meta); let cand = r.text, callId = r.callId, composite = false; let v = mirrorParsed(st, cand); const errorsBefore = v.errors.length;
    ...
    if (r.stopReason === "max_tokens") { stats.truncations++; if (typeof llm.onValidation === "function") llm.onValidation({ callId: r.callId, valid: false, errors: [REVIEW_OUTPUT_TRUNCATED], stage: "EF-03B-adapter" });   /* la reponse brute tronquee n'est jamais reutilisable */
      ...
      if (cfg.enableCompletion && sv.complete.length && sv.missingDims.length && sv.missingDims.length <= cfg.maxCompletionDims) {
        const rc = await call(st, buildCompletionPrompt(prompt, st.ctx, sv.missingDims, sv.complete.map((f) => f.dimensionId), cfg.budget), meta, { purpose: "EF-03B review completion", strategy: STRATEGY.COMPLETION, repairScope: sv.missingDims }); stats.completions++;
    ...
    if (cfg.enableLiteralization && v && !v.ok && RE.onlyTargetRefErrors(v.errors) && v.parsed) { const faulty = RE.faultyDimensions(v.errors); const rejected = RE.rejectedRefsOf(v.errors, v.parsed);
      ...
      if (proposals.some((p) => p.refs.some((x) => x.exactSpan))) { const rctx = RE.repairContext({ twin: { twinId: meta.twinId, professionalRef: meta.professionalRef || null }, targetDoc: docLike(st.ctx), parsed: v.parsed, faultyDimensions: faulty });
        const rl = await call(st, buildLiteralizationPrompt(st.ctx, proposals, { excerptChars: cfg.excerptChars, context: rctx }), meta, { purpose: "EF-03B review literalization", strategy: STRATEGY.LITERALIZATION, repairScope: faulty, repairContextFingerprint: rctx.fingerprint }); stats.literalizations++;
  ...
  /* v1.0.15 (R1) — lecture seule de la carte canonique : ce que les gardes d'identite comparent, expose pour l'observabilite et les tests
     (jamais une source : requireAuthority relit la carte a chaque passe et refuse tout contenu qui n'en est pas l'egal). */
  const authorityFor = (targetId) => (canonical && canonical.byTarget[targetId] ? canonical.byTarget[targetId].content : null);
  const authoritySha256For = (targetId) => (canonical && canonical.byTarget[targetId] ? canonical.byTarget[targetId].sha256 : null);
  return { llmCall, onValidation, authorityFor, authoritySha256For, canonicalTargetIds: canonical ? canonical.targetIds.slice() : [], stats: () => Object.assign({}, stats, { reviews: order.map((k) => ({ twinId: reviews[k].twinId, targetId: reviews[k].targetId, passes: reviews[k].passes.length, costUsd: reviews[k].costUsd })) }), registry, config: cfg, STRATEGY };
}
