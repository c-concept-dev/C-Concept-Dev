...
const TI = require("./target-identity.js");   /* v1.0.15 (R1) : source unique de l'identite cible <-> document */
...
/** buildUserReport(a) — a = { state, reformulation, runContract, corpusSnapshot, screening, discovery, panel, twinSet, reviewSet, aggregation, qualification, readinessPre, readinessFull, seal, llmCounts, retrieval, lineageRefs, cost? (v1.0.5 : totaux du ledger) } */
function buildUserReport(a) {
  ...
  const targetLabels = {}; arr(a.targetDocuments).forEach((d, i) => { targetLabels[TI.targetIdOf(d, i)] = d.title || d.name; });   /* v1.0.15 (R1) : meme derivation d'identifiant que le pipeline et l'etage professionnel */
  ...
  const report = {
    ...
    /* v1.0.5 — cout REEL du run (ledger, tarif du snapshot du run) ; null pour un run sans ledger ; jamais une estimation */
    cost: a.cost ? { label: "ACTUAL_COST", currency: a.cost.currency || "USD", totalUsd: a.cost.totalUsd, inputUsd: a.cost.inputUsd, outputUsd: a.cost.outputUsd, cacheWriteUsd: a.cost.cacheWriteUsd, cacheReadUsd: a.cost.cacheReadUsd, realCalls: a.cost.calls && a.cost.calls.real, reusedCalls: a.cost.calls && a.cost.calls.reuse, unpricedCalls: a.cost.calls && a.cost.calls.unpriced, byStage: Object.keys(a.cost.byStage || {}).reduce((o, k) => { o[k] = a.cost.byStage[k].totalUsd; return o; }, {}), pricingVersion: a.cost.pricingVersion || null, pricingSnapshotHash: a.cost.pricingSnapshotHash || null } : null,
