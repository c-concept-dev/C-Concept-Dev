...
const GATES = Object.freeze({ CONFIRM_PLAN: "CONFIRM_PLAN", RATIFY_SOURCES: "RATIFY_SOURCES", ECONOMIC_REVIEW: "ECONOMIC_REVIEW" });   /* v1.0.7 : porte economique humaine (jamais scientifique) */
...
function createRunStore(runId) {
  ...
  function publicState(state) {
    ...
    return { runId: state.runId, status: state.status, stage: state.stage, stageLabel: STAGE_LABELS[state.stage] || state.stage, stages: state.stages, gate: state.gate || null, resumable: resumable, stop: state.stop || null,
      ...
      summary: state.summary || null, createdAt: state.createdAt, updatedAt: state.updatedAt, seal: state.seal || null, counters: state.counters || null, live: state.live || null,   /* v1.0.8 : observabilite live (appel en cours, jumeau, revues logiques, flux) */ attempts: state.attempts || 0, kind: state.kind || "NATIVE", provider: state.provider || null, checkpoints: state.checkpoints || null, cost: costSummary(state) };
  }
