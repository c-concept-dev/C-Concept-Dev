 ...
 * avant chaque evenement. v1.0.9 : en-tetes X-EvidenceForge-Proxy-Version / X-EvidenceForge-Timeouts du proxy v0.8 (opts.proxyVersion /
 * opts.timeoutsHeader, ou null pour simuler v0.6 / v0.7). Aucun secret reel ; aucun appel amont.
 */
...
function startFakeWorker(opts) {
  ...
  const srv = http.createServer((req, res) => {
    const V08 = { version: opts.proxyVersion === null ? null : (opts.proxyVersion || "evidenceforge-llm-proxy/0.8-stream-timeouts"), timeouts: opts.timeoutsHeader === null ? null : (opts.timeoutsHeader || "header=60000;stream-inactivity=client;stream-max=client") };
    const send = (st, obj, extra) => { const h = { "content-type": "application/json", "x-evidenceforge-proxy": "evidenceforge-llm-proxy", "x-evidenceforge-transport": "buffered" }; if (V08.version) h["x-evidenceforge-proxy-version"] = V08.version; if (V08.timeouts) h["x-evidenceforge-timeouts"] = V08.timeouts; res.writeHead(st, Object.assign(h, extra || {})); res.end(typeof obj === "string" ? obj : JSON.stringify(obj)); };
