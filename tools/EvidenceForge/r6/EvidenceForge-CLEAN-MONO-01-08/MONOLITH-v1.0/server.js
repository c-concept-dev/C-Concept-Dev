...
const errOut = (res, e) => send(res, e.code === "RUN_NOT_FOUND" ? 404 : (e.code === "GATE_NOT_OPEN" || e.code === "HUMAN_IDENTITY_REQUIRED" || e.code === "MISSION_INVALID" || e.code === "BAD_JSON") ? 400 : 500, { error: e.code || "ERROR", message: e.userMessage || e.message });
...
const server = http.createServer(async function (req, res) {
  ...
  try {
    ...
    if (m === "POST" && p === "/api/runs") { const b = await readBody(req); const files = (b.files || []).map((f) => ({ name: f.name, bytes: Buffer.from(String(f.contentBase64 || ""), "base64") })); const r = await PL.startRun({ question: b.question, files }); PL.advance(r.runId); return send(res, 201, r); }
...
server.listen(PORT, HOST, function () {
  console.log("EvidenceForge MONOLITH v1.0 — http://" + HOST + ":" + PORT);
