...
async function probe(intent) {
  ...
  let res, raw;
  try { res = await fetch(base.replace(/\/$/, "") + "/v1/messages", { method: "POST", headers: { "content-type": "application/json", authorization: "Bearer " + key, "x-evidenceforge-request-id": localRequestId }, body }); raw = await res.text(); }
  catch (e) { throw Object.assign(new Error("NETWORK_UNAVAILABLE: " + e.message), { code: "NETWORK_UNAVAILABLE" }); }
