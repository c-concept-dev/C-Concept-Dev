...
async function discoverProfessionals(input) {
  ...
  const discovery = await adapter.discoverProfessionals({ corpusSnapshot: input.corpusSnapshot, missionDimensionSet: dimensionSetLite });
  ...
  return { discovery, verification, includedCount: included.length, candidates: cands.length, resolved: cands.filter((c) => c.candidateRef).length, networkCalls: client.calls };
}
...
module.exports = { discoverProfessionals, runPanelAndDownstream, loadSealedMono11, realMono09Deps, createOpenAlexClient };
