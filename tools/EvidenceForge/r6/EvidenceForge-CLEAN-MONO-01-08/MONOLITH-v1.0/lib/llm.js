"use strict";
/**
 * EvidenceForge MONOLITH v1.0 — lib/llm.js
 * Transport LLM REEL (Worker delegue evidenceforge-llm-proxy) + politique de reutilisation MONO-11 v0.2 (gelee)
...
/**
 * createLlm({ runDir, runId, sealHash, model, minSpacingMs, maxAttemptsOnCapacity, storePath })
...
function createLlm(opts) {
  ...
  async function llmCall(prompt, meta) {
    ...
    if (d.decision === policy.DECISION.REUSE_VALID) {
      ...
      const candidates = [d.entry.cachePath, path.join(CACHE, d.entry.responseSha256 + ".response.json")].concat(extraCacheDirs.map((dir) => path.join(dir, d.entry.responseSha256 + ".response.json"))).filter(Boolean);
      const src = candidates.find((f) => fs.existsSync(f)) || null;   /* corps reel : chemin consigne, cache du run, ou caches de runs anterieurs declares */
      if (src) {
    ...
    policy.recordReal({ promptSha256: rec.promptSha256, responseSha256: rec.responseSha256, sourceRunId: opts.runId, sourceCallId: rec.providerRequestId || localRequestId, providerRequestId: rec.providerRequestId, providerId: "anthropic", modelId: MODEL, completedAt: rec.completedAt, httpStatus: 200, purpose: rec.purpose, sourceSealHash: opts.sealHash || null, cachePath: path.join(CACHE, rec.responseSha256 + ".response.json") });
  ...
  async function preflight() {
    ...
    try { const r = await httpCall(JSON.stringify({ model: MODEL, max_tokens: 8, messages: [{ role: "user", content: "Reponds uniquement: ok" }] }), "efm-preflight-" + crypto.randomBytes(4).toString("hex")); return Object.assign({ ok: r.res.status === 200, httpStatus: r.res.status, model: MODEL }, r.state); }
  ...
  return { llmCall, onValidation, preflight, workerCallFn, model: MODEL, counts: () => ({ real: REAL, reused: REUSED }), PROVIDER_STATES };
}
