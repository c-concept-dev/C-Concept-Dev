...
function buildUserReport(a) {
  ...
  const dimLabel = {}; arr(a.runContract && a.runContract.disciplinesProposees).forEach((d) => { dimLabel[d.id] = d.discipline || d.id; dimLabel[d.discipline] = d.discipline; });
  ...
  const limits = [
    ...
    "Le panel a été admis par une porte machine à base de preuves (MONO-11 v0.2), sans acte humain : c'est une réserve permanente consignée.",
  ];
  if (a.reviewSet && a.reviewSet.summary && a.reviewSet.summary.scientificValidity === false) limits.push("Le jeu de revues porte scientificValidity=false (drapeau du lot gelé EF-03B) : les constats sont des analyses documentaires, non des résultats scientifiques.");
  ...
  const report = {
    ...
    counts, pipeline: { disciplines: arr(a.runContract && a.runContract.disciplinesProposees).filter((d) => d.statut === "retenue").map((d) => d.discipline), sourcesRetrieved: a.retrieval && a.retrieval.sourceCount, sourcesIncluded: arr(a.corpusSnapshot && a.corpusSnapshot.sources).filter((s) => s.statutScreening === "inclus").length,
  ...
  report.reportHash = sha(JSON.stringify(Object.assign({}, report, { generatedAt: null })));
  return report;
}

module.exports = { buildUserReport, scientificallyUsable, LABELS, QUALIFICATION_USER };
