...
async function buildCorpusFromDecisions(input) {
  ...
  if (!sub || sub.state !== "SUCCESS") { const e = new Error("CORPUS_NOT_BUILT: " + JSON.stringify(sub)); e.code = "CORPUS_NOT_BUILT"; e.userMessage = "Le corpus de sources n'a pas pu être constitué (" + ((sub && sub.lastError) || "état " + (sub && sub.state)) + ")."; throw e; }
