...
function createLlm(opts) {
  ...
  let REFUSED = 0;
  const PROVIDER_ID = "anthropic", VALIDATION_CONTRACT = "MONO-11-v2";   /* contrat de validation des reponses (enforcers/parseurs geles MONO-11 v0.2 / MONO-01) */
  ...
  let LATCH = null;
  const TRANSPORT_CODES = ["NETWORK_UNAVAILABLE", "PROVIDER_TIMEOUT", "PROVIDER_CREDIT_EXHAUSTED", "PROVIDER_RATE_LIMITED", "PROVIDER_UNAVAILABLE", "PROVIDER_NOT_CONFIGURED", "PROVIDER_EMPTY_RESPONSE"];
  ...
  return { llmCall, onValidation, preflight, workerCallFn, model: MODEL, counts: () => ({ real: REAL, reused: REUSED, reuseRefused: REFUSED }), PROVIDER_STATES,
    transportFailure: () => LATCH, latch: latch, resetLatch: () => { LATCH = null; }, TRANSPORT_CODES };
}
