...
module.exports = async function (h) {
  ...
  await T("T-EF03B-34", "purete et frontiere : mergeLiteralRefs / preserveLineageRepairs sont pures (meme entree => meme sortie, entrees non mutees), n'inventent aucune chaine, ne lisent que targetEvidenceRefs ; le seul point d'appel est la recomposition apres litteralisation (bloc G) ; RE.recompose gele inchange", () => {
    ...
    const src = fs.readFileSync(path.join(__dirname, "..", "lib", "ef03b-resilience.js"), "utf8"); assert((src.match(/preserveLineageRepairs\(v\.parsed/g) || []).length === 1 && /RE\.recompose\(v\.parsed, kept\)/.test(src) && /RE\.recompose\(st\.parsed, rp\.repairs/.test(src), "un seul point de fusion : la litteralisation ; le miroir de la reparation ciblee gelee reste le recompose gele");
