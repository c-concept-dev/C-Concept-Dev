...
function buildProfessionalsEconomics(store) {
  ...
  return { schema: "EvidenceForge.ProfessionalsEconomics", schemaVersion: "MONOLITH-v1.0.5", runId: state.runId, status: cp ? "FROM_CHECKPOINT" : "PARTIAL_IN_PROGRESS", ledgerPresent, generatedAt: new Date().toISOString(),
    ...
    earlyStop: Object.assign({ policy: (P.CONFIG.professionals && P.CONFIG.professionals.earlyStop && P.CONFIG.professionals.earlyStop.enabled !== false) ? ((P.CONFIG.professionals.earlyStop.policy && P.CONFIG.professionals.earlyStop.policy.id) || "PANEL-SUFFICIENCY-v2") : "NONE", provenance: "PRODUCT_POLICY", arbitraryThreshold: false, statement: "Arrêt anticipé uniquement par SUFFISANCE selon une politique produit explicite (chaque angle : représentants approuvés et indépendants au-dessus du plancher contractuel, ou vivier épuisé déclaré comme manque) — jamais par un nombre d'évaluations ; le plateau n'est qu'un signal. Cette mesure sert à étudier, sur plusieurs runs réels, le gain marginal par rang." }, cp && cp.sufficiency ? { state: cp.sufficiency.state, panel: cp.sufficiency.panel || null, evaluated: cp.sufficiency.evaluated, skipped: cp.sufficiency.skipped, approved: cp.sufficiency.approved != null ? cp.sufficiency.approved : cp.sufficiency.admissible, reasons: cp.sufficiency.reasons } : {}),
