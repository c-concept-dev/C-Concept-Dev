...
const RESUMABLE = ["INTERRUPTED_BY_RESTART", "LEDGER_DUPLICATE_OR_INVALID", "PROVIDER_CREDIT_EXHAUSTED", "PROVIDER_RATE_LIMITED", "NETWORK_UNAVAILABLE", "PROVIDER_TIMEOUT", "PROVIDER_UNAVAILABLE", "PROVIDER_NOT_CONFIGURED", "LLM_UNAVAILABLE", "PROVIDER_EMPTY_RESPONSE",
  ...
  const p = (async function () {
    ...
    try {
      ...
      if (!isDone("CORPUS")) {
        ...
        store.saveJson("audit-decisions.json", auditDecisionsInput); const dc = {}; auditDecisionsInput.decisions.forEach((d) => { dc[d.decision] = (dc[d.decision] || 0) + 1; }); store.event({ level: "user", message: "Décisions ratifiées : " + JSON.stringify(dc).replace(/[{}"]/g, "").replace(/,/g, ", ") + " (" + auditDecisionsInput.decisions.length + " sources)." });
...
function importReport(report) {
  ...
  store.saveJson("report.json", report); store.write(state); store.event({ level: "user", message: state.userMessage });
