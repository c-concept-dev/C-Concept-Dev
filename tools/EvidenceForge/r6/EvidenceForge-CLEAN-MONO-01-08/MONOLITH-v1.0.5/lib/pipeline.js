  ...
  const p = (async function () {
    ...
    try {
      ...
      if (!isDone("PROFESSIONALS")) {
        ...
        const onSufficiency = (d) => store.event({ level: "user", actor: "machine", event: "panel_sufficiency", code: d.state, message: d.state === "EARLY_STOP_CONFIRMED" ? "Panel jugé suffisant après " + d.evaluated + " évaluation(s) : " + d.statement + " (" + d.reasons.join(" ; ") + ")" : d.state === "EARLY_STOP_CANDIDATE" ? "Plateau observé après " + d.evaluated + " évaluation(s) : " + d.statement : "Évaluation des professionnels : reprise (" + d.statement + ")", _state: state });
        const r = await SP.runPanel({ runDir: store.dir, runId, attemptId: state.attempts, attemptRunId, missionId, missionHash: state.mission.questionSha256, missionQuestion, runContract: disciplines.runContract, corpusSnapshot, discovery: disc.discovery, verification: disc.verification, llm, log, selection, onSelection, onSufficiency });
      ...
      if (!isDone("QUALIFICATION")) {
        ...
        if (cpPro.sufficiency && cpPro.sufficiency.skipped > 0) upstreamReservations.push({ code: "PROFESSIONAL_EVALUATION_EARLY_STOP", detail: "Arrêt anticipé de l'évaluation (" + cpPro.sufficiency.rule + ", état " + cpPro.sufficiency.state + ") : " + cpPro.sufficiency.evaluated + " professionnel(s) évalué(s), " + cpPro.sufficiency.skipped + " non évalué(s) car leur angle avait atteint sa cible d'admissibles ou le panel était jugé suffisant (" + cpPro.sufficiency.reasons.join(" ; ") + ")." });
