...
function buildProfessionalsEconomics(store) {
  ...
  return { schema: "EvidenceForge.ProfessionalsEconomics", schemaVersion: "MONOLITH-v1.0.5", runId: state.runId, status: cp ? "FROM_CHECKPOINT" : "PARTIAL_IN_PROGRESS", ledgerPresent, generatedAt: new Date().toISOString(),
    ...
    earlyStop: Object.assign({ policy: (P.CONFIG.professionals && P.CONFIG.professionals.earlyStop && P.CONFIG.professionals.earlyStop.enabled !== false) ? "PANEL-SUFFICIENCY-v1" : "NONE", arbitraryThreshold: false, statement: "Arrêt anticipé uniquement par SUFFISANCE démontrée (chaque angle : cible d'admissibles atteinte ou vivier épuisé) — jamais par un nombre d'évaluations ; le plateau n'est qu'un signal. Cette mesure sert à étudier, sur plusieurs runs réels, le gain marginal par rang." }, cp && cp.sufficiency ? { state: cp.sufficiency.state, evaluated: cp.sufficiency.evaluated, skipped: cp.sufficiency.skipped, admissible: cp.sufficiency.admissible, reasons: cp.sufficiency.reasons } : {}),
