...
const EP = require("./economic-panel.js");   /* v1.0.7 : economic panel (SUFFICIENCY_FIRST), fanout des revues, projection, gate economique */   /* v1.0.6 AUTO-CHUNK UPLOAD : metadonnees de segments persistees, verifiees a chaque chargement */
    ...
    try {
      ...
      if (!isDone("QUALIFICATION")) {
        ...
        const r = await SP.runDownstreamFromCheckpoint({ runDir: store.dir, runId, attemptId: state.attempts, attemptRunId, missionId, missionHash: state.mission.questionSha256, missionQuestion, runContract: disciplines.runContract, corpusSnapshot, discovery: disc.discovery, verification: disc.verification,
          checkpoint: cpPro, targetDocuments: targetDocs, llm, upstreamReservations, log });   /* v1.0.7 : cibles = dossier de mission (ou par document, explicite) */
...
module.exports = { startRun, advance, confirmPlan, ratifySources, confirmEconomics, buildDownstreamProjection, gateView, importReport, updateBudget, costView, economicsView, requestRunStop, RESUMABLE, isResumable, _loadDocuments: loadDocuments /* v1.0.6 : expose pour test-chunking (verification des segments au rechargement) */ };
