 ...
 *   MONO-11 v0.3-r1 (GELE, sceau verifie AVANT chargement : corpus reels -> suffisance -> pertinence reelle -> gate machine ->
...
const EP = require("./economic-panel.js");   /* v1.0.7 : SUFFICIENCY_FIRST — lots, contribution marginale, arret budgetaire AVANT un lot (jamais une suffisance) */
...
/** Sceau MONO-11 (version declaree par la config : v0.3-r1) verifie AVANT tout require du lot (RUN_ON_UNSEALED_CODE sinon) ; zip canonique compare a la config. */
function loadSealedMono11() {
  const SG = require(path.join(P.MONO11, "core", "run-seal-guard.js"));
  const SEAL = SG.assertSealedRuntime({ lotDir: P.MONO11, zipPath: P.MONO11_ZIP, expectedZipSha256: P.CONFIG.frozenLots["MONO-11"].canonicalZipSha256, expectedVersion: P.MONO11_VERSION });
 ...
 * MONO-11 (gele) refuse alors, a juste titre, d'enregistrer deux fois `mono11:corpus-sufficiency:<candidateRef>`
...
async function openMono10Run(input) {
  ...
  const intent = { runId: input.attemptRunId, missionHash: input.missionHash, producerId: "EvidenceForge-" + P.CONFIG.product.version, producerVersion: "MONO-11-" + SEAL.mono11Version + "(seal " + SEAL.runtimeSealSha256.slice(0, 12) + ")+MONO-10-v0.19", executionMode: "PRODUCTION", openedAt: now() };
  ...
  const run = { SEAL, M11, F, M10, R, manifest, registry, verifier, ctx, att, bind, ledger, llmConfig: null, capability: null, capabilityArtifact: null, cfgPath,
  ...
  if (!input.skipProbe) await certifyLlm(run, input.llm, input.attemptRunId);
  return run;
}

/** Capacite LLM REELLE certifiee par la frontiere (sonde reelle, delai borne) — appelee APRES la persistance de la selection plafonnee (v1.0.3). */
async function certifyLlm(run, llm, attemptRunId) {
  const { verifier, registry, bind, R, ctx } = run;
  const llmConfig = { providerId: "anthropic", modelId: llm.model, workerBindingId: "evidenceforge-llm-proxy", credentialPresent: !!process.env.EVIDENCEFORGE_WORKER_API_KEY, authMode: "delegated", runId: attemptRunId };
  let capability; try { capability = bind("llm-capability", R.CAPABILITY, await verifier.runLlmProbe(llmConfig, ctx)); } catch (e) { llm.latch(e, "probe"); throw e; }
  ...
  run.llmConfig = llmConfig; run.capabilityArtifact = capability; run.capability = { status: capability.status, probeStatus: capability.probeStatus, requestId: capability.requestId, certified: cert.certified };
...
async function runPanel(input) {
  ...
  const run = await openMono10Run(Object.assign({}, input, { included, discovery, skipProbe: true }));   /* aucun appel LLM (meme la sonde) avant la persistance de la selection */
  ...
  const assessmentFullRef = M10.LIN.artifactRef(assessment, "candidate-assessment", R.ASSESSMENT);
  log({ event: "mono10_assessment", summary: assessment.summary, mono10RunId: input.attemptRunId });

  /* ===== PLAFOND D'EVALUATION (v1.0.3) : selection deterministe sur le pool complet, PERSISTEE avant tout appel LLM ; a la reprise,
     la selection persistee est reprise a l'identique (verification des hashes du pool et de la selection) ===== */
  const cap = Number(P.CONFIG.professionals && P.CONFIG.professionals.maxCandidatesToEvaluate);
  const computed = PSEL.selectCandidates({ assessment, discovery, cap });
  let selection = computed;
  if (input.selection) {
    if (input.selection.poolHash !== computed.poolHash || input.selection.cap !== computed.cap || input.selection.algorithm !== computed.algorithm) { const e = new Error("SELECTION_POOL_MISMATCH: la selection persistee ne correspond pas au pool courant"); e.code = "SELECTION_POOL_MISMATCH"; e.userMessage = "La sélection plafonnée persistée ne correspond plus au pool de candidats : reprise refusée (aucune nouvelle sélection n'est calculée en silence)."; throw e; }
    if (input.selection.selectionHash !== computed.selectionHash) { const e = new Error("SELECTION_HASH_MISMATCH"); e.code = "SELECTION_HASH_MISMATCH"; e.userMessage = "La sélection recalculée diffère de la sélection persistée : reprise refusée."; throw e; }
    selection = input.selection;
  } else if (typeof input.onSelection === "function") { await input.onSelection(selection); }   /* persistance AVANT la sonde et AVANT le premier appel oracle */
  log({ event: "evaluation_cap_selection", cap: selection.cap, poolCount: selection.poolCount, selected: selection.selectedCount, reason: selection.reason, selectionHash: selection.selectionHash, perDimension: selection.perDimension });
  ...
  /* v1.0.7 — ECONOMIC PANEL (SUFFICIENCY_FIRST) : lots planifies sur l'ordre de selection (tourniquet par discipline) ; le controleur consulte le traqueur
     (autorite scientifique) puis le budget AVANT chaque lot ; il n'ajoute aucun appel et ne fabrique jamais un PANEL_SUFFICIENT. Desactive => comportement v1.0.6. */
  const ecoCfg = EP.normalizeEconomicPolicy(P.CONFIG.professionals && P.CONFIG.professionals.economic);
  const ecoPlan = EP.planBatches({ orderedCandidates: selection.selectedIds.map((id) => ({ candidateRef: id, dimension: primaryDim.get(id) })), dimensions: dimensionSet.dimensions.map((d) => d.id), policy: ecoCfg });
  const ecoBudget = input.economic && input.economic.budget ? input.economic.budget : null;
  const eco = ecoCfg.enabled ? EP.createEconomicController({ tracker, plan: ecoPlan, policy: ecoCfg, budget: ecoBudget, spentUsd: input.economic && typeof input.economic.spentUsd === "function" ? input.economic.spentUsd : () => 0, perCandidateUsd: input.economic && typeof input.economic.perCandidateUsd === "function" ? input.economic.perCandidateUsd : () => [], historyPerCandidateUsd: input.economic && input.economic.historyPerCandidateUsd || null, log }) : null;
  log({ event: "economic_panel_plan", policy: ecoCfg.policy, enabled: ecoCfg.enabled, capIsGuardNotTarget: true, cap: selection.cap, selected: selection.selectedCount, initialPanelSize: ecoPlan.initialPanelSize, plannedBatches: ecoPlan.batches.length, expansionBatch: ecoCfg.expansionBatch, budget: ecoBudget ? { mode: ecoBudget.mode, costBudgetUsd: ecoBudget.costBudgetUsd } : null });
  ...
  async function fetchAuthorWorks(candidateRef) {
    ...
    const dec = eco ? eco.beforeEvaluate(candidateRef) : tracker.shouldEvaluate(candidateRef);
    if (!dec.evaluate) {
      /* arret budgetaire : le candidat est DIFFERE (jamais « epuise » : le vivier scientifique reste ouvert, PANEL_CONTINUE) ; suffisance : non evalue (skip v1.0.5) */
      if (dec.deferred) log({ event: "candidate_deferred_budget", candidateId: candidateRef, reason: dec.reason, rule: EP.POLICY_ID }); else { tracker.skip(candidateRef, dec.reason); log({ event: "candidate_skipped_sufficiency", candidateId: candidateRef, reason: dec.reason, rule: eco ? EP.POLICY_ID + "+" + PSUF.RULE_ID : PSUF.RULE_ID }); }
      /* v1.0.7 — arret budgetaire AVANT un lot : verrou de transport (aucun appel suivant), run STOPPED / PANEL_INCOMPLETE_BUDGET_LIMIT reprenable ; jamais une suffisance */
      if (dec.code === EP.CODES.BUDGET_LIMIT && !llm.transportFailure()) { const be = new Error(EP.CODES.BUDGET_LIMIT + ": le lot suivant (" + (dec.detail && dec.detail.units) + " candidat(s), estime " + (dec.detail && dec.detail.nextBatchEstimateUsd) + " USD) depasserait le budget " + (dec.detail && dec.detail.costBudgetUsd) + " USD (depense " + (dec.detail && dec.detail.spentUsd) + " USD)"); be.code = EP.CODES.BUDGET_LIMIT; be.fatal = true; be.economicStop = dec.detail || null;
        be.userMessage = "Le lot suivant de professionnels n'a pas été lancé : son coût estimé dépasserait votre budget. Le panel est INCOMPLET (couverture actuelle et angles sous-couverts conservés) ; augmentez le budget pour reprendre — rien n'est présenté comme suffisant."; llm.latch(be, "expansion du panel (lot " + (dec.detail && dec.detail.at) + ")"); }
      throw Object.assign(new Error("PANEL_SUFFICIENT_EARLY_STOP: " + dec.reason + " — évaluation non demandée (" + (eco ? EP.POLICY_ID : PSUF.RULE_ID) + ")"), { code: "PANEL_SUFFICIENT_EARLY_STOP", reason: dec.reason }); }
  ...
  const partialStateFor = (e) => { const d = tracker.record(); const f = (llm.transportFailure && llm.transportFailure()) || null;
    return { schema: "EvidenceForge.PanelSufficiencyPartialState", schemaVersion: "MONOLITH-v1.0.5", notAVerdict: true, runId: input.runId, attemptId: input.attemptId, mono10RunId: input.attemptRunId, reason: e.code || "UNEXPECTED_ERROR", transportFailure: f ? { code: f.code, message: String(f.message || "").slice(0, 300) } : null,
      rule: d.rule, policy: d.policy, independenceAlgorithm: d.independenceAlgorithm, scientificPanelState: d.panel, earlyStopState: d.state, economicEvaluationState: eco ? eco.record().economicState : null, economicPolicy: eco ? EP.POLICY_ID : null, economicPanel: eco ? eco.record() : null,
      ...
      limitations: ["état partiel au moment de l'arrêt : ni verdict, ni checkpoint, ni preuve de suffisance", "le gate gelé de fin de boucle (gatePanel) n'a pas produit de panel : les approbations sont celles de gateCandidate par candidat", eco ? "economicEvaluationState : SUFFICIENCY_FIRST (v1.0.7) — le cout peut empecher un lot, jamais fabriquer une suffisance" : "economicEvaluationState réservé : aucune politique économique n'est évaluée dans cette version"] }; };
  let res;
...
async function runDownstreamFromCheckpoint(input) {
  ...
  /* re-ancrage (v1.0.3-r1) : la REFERENCE du checkpoint (hash de contenu verifie au chargement, run MONO-10 d'origine, racines
     d'origine) est inscrite dans le registre de lignee MONO-11 (gele : record() lie l'artefact au manifeste MONO-10 du run courant
     via RM.bindArtifact et le chaine par hash, sans relation typee) — la frontiere prevue par MONO-11 pour ses artefacts de
     lignee ; le registre MONO-10 (relations typees) ne recoit que l'assessment sous son type canonique. Aucune nouvelle autorite. */
  ledger.record("mono11:professionals-checkpoint-ref", "checkpoint-ref", { schema: "EvidenceForge.ProfessionalsCheckpointRef", schemaVersion: "MONOLITH-v1.0.3", contentHash: cp.contentHash, committedAt: cp.committedAt || null, sourceMono10RunId: cp.mono10RunId, sourceLedgerRoot: cp.outputHashes.ledgerRoot, sourceRegistryRoot: cp.outputHashes.registryRoot, panelHash: cp.outputHashes.panelHash, evidenceCount: cp.outputHashes.evidenceCount, selectionHash: cp.selection ? cp.selection.selectionHash : null, reanchoredBy: input.attemptRunId }, []);
...
module.exports = { discoverProfessionals, runPanel, runDownstreamFromCheckpoint, openMono10Run, certifyLlm, openAlexTransportError, assertNoTransportFailure, assertNoClassificationError, dedupeDiscovery, loadSealedMono11, realMono09Deps, createOpenAlexClient };
