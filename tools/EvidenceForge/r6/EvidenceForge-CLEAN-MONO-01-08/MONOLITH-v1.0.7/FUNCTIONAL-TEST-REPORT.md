# FUNCTIONAL-TEST-REPORT — MONOLITH v1.0 → v1.0.2 (2026-09-14 / 2026-09-15)
...
## F. Addendum v1.0.1 / v1.0.2 (2026-09-15)

Runs humains réels (dossier `~/evidenceforge-work/reports/audit-final-runs/`, actes du propriétaire aux portes) et incidents corrigés :

| Run | Actes réels | Incident | Correctif |
|---|---|---|---|
| `efm-20260915-04c86cc3` (question seule) | porte 1 + porte 2 (185 sources, 0 renversement) | `CORPUS_NOT_BUILT` : EF-01A gelé, cardinalité `documentsAudites` (0) ≠ `targetDocuments` (1) | v1.0.1 `effectiveDocuments` (E6, E7) + garde précoce `RUNCONTRACT_TARGET_MISMATCH` (Q9) + persistance des décisions ratifiées avant l'appel gelé |
| `efm-20260915-963844b7` (question seule, créé avant le correctif) | porte 1 | interrompu par un redémarrage serveur (mon fait) puis arrêté par la garde précoce | — |
| `efm-20260915-2257172b` (question seule) | porte 1 + porte 2 (172 sources : 58/113/1) ; corpus constitué | `LEDGER_DUPLICATE_OR_INVALID` (même auteur dans deux disciplines) ; puis 424 `corpus_fetch_error` réseau absorbés en insuffisance documentaire par MONO-11 (attempt 4) | v1.0.1 `dedupeDiscovery` (D1) ; v1.0.2 verrou de panne + frontière fail-closed (X1–X3), checkpoints (X4–X6) |
| BUG-H1-002 (capture propriétaire) | porte 1 cliquée, page figée | abonnés SSE par instance de magasin : la page ne recevait pas les mises à jour | v1.0.1 abonnés partagés par runId + relecture périodique + retour immédiat après le clic (Q8) ; v1.0.2 dédoublonnage par `eventId` |

Tests v1.0.2 ciblés (tous OK) : X0 régression `done(stage,{status})` ; X1 panne réseau pendant le corpus ⇒ STOPPED sans panel ; X2 classification transport OpenAlex ; X3 verrou LLM ; X4 checkpoint puis panne aval ⇒ reprise directe en aval (panel non recalculé, `runPanel` jamais rappelé) ; X5 écriture atomique interrompue ; X6 préflight ne dégrade pas DONE ; X7 reuse contextuelle (modèle, sceau, corps altéré) ; X8 sonde à délai borné ; X9 `classificationError` ; X10 source sans proposition ; X11/X12 UI et traceur. Navigateur réel (CDP, `tools/browser-tests.js`) : 12/12 — CTA Reprendre sur FAILED reprenable, porte visible depuis le mode expert, source sans proposition (aucune exception console, décision explicite, persistance locale), reconnexion SSE sans doublon, échec GET porte + Réessayer sans mutation d'état.

Test humain H1 (question seule, traceur ON) : à exécuter après packaging v1.0.2 — voir bloc final du chantier.
