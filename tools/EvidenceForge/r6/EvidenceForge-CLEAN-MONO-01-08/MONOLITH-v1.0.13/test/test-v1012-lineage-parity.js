...
module.exports = async function (h) {
  ...
  const viaAdapter = async (i, raw, repairs) => { const d = tmp(); const llm = fakeLlm((prompt, meta) => meta.strategy && /TARGETED/.test(meta.strategy) ? { text: repairJson(repairs) } : { text: raw });
    const adapter = EF3.createReviewAdapter({ llm, runDir: d, runId: "t", attemptId: 1, sealHash: SEAL.runtimeSealSha256, config: { maxTokens: 12288, excerptChars: 40, enableLiteralization: false }, twinsTotal: () => 1, documentAuthority: () => DOC });
  ...
  await T("V12-18", "version du produit et frontiere : config MONOLITH-v1.0.13, lot MONO-11 v0.4 ; le monolithe ne fabrique aucun prompt de revue (hors adaptateur EF-03B) ; lots geles 0 divergence", () => {
    assert((P.CONFIG.product.version) === ("MONOLITH-v1.0.13")); assert((P.CONFIG.frozenLots["MONO-11"].dir) === ("MONO-11/v0.4"));
