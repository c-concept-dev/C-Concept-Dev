"use strict";
/**
 * EvidenceForge MONOLITH v1.0.13 — test/test-v1013-document-authority.js : RA2 DOCUMENT CONTENT AUTHORITY (RA2-01…08) + RA1 TRACE PARITY (RA1-01…03).
...
module.exports = async function (h) {
  ...
  console.log("\n— v1.0.13 : RA2 DOCUMENT CONTENT AUTHORITY + RA1 TRACE PARITY —");
  ...
  const mkDoc = (content) => ({ targetId: "target-01", label: "Document", role: "review_target_not_evidence", content: content, sourceDocumentRef: sha(content) });
  ...
  await T("RA2-04", "prompt reconstruction differs from doc.content : l'AUTORITE fait foi (le contenu du prompt n'est jamais l'autorite finale de litteralite)", async () => {
    const doc = mkDoc(MARKER_DOC);
    /* autorite = document reel ; le prompt (construit par le lot gele) reste l'artefact de generation */
    const o = await viaAdapter(4, doc, () => ({ text: reviewJson(4, { "DIM-01": [AFTER_B] }, BEFORE) }));
    assert(o.r.review.reviewStatus === "complete", JSON.stringify(o.r.review.error));
    assert(JSON.stringify(refsOf(o.r, "DIM-01")) === JSON.stringify([AFTER_B]), "citation apres marqueur conservee");
    /* et inversement : une autorite qui ne contient PAS la citation la refuse, meme si le prompt la contient */
    const plain = mkDoc(PLAIN_DOC);
    const o2 = await viaAdapter(41, plain, (prompt, meta) => ({ text: reviewJson(41, { "DIM-01": [AFTER_A] }, BEFORE) }), { documentAuthority: () => PLAIN_DOC });
    assert(o2.r.review.reviewStatus === "error" || refsOf(o2.r, "DIM-01").indexOf(AFTER_A) === -1, "citation absente de l'autorite : jamais acceptee"); });
  ...
  await T("RA1-02", "parseRepair failure : aucune ecriture au registre (aucune revue acceptee) ; l'echec reste explicite", async () => {
  ...
  await T("RA1-03", "parseRepair failure : le resultat final est celui du lot gele (aucune acceptation modifiee) ; le miroir n'a fait que tracer", async () => {
