...
function createLlm(opts) {
  ...
  async function llmCallInner(prompt, meta) {
    ...
    const body = JSON.stringify(Object.assign({ model: CALL_MODEL, max_tokens: 8192, messages: [{ role: "user", content: prompt }] }, wantStream ? { stream: true } : {}));   /* v1.0.8 : `stream: true` = transport seulement (meme prompt, meme max_tokens) */
    ...
    const rec = { kind: "LLM_CALL", decision: d.decision, purpose: (meta && meta.purpose) || null, pass: (meta && meta.pass) || null, runId: opts.runId, startedAt, completedAt: new Date().toISOString(), localRequestId,
      ...
      twinId: (meta && meta.twinId) || null, targetId: (meta && meta.targetId) || null };   /* v1.0.8 : identite de la revue journalisee (absente en v1.0.7) */
  ...
  return { llmCall, onValidation, preflight, workerCallFn, model: MODEL, counts: () => ({ real: REAL, reused: REUSED, reuseRefused: REFUSED }), PROVIDER_STATES, streaming: { enabled: STREAMING, inactivityMs: SCFG.inactivityMs, maxTotalMs: SCFG.maxTotalMs, firstByteTimeoutMs: timeoutMs },
