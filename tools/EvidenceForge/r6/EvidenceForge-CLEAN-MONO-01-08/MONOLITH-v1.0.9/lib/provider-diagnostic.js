...
async function probeWorker(opts) {
  ...
  /* v1.0.9 : la sonde rapporte la version du proxy et ses delais annonces (en-tetes non secrets du Worker v0.8 ; null pour v0.6 / v0.7) — le doctor prouve ainsi quel proxy est deploye */
  if (res.status === 400 && j && j.error === "invalid_request") return { ready: true, reachable: true, authOk: true, code: "OK", userMessage: "Worker joignable, clé acceptée (sonde gratuite : aucun appel amont).", httpStatus: 400, ms, credentials: cs, proxyVersion: res.headers.get("x-evidenceforge-proxy-version") || null, proxyTimeouts: res.headers.get("x-evidenceforge-timeouts") || null, checkedAt: new Date().toISOString() };
