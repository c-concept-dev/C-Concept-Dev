 ...
 * v1.0.7 : POST /api/runs/:id/confirm-economics (porte economique humaine), GET /api/runs/:id/projection (projection avant revues).
...
const server = http.createServer(async function (req, res) {
  ...
  try {
    ...
    if (mr) {
      ...
      /* v1.0.7 — porte ECONOMIQUE (humaine) : confirmation explicite du cout projete avant les revues ; jamais une decision scientifique */
      if (m === "POST" && action === "replay-reviews") { const b = await readBody(req); try { return send(res, 200, PL.replayReviews(runId, { requestedBy: b && typeof b.requestedBy === "string" ? b.requestedBy.trim().slice(0, 120) : "user" })); } catch (e) { return send(res, 409, { error: e.code || "REPLAY_REFUSED", message: e.userMessage || e.message }); } }   /* v1.0.10 : replay cible des revues EF-03B (acte explicite) */
      if (m === "POST" && action === "confirm-economics") { const b = await readBody(req); return send(res, 200, await PL.confirmEconomics(runId, b)); }
      if (m === "GET" && action === "projection") { const st = RS.createRunStore(runId); return send(res, 200, st.loadJson("economic-projection.json") || { status: "NOT_AVAILABLE" }); }
      /* RUN SAFETY — arret utilisateur persistant : aucun nouvel appel apres la demande ; STOPPED / STOPPED_BY_USER, reprenable explicitement */
      if (m === "POST" && action === "stop") { const b = await readBody(req); return send(res, 200, PL.requestRunStop(runId, { actor: (b && typeof b.actor === "string" && b.actor.trim()) ? b.actor.trim().slice(0, 120) : "user" })); }
