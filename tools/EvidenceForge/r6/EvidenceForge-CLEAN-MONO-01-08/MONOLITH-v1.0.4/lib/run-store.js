...
function createRunStore(runId) {
  ...
  function publicState(state) {
    ...
    return { runId: state.runId, status: state.status, stage: state.stage, stageLabel: STAGE_LABELS[state.stage] || state.stage, stages: state.stages, gate: state.gate || null, resumable: resumable,
      ...
      summary: state.summary || null, createdAt: state.createdAt, updatedAt: state.updatedAt, seal: state.seal || null, counters: state.counters || null, attempts: state.attempts || 0, kind: state.kind || "NATIVE", provider: state.provider || null, checkpoints: state.checkpoints || null };
  }
