...
if (!bundleRoot || !wsRoot) { console.error("Usage: node test-mono08-v0.8-integration.js <bundleRoot> <workspaceRoot>"); process.exit(2); }
...
(async () => {
  ...
  console.log("");
  if (failed.length) { console.log("ECHECS (" + failed.length + ") :"); failed.forEach(r => console.log("  - " + r.name)); console.log(""); console.log("MONO08_V08_INTEGRATION = FAIL"); process.exit(1); }
  console.log("TOUS LES TESTS PASSENT (" + results.length + ")");
  console.log("");
  console.log("MONO08_V08_INTEGRATION = PASS");
