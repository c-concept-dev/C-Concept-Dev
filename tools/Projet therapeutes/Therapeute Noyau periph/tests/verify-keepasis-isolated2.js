...
(async () => {
  ...
  // Stub fetch immediately so the real adocSend triggered by form submit finishes fast
  await page.evaluate(() => {
  ...
  const thinkingBefore = await page.evaluate(() => document.getElementById('adoc-send-btn').disabled);
  console.log('send button disabled after initial (stubbed) send completed:', thinkingBefore);
  ...
  console.log('clean isolated adocKeepAsIs result:', result);
  await browser.close();
})();
