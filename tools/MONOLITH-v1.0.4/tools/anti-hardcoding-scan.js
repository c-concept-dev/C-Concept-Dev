...
const FORBIDDEN = [/\bJMJS\b/i, /\bD103\b/, /\bS0[12]\b/, /ma-mission-001/, /\b(48|110|36|88|22) (candidats?|sources?|professionnels?)\b/i, /quota\s*[:=]\s*\d+/i, /panelSize\s*[:=]\s*\d+/i, /fixedPanel/i,
...
function caseTokens() {
  ...
  env.split(":").filter(Boolean).forEach(function (f) { if (!fs.existsSync(f)) return; let j; try { j = JSON.parse(fs.readFileSync(f, "utf8")); } catch (e) { return; }
    (function walk(v) { if (Array.isArray(v)) v.forEach(walk); else if (v && typeof v === "object") Object.keys(v).forEach((k) => { if (["displayName", "missionId", "snapshotId", "runId", "candidateRef", "orcid", "affiliation"].indexOf(k) !== -1 && typeof v[k] === "string" && v[k].length >= 6) toks.add(v[k]); walk(v[k]); }); })(j); });
