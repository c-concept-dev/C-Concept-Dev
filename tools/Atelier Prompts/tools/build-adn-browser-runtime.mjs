...
const modules = [
  ...
  /* La politique de sollicitation définit l'atomicité d'une question — pour le plan
   * rapide comme pour la frontière d'affichage. L'orchestrateur l'importe ; elle
   * doit donc être embarquée, sans quoi `guardDisplayedQuestion` se résout à
   * `undefined` dans le navigateur. C'est précisément le défaut que cette liste
   * de dépendances a déjà laissé passer une fois. */
  {
    dir: 'workers/shared',
  ...
  {
    dir: 'workers/shared',
    file: 'core-first-plane.js',
    name: 'COREPLANE',
    exports: ['NOMINAL_PLANES','DEFAULT_NOMINAL_PLANE','ESCALATION_KINDS','ESCALATION_VERDICTS','CORE_CRITIC_VERDICTS','CORE_SYSTEM_PROMPT','CORE_OUTPUT_FIELDS','CORE_JSON_SCHEMA','makeCoreUserMessage','validateCoreInput','validateCoreOutput','parseCoreOutput','assessEscalation','CORE_CRITIC_SYSTEM_PROMPT','CORE_CRITIC_JSON_SCHEMA','makeCoreCriticUserMessage','validateCoreCriticOutput','parseCoreCriticOutput','CORE_ARBITER_SYSTEM_PROMPT','CORE_ARBITER_JSON_SCHEMA','makeCoreArbiterUserMessage','parseCoreArbiterOutput','CORE_ROLE_DEFINITIONS','runCoreFirstTurn','resolveNominalPlane'],
    deps: ['ORCORE']
  },
  {
    dir: 'workers/shared',
    ...
    name: 'ORORCH',
    exports: ['OPERATIONAL_REQUEST_ROLE_SEQUENCE','OPERATIONAL_REQUEST_TURN_ORIGIN_STATE','runOperationalRequestTurn','applyDisplayGuardToTurn','assertOrchestratedRolesCoverOprie'],
    deps: ['ORSTATE','ORCORE','DECISIONCORE','ROLEDEG','SOLICIT','COREPLANE']
  },
  {
