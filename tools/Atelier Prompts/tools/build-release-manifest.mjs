...
export function inventaire() {
  // BETA-04: the physical archive, not Git's index, is the delivery authority.
  const walk=(dir='')=>fs.readdirSync(path.join(racine,dir),{withFileTypes:true}).flatMap(e=>{
    if(['.git','node_modules','__MACOSX','.DS_Store','.wrangler'].includes(e.name)||/^\.env(?:\.|$)/.test(e.name))return [];
    const f=dir?`${dir}/${e.name}`:e.name;
    return e.isDirectory()?walk(f):e.isFile()?[f]:[];
  });
  const suivis = walk().sort();
...
/* Le lot BETA-04 travaille sur une COPIE PHYSIQUE, hors dépôt : `git rev-parse` n'y répond pas.
   Le manifeste doit pourtant nommer un commit, parce qu'un publieur en a besoin pour savoir de quoi
   l'arbre descend. Quand le dépôt est là, il répond ; sinon le commit d'origine de la copie est
   déclaré, et la date dit explicitement que l'arbre porte en plus les modifications du lot — le
   contenu réel, lui, est identifié par les empreintes plus bas, pas par ce commit. */
const COMMIT_ORIGINE_COPIE = '169a74ab23d45589d1a6e89a411d23da019ae167';
let commit;
let dateCommit;
try {
  commit = git('git rev-parse HEAD');
  dateCommit = git('git log -1 --format=%cI');
} catch {
  commit = COMMIT_ORIGINE_COPIE;
  dateCommit = 'Copie physique BETA-04 : descend de ce commit, modifications du lot non commitées';
}
