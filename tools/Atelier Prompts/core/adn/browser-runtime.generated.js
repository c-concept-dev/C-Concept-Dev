 ...
 * source-sha256: 125f7ec7e78834c5d22b5c50cedc4d144df63b88f40b3bab9c37480c1ba408cc
...
const CANDIDATE_FIELD_DEFINITIONS = Object.freeze({
  objective:
...
MISSION
1. Vérifiez que chaque élément matériel du candidat est réellement ancré, via sa provenance déclarée, dans l'une des trois sources contractuelles du tour : original_request, clarification_history, ou le matériau transmis à l'Analyste. Un élément portant la provenance user_provided_material est ancré dans ce matériau : son absence de original_request et de clarification_history n'est donc pas en soi un défaut lorsque material_context.deep_content_available vaut true. Cela ne vous demande aucune confiance aveugle et ne rend la valeur ni vraie, ni suffisante, ni pertinente — vous contrôlez la COHÉRENCE de la provenance déclarée, jamais un contenu que vous ne recevez pas. Une provenance user_provided_material alors que material_context indique deep_content_available false ou "unknown" est incohérente et doit être signalée ; une provenance explicit_user_statement ou clarification_answer portant une valeur introuvable dans la demande ou l'historique l'est tout autant. Listez dans unsupported_additions_found (operational_request_candidate_review) tout élément dont la provenance déclarée ne correspond à rien de réel. Un ajout non tracé n'est pas automatiquement un veto : évaluez sa matérialité (cf. définition MISSION point 3 de l'Analyste) — non tracé et non matériel, il reste simplement consigné dans unsupported_additions_found sans exiger disagreement ; non tracé et matériel, il doit être escaladé en veto qualifié ou en missed_material_issue. Symétriquement, listez dans unsupported_removals_found tout élément matériel d'original_request ou clarification_history ayant silencieusement disparu du candidat, sans provenance ni justification associée.
...
function makeArbiterUserMessage({ original_request, clarification_history = [], analyst_output, critic_output, material_context } = {}) {
  return JSON.stringify({
    original_request: text(original_request),
    clarification_history: list(clarification_history),
    material_context: normalizeMaterialContext(material_context),
...
async function runCriticBatchedPipeline({ original_request, clarification_history = [], analyst_output, previous_vetoes = [], material_context, capability, candidateFamilyGroups } = {}, { executeGlobal, executeBatch, concurrency, signal, telemetry } = {}) {
  ...
  /* Mesures portées par l'objet `telemetry` que cette fonction reçoit DÉJÀ en option. Rien de
     nouveau n'est créé, et sans `telemetry` le comportement est identique : des durées, jamais un
     contenu. */
  ...
  if (telemetry && typeof telemetry === "object") {
    telemetry.global_ms = Date.now() - globalStartedAt;
    telemetry.waves = [];
  }
  ...
  for (let waveStart = 0; waveStart < batchPlan.length; waveStart += waveSize) {
    ...
    if (telemetry && Array.isArray(telemetry.waves)) {
      telemetry.waves.push({
        wave_index: telemetry.waves.length,
        batch_indexes: [...waveIndexes],
        tasks: tasks.length,
        duration_ms: Date.now() - waveStartedAt
      });
    }
  ...
  if (telemetry && typeof telemetry === "object") {
    ...
    telemetry.total_ms = Date.now() - pipelineStartedAt;
...
const SOLICITING_TYPES = Object.freeze(['ASK_CLARIFICATION', 'ASK_CONFIRMATION']);
...
const SILENT_INTERACTION = Object.freeze({
 ...
 */
function countInterrogations(texte) {
...
function countNamedAlternatives(texte) {
  const t = minuscule(texte);
 ...
 */
function guardFastSolicitation(candidate, snapshot = {}) {
...
const DISPLAY_VERDICTS = Object.freeze(['ALLOW', 'REPLACED', 'NOT_DISPLAYABLE']);
...
function isAtomicQuestion(texte) {
  const t = String(texte || '').trim();
  if (!t) return false;
...
const NOMINAL_PLANES = Object.freeze(["core", "legacy"]);
const DEFAULT_NOMINAL_PLANE = "core";
...
const ESCALATION_VERDICTS = Object.freeze([
...
const CORE_CRITIC_VERDICTS = Object.freeze(["PASS", "CORRECT", "ESCALATE_TO_ARBITER"]);
...
5. question_candidates : les questions réellement non substituables, classées par valeur informationnelle décroissante, ou aucune. Ce n'est pas un quota : n'y mettez jamais la conversion mécanique d'un issue en question. CE QU'ELLES DEVIENNENT, ET POURQUOI ELLES COMPTENT. Votre next_question peut être refusée à l'affichage — parce qu'elle demande à la personne de définir ce que nous devons produire, parce qu'elle porte plusieurs manques, ou parce que ce manque a déjà été sollicité. Dans ce cas, c'est UNE DE CES CANDIDATES qui sera montrée, dans l'ordre où vous les avez classées ; s'il n'y en a aucune d'affichable, le tour est perdu et la personne ne reçoit rien. Une candidate n'est donc utile que si elle peut être posée TELLE QUELLE, à la place de la principale : elle vise un manque DIFFÉRENT — jamais la même chose reformulée —, elle ne porte qu'un seul manque, elle n'a pas déjà reçu de réponse dans l'historique, et elle fait progresser réellement vers un résultat produisible. Renseignez pour chacune les mêmes champs que pour next_question, missing_determinant_id compris : une candidate sans identité de manque ne peut pas être protégée contre la répétition. N'en fabriquez aucune pour remplir la liste : une inconnue substituable n'y a pas sa place, et il vaut mieux aucune candidate qu'une question que vous ne poseriez pas vous-même.
 ...
 * l'exigent.
 */
const CORE_JSON_SCHEMA = Object.freeze((() => {
...
function parseCoreOutput(content) {
 ...
 */
function assessEscalation(escalation, turn) {
...
const CORE_CRITIC_SYSTEM_PROMPT = `RÔLE
...
const CORE_CRITIC_JSON_SCHEMA = Object.freeze((() => {
...
function makeCoreCriticUserMessage({ original_request, clarification_history = [], turn, escalation_reason } = {}) {
  return JSON.stringify({
...
function validateCoreCriticOutput(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
...
function parseCoreCriticOutput(content) {
...
const CORE_ARBITER_SYSTEM_PROMPT = `RÔLE
...
const CORE_ARBITER_JSON_SCHEMA = ARBITER_JSON_SCHEMA;

function makeCoreArbiterUserMessage({ original_request, clarification_history = [], turn, divergence } = {}) {
  return JSON.stringify({
    original_request: String(original_request || ""),
    clarification_history: Array.isArray(clarification_history) ? clarification_history : [],
    turn: turn ? clone(turn) : null,
...
const parseCoreArbiterOutput = (content) =>
...
function resolveNominalPlane(env = {}) {
...
return {NOMINAL_PLANES,DEFAULT_NOMINAL_PLANE,ESCALATION_KINDS,ESCALATION_VERDICTS,CORE_CRITIC_VERDICTS,CORE_SYSTEM_PROMPT,CORE_OUTPUT_FIELDS,CORE_JSON_SCHEMA,makeCoreUserMessage,validateCoreInput,validateCoreOutput,parseCoreOutput,assessEscalation,CORE_CRITIC_SYSTEM_PROMPT,CORE_CRITIC_JSON_SCHEMA,makeCoreCriticUserMessage,validateCoreCriticOutput,parseCoreCriticOutput,CORE_ARBITER_SYSTEM_PROMPT,CORE_ARBITER_JSON_SCHEMA,makeCoreArbiterUserMessage,parseCoreArbiterOutput,CORE_ROLE_DEFINITIONS,runCoreFirstTurn,resolveNominalPlane};
})({...ORCORE});
const ORORCH=((deps)=>{
const {isLegalTransition,OPRIE_ROLES,createDegradedRoleResult,validateAnalystInput,validateDegradedRoleResult,DecisionHttpError,TRANSPORT_LIMITS,corsHeaders,jsonResponse,readJsonBody,degradedResultFromProviderChainError,guardDisplayedQuestion,runCoreFirstTurn,resolveNominalPlane}=deps;
...
function isValidAttemptIndex(value) {
 ...
 */
function isValidErrorFingerprint(value) {
 ...
 */
function providerAttemptOccurred(record) {
 ...
 */
function computeJournalComplete(record) {
...
async function runOperationalRequestTurn(input, { executeRole, log = defaultLog, trace = null } = {}) {
  ...
  /* ATTRIBUTION DE LATENCE, PAR ÉTAPE. Elle est permanente, et voici pourquoi : le lot 03A avait
     conclu « le Critique coûte le plus » sans chronométrer, et la mesure a montré trois rôles
     quasi égaux — 33 %, 31 %, 36 % — donc aucune étape dominante à optimiser. Sans ces repères,
     la latence, qui reste le premier défaut produit, se rediagnostique à l'intuition.

     Rien n'est ajouté à la réponse : ces repères partent dans le `log` déjà estampillé par
     l'orchestrateur, donc avec le même invocation_id (le cf-ray de Cloudflare) que tous les autres
     événements. Aucune donnée utilisateur n'y figure : des noms d'étape, des millisecondes, et
     rien d'autre. */
  const t0 = Date.now();
  const ms = () => Date.now() - t0;
  log({ event: "operational_request_deep_start", stage: "OPRIE_DEEP_START", elapsed_ms: 0, status: "ok" });
  ...
  for (const role of OPERATIONAL_REQUEST_ROLE_SEQUENCE) {
    ...
    log({ event: "operational_request_stage", stage: `${role.toUpperCase()}_START`, elapsed_ms: ms(), status: "ok" });
    ...
    log({ event: "operational_request_stage", stage: `${role.toUpperCase()}_END`, elapsed_ms: ms(),
  ...
  log({ event: "operational_request_deep_end", stage: "OPRIE_DEEP_END", elapsed_ms: ms(),
        duration_ms: ms(), status: turn.state });
  return turn;
}

/**
 * Le tour V2, rendu au format exact du tour historique.
 *
 * Deux choses seulement sont faites ici, et elles sont les mêmes que sur le chemin historique : la
 * légalité de l'état est vérifiée contre la machine d'état gelée, et la question affichée passe par
 * la frontière d'atomicité. Les `question_candidates` du Core y servent de source de remplacement,
 * exactement comme celles de l'Analyste sur l'ancien chemin.
 */
...
/**
 * LA FRONTIÈRE D'AFFICHAGE DU PLAN PROFOND.
 *
 * Mesuré deux fois en bêta réelle : la question affichée après 78 secondes était un catalogue de
 * productions — « un itinéraire jour par jour, une sélection de recommandations par thème, une
 * checklist…, ou autre chose ? ». Son auteur a été établi par mesure : l'ANALYSTE l'écrit dans
 * `question_candidates`, l'ARBITRE la recopie dans `next_question`. Le garde d'atomicité la
 * reconnaissait déjà ; il ne gardait qu'une porte, et la personne entrait par l'autre.
 *
 * Ce qui est contraint ici est la FORME de la question affichée, et rien d'autre. L'état reste celui
 * qu'OPRIE a décidé, la readiness reste la sienne, le contrat canonique n'est pas touché. Quand la
...
async function handleOperationalRequest(request, env, { executeRole, log, resolveModel } = {}) {
  ...
  try {
    ...
    /* V2 — CORE FIRST. Le chemin nominal est un appel, et le plan historique reste atteignable :
       ATELIER_NOMINAL_PLANE=legacy rend la séquence Analyste → Critique → Arbitre, à l'identique.
       C'est le retour arrière, et il ne demande aucun redéploiement de code. */
    const plane = resolveNominalPlane(env);
    stampedLog({ event: "nominal_plane", plane });
    const turn = plane === "core"
      ? await runCoreFirstTurnForRequest(input, { executeRole, log: stampedLog, trace })
      : await runOperationalRequestTurn(input, { executeRole, log: stampedLog, trace });
...
return {OPERATIONAL_REQUEST_ROLE_SEQUENCE,OPERATIONAL_REQUEST_TURN_ORIGIN_STATE,runOperationalRequestTurn,applyDisplayGuardToTurn,assertOrchestratedRolesCoverOprie};
})({...ORSTATE,...ORCORE,...DECISIONCORE,...ROLEDEG,...SOLICIT,...COREPLANE});
...
function createTurnSnapshot({ turn_id, original_request, clarification_history = [], current_answer = null, canonical_version = 0, material_present = false } = {}) {
