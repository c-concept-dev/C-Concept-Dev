 ...
 * source-sha256: 53c005721ed646f478a2b3e5ed94dd280681cb823bb1e8c703f5a35808ca7b99
...
const CONVERSATION=(()=>{
const CONVERSATION_ORCHESTRATOR_VERSION = "1.0";
...
function conversationQuestionsSimilar(left, right) {
  const a = words(left);
  const b = words(right);
  if (!a.size || !b.size) return text(left).toLowerCase() === text(right).toLowerCase();
  let common = 0;
 ...
 */
function nextConversationAction({
...
function createConversationAuditEvent(action, {
  turn,
...
function validateConversationAuditEvent(event) {
  const keys = [
...
return {CONVERSATION_ORCHESTRATOR_VERSION,CONVERSATION_STATES,conversationQuestionsSimilar,nextConversationAction,createConversationAuditEvent,validateConversationAuditEvent};
})();
const CANON=(()=>{
...
function makeAnalystUserMessage({ original_request, clarification_history = [] } = {}) {
...
function makeCriticUserMessage({ original_request, clarification_history = [], analyst_output, previous_vetoes = [] } = {}) {
  return JSON.stringify({
...
function validateOutputAgainstCanonicalContract({ canonical_contract, output, checks } = {}) {
  if (!canonical_contract || typeof canonical_contract !== 'object' || Array.isArray(canonical_contract)
...
global.__ATELIER_ADN_RUNTIME__=Object.freeze({...ADN,...LOCKS,...ROUTING,...READINESS,...CONVERSATION,...CANON,...ARCHENRICH,...ORSTATE,...DECISIONCORE,...PROVIDERHA,...ORCORE,...ROLEDEG,...ORORCH,...RAPIDEENRICH,...OUTPUTQG,...QG,...MANUAL,...ADAPTERS,source_sha256:'53c005721ed646f478a2b3e5ed94dd280681cb823bb1e8c703f5a35808ca7b99'});
})(window);
