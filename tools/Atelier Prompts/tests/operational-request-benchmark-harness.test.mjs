...
test("Groq : le benchmark envoie exactement le même prompt, schéma et paramètres d'inférence que le runtime", async (t) => {
  ...
  // 3F.3.3-S3 : réutilise makeCriticUserMessage (comme le runtime, via ROLE_DEFINITIONS) plutôt que
  // de retaper le corps à la main, pour que cette parité reste vraie même quand le message évolue
  // (question_review_targets), sans avoir à dupliquer chaque futur champ dans ce fichier.
  await callGroq("critic", CRITIC_SYSTEM_PROMPT, makeCriticUserMessage({ original_request: "x", clarification_history: [], analyst_output: analystOutputStub, previous_vetoes: [] }), CRITIC_JSON_SCHEMA);
