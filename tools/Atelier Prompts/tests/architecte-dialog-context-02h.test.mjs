...
test('T-02H-05 : aucune fuite d’un dialogue antérieur — `resetAll` vide l’unique source', () => {
  ...
  /* BETA-04 — l'effacement a été NOMMÉ, pas déplacé. `state.answers` n'a qu'un écrivain, et deux
     chemins l'appellent : une nouvelle demande, et une demande réécrite. C'est cet écrivain unique
     que T-CLEAN02-05 exige, et c'est lui qu'on suit ici plutôt que son emplacement d'hier. */
  const ecrivain = html.slice(html.indexOf('function v11ForgetDialogue(){'), html.indexOf('function resetAll(){'));
  assert.match(ecrivain, /state\.answers\s*=\s*\[\]/, 'l’écrivain unique remet l’historique à zéro');
  const i = html.indexOf('function resetAll(){');
  assert.notEqual(i, -1);
  const corps = html.slice(i, i + 400);
  assert.match(corps, /v11ForgetDialogue\(\)/, 'et une nouvelle demande passe par lui');
