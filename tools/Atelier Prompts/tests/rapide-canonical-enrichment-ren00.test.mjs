...
test('T-REN-06 « exactement 7 » produit une quantité EXACTE, jamais min = max', () => {
  ...
  /* 02F-bis : la cible est relue dans le texte d'origine — elle garde ses accents et sa casse,
     parce qu'elle est écrite telle quelle dans le prompt livré à la personne. */
  assert.deepEqual(clone(contract.quantities), [{
    target: 'idées', unit: null, exact: 7, min: null, max: null, source: 'derived_deterministic'
  }]);
...
test('T-REN-07 les bornes basses sont dérivées, et seulement quand elles sont dites', () => {
  ...
  assert.deepEqual(deriveQuantityFromRequest('Donne au moins 3 idées.'), { exact: null, min: 3, max: null, target: 'idées', rule: 'lower_bound' });
  assert.deepEqual(deriveQuantityFromRequest('Donne 3 minimum.'), { exact: null, min: 3, max: null, target: null, rule: 'lower_bound_suffix' });
  assert.deepEqual(deriveQuantityFromRequest('Donne 5 idées.', { counting_units: UNITES }), { exact: null, min: 5, max: null, target: 'idées', rule: 'counted_unit' });
...
test('T-REN-08 les bornes hautes et les fourchettes sont dérivées fidèlement', () => {
  assert.deepEqual(deriveQuantityFromRequest('Donne au plus 9 idées.'), { exact: null, min: null, max: 9, target: 'idées', rule: 'upper_bound' });
  assert.deepEqual(deriveQuantityFromRequest('Donne entre 3 et 5 idées.'), { exact: null, min: 3, max: 5, target: 'idées', rule: 'range' });
  assert.deepEqual(deriveQuantityFromRequest('Donne entre 5 et 3 idées.'), { exact: null, min: 3, max: 5, target: 'idées', rule: 'range_reversed' });
