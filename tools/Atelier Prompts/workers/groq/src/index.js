...
export const FAST_INTERACTION_SYSTEM_PROMPT = [
  ...
  "CE REGISTRE EST INDÉPENDANT DE LA DÉCISION DU TOUR. Il contient ce que la personne a déclaré ne",
  "pas connaître, même quand vous rendez ASK_CLARIFICATION sur une AUTRE variable :",
...
export function makeFastInteractionUserMessage(snapshot) {
  return JSON.stringify({
    ...
    /* BETA-04 : la PRÉSENCE d'un matériau, jamais son contenu. Sans elle, le plan rapide demandait
       un texte que la personne venait de fournir. */
    materiau_fourni: snapshot.material_present === true
  });
}
  ...
  const providers = order.map((name) => ({
    ...
    execute: async () => {
      try {
        ...
        /* BETA-04 — garde d'atomicité DÉTERMINISTE : aucun appel fournisseur supplémentaire.
 ...
 */
export async function runCriticWithGroq(input, env, { retryOverrides = {}, telemetry } = {}) {
 ...
 */
export async function runCriticWithGroqFanOut(input, env, { candidateFamilyGroups, retryOverrides = {}, telemetry } = {}) {
  const criticRetryOverrides = { ...ROLE_GROQ_RETRY_POLICIES.critic, ...retryOverrides };
  const pacer = createGroqRateLimitPacer({ sleepFn: retryOverrides.sleepFn });
  return runCriticBatchedPipeline(
 ...
 */
export async function runCriticWithAnthropic(input, env, { telemetry } = {}) {
  return runCriticBatchedPipeline(
    { ...input, capability: groqCriticBatchPlanCapability(input) },
    {
...
function parseRoleOutput(role, content, provider) {
  try {
    return ALL_ROLE_DEFINITIONS[role].parseOutput(content);
  } catch (error) {
 ...
 */
export async function runCriticWithOpenAI(input, env, { telemetry } = {}) {
  return runCriticBatchedPipeline(
    { ...input, capability: groqCriticBatchPlanCapability(input) },
    {
      /* M-03 — même activation, même source : ce fournisseur n'a ni stimulateur
...
/* BETA-04 — LES TROIS ENTRÉES TRANSMETTENT LEURS OPTIONS.
   Deux d'entre elles jetaient silencieusement le troisième argument : seul `groq` le passait. Le
   défaut s'est révélé en mesurant — le Critique tournait sur Anthropic, et l'objet de mesure fourni
   par l'appelant n'arrivait jamais. Tout ce que l'appelant confie à un pipeline doit lui parvenir,
   quel que soit le fournisseur : sans quoi une option n'est honorée que par hasard. */
const CRITIC_PIPELINES = Object.freeze({
  groq: (input, env, options) => runCriticWithGroq(input, env, options),
  anthropic: (input, env, options) => runCriticWithAnthropic(input, env, options),
  openai: (input, env, options) => runCriticWithOpenAI(input, env, options)
});

/* V2 — LES RÔLES DU CORE REJOIGNENT LE REGISTRE, ILS NE LE DOUBLENT PAS.
 * Les adaptateurs de fournisseur, la chaîne HA, la validation de contrat et le parseur de sortie
 * sont ceux qui existaient : un rôle V2 s'exécute par le même chemin qu'un rôle historique. */
...
export async function runRoleWithHaChain(role, input, env, { order = ROLE_PROVIDER_ORDER, log, retryOverrides, corrective = null } = {}) {
  ...
  return avecReleveDeChaine(role, order, env, log, (observer) => runProviderChain({
    ...
    providers: order.map((name) => ({
      ...
      execute: isCritic
        ? async () => {
            /* BETA-04 — l'objet `telemetry` que le pipeline sait déjà remplir n'était jamais
               fourni en production : les vagues de review se mesuraient donc à l'aveugle. Il l'est
               ici SEULEMENT quand un `log` existe, et son contenu est journalisé — des compteurs
               et des millisecondes, jamais une issue, jamais un texte utilisateur. */
            const chrono = typeof log === "function" ? {} : null;
            try {
              const out = await CRITIC_PIPELINES[name](input, env, {
                ...(retryOverrides ? { retryOverrides } : {}),
                ...(chrono ? { telemetry: chrono } : {})
              });
              if (chrono) log({
                event: "critic_pipeline_timing", stage: "CRITIC_PIPELINE", provider: name, status: "ok",
                duration_ms: chrono.total_ms ?? null,
                global_ms: chrono.global_ms ?? null,
                waves: Array.isArray(chrono.waves) ? chrono.waves.length : null,
                wave_durations_ms: Array.isArray(chrono.waves) ? chrono.waves.map((w) => w.duration_ms) : null,
                batches_planned: chrono.batches_planned ?? null,
                batches_launched: chrono.batches_launched ?? null,
                early_stop: chrono.early_stop ?? null,
                early_stop_reason: chrono.early_stop_reason ?? null
              });
              return out;
            } catch (error) {
              if (chrono && typeof log === "function") log({
                event: "critic_pipeline_timing", stage: "CRITIC_PIPELINE", provider: name, status: "error",
                duration_ms: chrono.total_ms ?? null, waves: Array.isArray(chrono.waves) ? chrono.waves.length : null
              });
...
export default {
  fetch(request, env) {
    ...
    if (new URL(request.url).pathname === FAST_INTERACTION_PATHNAME) {
      /* BETA-04 — le plan rapide journalise désormais sous le MÊME identifiant d'invocation que le
         plan profond : c'est le cf-ray, déjà résolu pour /operational-request. Sans lui, un refus
         de schéma rapide n'était rattachable à rien, et il est resté muet pendant toute une bêta. */
      const invocation_id = resolveInvocationId(request);
      const log = (event) => console.log(JSON.stringify({ ...event, invocation_id }));
      return handleFastInteractionRequest(request, env, {
        executeFast: (snapshot, fastEnv) => runFastInteractionWithHaChain(snapshot, fastEnv, { order: resolveFastProviderOrder(fastEnv), log }),
        log
      });
    }
