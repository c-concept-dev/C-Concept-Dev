...
# BETA-04, SECOND TOUR — LE OWNER-BETA A INVALIDÉ MON VERDICT

Deux essais réels du propriétaire, sur « Je veux préparer un voyage à Lisbonne au printemps. », ont
rendu après une très longue attente un catalogue de productions. P0-B et P0-D étaient donc encore
ouverts, et mon verdict précédent était trop étroit : je gardais une porte, et la personne entrait
par l'autre.

## Ce que je n'ai pas pu faire, et pourquoi

Les deux invocations du propriétaire **n'ont pas pu être retrouvées dans les journaux**.
L'observabilité est bien activée sur le service (`observability.enabled = true`,
`head_sampling_rate = 1`), mais le jeton disponible ici porte `workers_tail:read` — le flux live — et
aucun droit de lecture d'observabilité : l'API de requête répond HTTP 403. Lire un journal passé
demanderait un jeton avec « Workers Observability Read », que je ne peux pas créer sans le tableau de
bord. Je n'ai donc pas deviné : j'ai **reproduit**, sur le runtime déployé, et chaque chiffre ci-dessous
porte l'identifiant d'invocation de sa propre reproduction.

## Diagnostic, par mesure

| Champ | Reproduction 1 | Reproduction 2 (après correctif rapide) |
|---|---|---|
| INVOCATION_ID | `a3a56bcc5861d128` | `a3a57c07ae720071` |
| FAST_TYPE | `WAIT_FOR_DEEP_VALIDATION` | `ASK_CLARIFICATION` |
| FAST_MS | 536 – 702 | 503 – 645 |
| DEEP_STARTED | OUI | oui, mais après la question |
| ANALYST_MS | 23 199 | 23 877 |
| CRITIC_MS | 26 275 | 27 019 |
| ARBITER_MS | 29 017 | 31 573 |
| DEEP_TOTAL_MS | 78 734 | 82 469 |
| Question affichée | catalogue de quatre productions | « Combien de jours prévoyez-vous de passer à Lisbonne ? » |
| FINAL_QUESTION_ORIGIN | **ANALYST** | ANALYST, forme gardée |

### La preuve de l'origine

L'Analyste a été appelé SEUL, sur la route `/analyst` du runtime déployé. Ses
`question_candidates` contenaient exactement deux entrées :

1. « Quel type de résultat vous serait le plus utile : un itinéraire jour par jour, des
   recommandations organisées par thème (visites, restaurants, quartiers…), une checklist pratique
   pour organiser le voyage, ou autre chose ? »
2. « Combien de jours prévoyez-vous de passer à Lisbonne ? »

Le catalogue est donc **écrit par l'Analyste**, et l'Arbitre le recopie dans `next_question`. Ce n'est
ni le Critique, ni l'Arbitre, ni un repli. Et le fait décisif est la seconde candidate : **le système
fabriquait déjà la question atomique, et affichait l'autre.**

## ROOT_CAUSE

Deux causes distinctes, et il fallait les deux pour produire ce que le propriétaire a vu.

1. **Le garde d'atomicité ne gardait qu'une porte.** Il s'appliquait aux sollicitations du plan
   rapide, et à elles seules. La question du plan profond ne passait devant aucun contrôle — alors
   que le même garde la reconnaissait déjà comme un catalogue. L'invariant était vrai dans le code et
   faux à l'écran.
2. **Le plan rapide ne posait pas la question qu'il devait poser.** Pour une demande qui nomme une
   INTENTION (« préparer un voyage ») sans nommer ce qu'il faut produire, il répondait
   `WAIT_FOR_DEEP_VALIDATION` : la personne attendait 78 secondes pour obtenir, au bout, une mauvaise
   question.

### WHY_FAST_DID_NOT_ASK

Le protocole disait déjà « si la demande nomme déjà ce qu'il faut produire, elle est exploitable :
ne demandez rien », mais il ne disait nulle part l'inverse. Le modèle de production fait 20 milliards
de paramètres ; le critère était enterré au milieu de 3 178 caractères, et il l'a appliqué dans un
seul sens. Mesuré : **1 sollicitation sur 6 demandes d'intention**. Le déplacement de ce test en TÊTE
du protocole, formulé comme premier examen, porte cette mesure à **5 sur 6**. Ce n'est pas un ajout
de texte : c'est l'ordre qui a changé le comportement.

## FIX

**1. Une porte unique pour toute question affichée.** `guardDisplayedQuestion` vit dans le même
module que le garde du plan rapide — une seule définition de l'atomicité, pour Fast comme pour
l'Analyste, le Critique, l'Arbitre, un repli, un mode dégradé ou une reprise. Quatre issues, aucune
n'appelant un modèle :
...
| `REDUCED` | la question porte sa forme atomique en tête : on coupe ce qui l'encombre |
| `FALLBACK_SAFE` | plus rien n'est récupérable : « Quel résultat principal souhaitez-vous obtenir ? » |

La réduction **coupe, elle ne rédige pas** : chaque mot du résultat vient du texte d'origine, et
`T04-22c` le vérifie mot par mot. Elle refuse trois choses : un fragment de moins de quatre mots, un
deux-points survivant, et un résultat qui reste une énumération — couper le « ou » final d'un
catalogue n'en retire que le marqueur.

Ce garde ne touche ni l'état, ni la readiness, ni le contrat canonique : **OPRIE reste seule autorité
sur ce qu'il faut demander**. Il contraint la forme de ce qui s'affiche, et rien d'autre.

**2. Trois trous d'atomicité, trouvés en mesurant, fermés.**

- **Proposition à verbe inversé.** « Quel est le délai prévu ET AVEZ-VOUS déjà identifié un
  logement ? » passait : la seconde proposition n'a pas de mot interrogatif. Elle compte désormais —
  mais seulement après « et », une virgule ou un point-virgule, jamais après « ou », qui offre le
  plus souvent les deux branches d'UN seul besoin.
- **Énumération sans déterminants.** « Itinéraire, recommandations, checklist, ou autre chose ? »
  passait. Un marqueur de choix — deux-points ou « ou » — plus trois segments suffisent à la
  reconnaître, sans rien connaître du sujet.
- **Parenthèse qui énumère.** Trouvée sur le runtime, après ma première correction :
  « Quel type de CV souhaitez-vous produire (format, contenu, objectif professionnel…) ? ». Tête
  interrogative parfaite, trois dimensions offertes. La parenthèse est retirée par réduction. Une
  parenthèse sans virgule précise ; elle n'énumère pas, et elle reste.

**3. Le protocole rapide.** Premier examen en tête : la demande nomme-t-elle ce qu'il faut produire,
ou seulement une intention ? Et une ligne qui ne devrait pas être nécessaire — « Répondez toujours en
français » — parce que le modèle a rendu « Got it! » pendant les mesures.

Aucun reviewer fournisseur ajouté. Le garde reste synchrone, local, et coûte zéro appel.

## Résultat, mesuré sur le runtime servi

| Champ | Valeur |
|---|---|
| FIRST_INTERACTION_MS_AFTER | **645 ms** (mesures répétées : 314 à 645) |
| QUESTION_AFTER | « Quel type de résultat souhaitez‑vous obtenir pour votre préparation de voyage à Lisbonne ? » |
| QUESTION_ATOMIC | **OUI** — une interrogation, aucune énumération, aucune production proposée |
| QUESTION_CATALOGUE | NON |
| DEEP_BEFORE_FIRST_QUESTION | **0** |
| Question du plan profond, même demande | « Combien de jours prévoyez-vous de passer à Lisbonne ? » — verdict journalisé `REPLACED`, 3 candidates disponibles |

Corpus adversarial, sur le runtime déployé — même invariant pour tous, aucun mot de domaine dans le
correctif :

| Demande | Avant | Après |
|---|---|---|
| préparer un voyage (Lisbonne) | WAIT, puis catalogue à 78 s | ASK atomique, 558 ms |
| préparer une présentation | WAIT | WAIT — la production est nommée |
| organiser un déménagement | ASK non atomique | ASK atomique, 333 ms |
| préparer une réunion | WAIT | ASK atomique, 573 ms |
| améliorer un CV | WAIT | ASK atomique, 566 ms |
| préparer un week-end (Rome) | WAIT | ASK atomique, 365 ms |
| comparaison en tableau | WAIT | WAIT — exploitable |
| explication en cinq paragraphes | WAIT | WAIT — exploitable |

## Ce qui reste vrai, et qu'il faut dire

**La latence n'a pas bougé** : 82 s pour un tour profond. Ce que ce lot change, c'est qu'on ne
l'attend plus pour obtenir la première question. P0-A reste OUVERT, et sa cause est structurelle.

**La nécessité d'une question reste un jugement de modèle.** 5 sur 6, pas 6 sur 6 : « Aide-moi à
préparer une présentation » nomme une production, et le modèle se tait — défendable, mais c'est lui
qui tranche, pas une garantie. Ce qui est désormais **garanti**, sans dépendre d'aucun modèle : aucune
question non atomique ne peut s'afficher, quelle qu'en soit la source.

## Chaînes et déploiement

| | |
|---|---|
| GLOBAL | **3 105 tests, 3 105 PASS** |
| FROZEN | **OK** — sept plages inchangées |
| NEW_VERSION | `77c65407-47e1-4e1b-b543-1e6096a1ff63` |
| NEW_DEPLOYMENT_ID | `3e4d0d05-9001-4040-9c7a-ed0fbfa3bc2b` |
| ACTIVE_BUNDLE_SHA256 | `f2ac0ef5e6cda7d42cbfb6bfdf8b858cc508ad2e56fc35b1f5f19b0d0ab4d3d5` |
| SOURCE_RUNTIME_MATCH | **YES** — `scriptVersion.id` servi = version déployée |
