...
export function scoreCriticOutput(output, oracle = {}, context = {}) {
  ...
  // 3F.3.3-S2, B-01B : question_substitution_review — cardinalité et ciblage par rapport aux VRAIES
  // issues Analyst (material + question), la seule chose que le validateur ne peut pas vérifier seul
  // (il n'a jamais accès à analyst_output — même choix architectural que ci-dessous pour
  // illegitimate_question_found). Le scorer ne juge JAMAIS reasonably_available, question_is_last_resort
  // ou available_alternative : ces jugements restent exclusivement ceux du LLM Critic, déjà vérifiés
  // structurellement par validateCriticOutput (six alternatives exactes, cohérence last_resort,
  // cohérence avec illegitimate_question_found). N'ajoute aucun critère quand le tableau est vide ET
  // qu'aucune issue material+question n'existe côté Analyst : un candidat sans question n'a rien à
  // auditer, jamais un critère tautologique.
  if (context.analyst_output) {
    const targetableIssueIds = context.analyst_output.issues
      .filter((issue) => issue.impact === "material" && issue.recommended_treatment === "question")
      .map((issue) => issue.id);
    if (output.question_substitution_review.length > 0 || targetableIssueIds.length > 0) {
      const analystIssueById = new Map(context.analyst_output.issues.map((issue) => [issue.id, issue]));
      // "Fausse revue" (mission S2 §33) : une entrée cible une issue inexistante, non matérielle, ou
      // dont le traitement réel n'est pas "question".
      const falseReviews = output.question_substitution_review.filter((review) => {
        const issue = analystIssueById.get(review.issue_id);
        return !issue || issue.impact !== "material" || issue.recommended_treatment !== "question";
      });
      criteria.push(verdict(
        "question_substitution_review_targets_valid_issue",
        falseReviews.length === 0,
        falseReviews.length ? `revue(s) ciblant une issue inexistante, non matérielle, ou dont le traitement réel n'est pas "question" : ${JSON.stringify(falseReviews.map((r) => r.issue_id))}` : null,
        "detection"
      ));
      // "Revue manquante" (mission S2 §32) : une issue material+question de l'Analyste sans entrée
      // correspondante dans question_substitution_review.
      const reviewedIssueIds = output.question_substitution_review.map((r) => r.issue_id);
      const missing = targetableIssueIds.filter((id) => !reviewedIssueIds.includes(id));
      criteria.push(verdict(
        "question_substitution_review_covers_all_targetable_issues",
        missing.length === 0,
        missing.length ? `issue(s) material+question sans revue correspondante dans question_substitution_review : ${JSON.stringify(missing)}` : null,
        "detection"
      ));
    }
  }
