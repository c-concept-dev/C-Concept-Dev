...
// PW-D090-RES-001 (corrigé une 2e fois) — synchronisation de la sélection.
// D090B-TEST-001 a révélé une erreur de conception du test, pas un bug produit :
// l'application utilise volontairement deux vocabulaires distincts. La carte
// affiche à la fois l'orientation (.kicker, ex. "Agréable") et le nom généré
// du parcours (.route-name, ex. "L'agréable"). Le badge carte et le contexte
// de détail reprennent uniquement l'orientation (routeUiMeta().label), jamais
// le nom généré. On compare donc chaque vue avec le niveau de libellé qu'elle
// affiche réellement, au lieu de supposer qu'un seul texte doit apparaître partout.
test("D090B la sélection reste synchronisée sur les quatre vues", async ({ page }) => {
  ...
  const orientationLabel = (await targetCard.locator(".kicker").textContent())?.trim();
  ...
  await expect(mapBadge).toContainText(orientationLabel);

  const detailContext = page.locator(".selected-route-context");
  await expect(detailContext).toContainText("Parcours sélectionné");
  await expect(detailContext).toContainText(orientationLabel);
});

// ST-NAV-001 (corrigé) — rotation et redimensionnement pendant la navigation
// plein écran. D090B-HARNESS-002 a montré que sous Chromium/Android, l'appel
// applicatif à requestFullscreen() engage un vrai plein écran natif au niveau
// du système d'exploitation ; le protocole de contrôle du navigateur refuse
// alors tout redimensionnement de fenêtre tant que ce plein écran natif n'est
// pas quitté (limitation de l'outil d'automatisation, pas de l'application).
// On neutralise donc requestFullscreen()/exitFullscreen() uniquement pour ce
// test, afin d'isoler ce qu'on cherche réellement à vérifier : la règle CSS
// body.navigating .results reste prioritaire quel que soit l'ordre du fichier,
// indépendamment du comportement natif du moteur.
test("D090B la carte reste plein écran (CSS) après rotation pendant le guidage", async ({ page }) => {
