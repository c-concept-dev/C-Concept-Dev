(() => {
  ...
  function saveD103ActivityIntent(intent) {
    ...
    const nextDocument = activityIntentHomeCore.chooseActivityIntent(longitudinalDocumentOrNull(), intent, { now });
  ...
  // Couleurs distinctes par type de point d'intérêt, alignées sur la charte
  // JMMJS V1 quand c'est possible. "Curiosité locale" sort de la charte
  // (violet) faute de 7e couleur définie — à revoir si besoin.
  const POI_MARKER_COLORS = Object.freeze({
    "Boulangerie": "#C9A15A",
    "Café": "#7A5230",
    "Restaurant": "#B5502E",
    "Point de vue": "#1B5E70",
    "Pique-nique": "#8A9A5B",
    "Patrimoine": "#3A3628",
    "Curiosité locale": "#6B5B95",
  });
  function poiMarkerColor(type) {
    return POI_MARKER_COLORS[type] || "#8a8578";
  }
  // Rend la liste complète (panneau) et les marqueurs carte, filtrés sur les
  // envies effectivement cochées — à la façon d'une recherche par catégorie,
  // pas un nuage générique de tous les points à proximité. N'effectue aucun
  // appel réseau : suppose route.pois déjà peuplé (enrichissement automatique
  // pendant le calcul, ou récupération manuelle via loadPois()).
  function renderPoiDisplay(route) {
    const pois = Array.isArray(route?.pois) ? route.pois : [];
    E.poiContent.innerHTML = pois.length
      ? '<div class="poi-list">' +
        pois
          .map((p) => {
            const view = describePoi(p);
            return '<article class="poi-item"><strong>' + esc(view.name) + "</strong><span>" + esc(view.type) + " · " + view.distance + " m de la trace · détour estimé " + view.detourMeters + " m</span><span>Source : " + esc(view.source || "Source non précisée") + "</span><span>" + esc(view.hoursLabel) + " · " + esc(view.presenceLabel) + "</span><span>" + esc(view.accessibilityLabel) + "</span></article>";
          })
          .join("") +
        "</div>"
    ...
    S.poiLayers = mapPois.map((p) => {
      const color = poiMarkerColor(p.type);
      return L.circleMarker([p.lat, p.lon], {
        ...
        .addTo(S.map)
        .bindPopup(
          "<strong>" + esc(p.name) + "</strong><br>" + esc(p.type) + " · " + p.distance + " m",
        );
    });
  }
